...
def read_NLDAS(file, latv, lonv):
    ...
    tbot  = ds['Tair'].values[ 0, lat_i[0], lon_j[0]]  #degree
    ...
    U     = ds['Wind_E'].values[0, lat_i[0], lon_j[0]]
    V     = ds['Wind_N'].values[0, lat_i[0], lon_j[0]]
    wind  = np.sqrt(U**2 + V**2)
    
    q     = ds['Qair'].values[0, lat_i[0], lon_j[0]]
    L = 2.5 * 10**6; Rv = 461
    e  = ( psrf/100 * q ) / 0.622
...
lon = np.array([[283.4375]]);   lai = np.array([[38.8125]])
lat_wkg = 38.8125; lon_wkg = -76.5625
edgew = np.array([283.4375]); edgee = np.array([283.4375])
edges = np.array([38.8125]);   edgen = np.array([38.8125])

'''Read variables'''
start_yr = 2022; end_year = 2024
timearray = datetime.datetime(start_yr, 2, 1)
start_date = datetime.datetime(1979, 2, 1) #file start date to get skip row
nmonth = ( end_year - start_yr ) * 12 - 1 # no 1979-01-01 data
...
for i in range(1, nmonth+1, 1):
    ...
    for i in range(0, nrow, 1):
        ...
        filename = 'NLDAS_FORA0125_H.A'+\
                   date_str +'00.020.nc'
        print(filename) 
        filepath = '/xdisk/chopinsong/chopinsong/CTSM_inputdata/climforcing/'+ \
                   'NLDAS/NLDAS_forcing/'
    ...
    ds_test.to_netcdf("/home/u8/sdwilson/data/EFC"+\
                      "/site3/1x1pt_site3/CLM1PT_data/{strs}.nc".\
