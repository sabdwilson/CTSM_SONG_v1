module CNNStateUpdate1Mod
  ...
  use clm_time_manager                , only : get_step_size_real,get_curr_date, get_curr_calday
  use clm_varpar                      , only : nlevdecomp, ndecomp_pools, ndecomp_cascade_transitions
  use clm_varpar                      , only : i_met_lit, i_cel_lit, i_lig_lit, i_cwd, i_met_bio, i_lig_bio ! Sabrina
  use clm_varctl                      , only : iulog, use_nitrif_denitrif,use_biochar, init_bio_yr, Biochar_Tyear
...
contains
  ...
  subroutine NStateUpdate1(num_soilc, filter_soilc, num_soilp, filter_soilp, &
    ...
    !
    ! !LOCAL VARIABLES:
    integer :: c,p,j,l,g,k, d ! indices Sabrina
      ...
      do j = 1, nlevdecomp
         do fc = 1,num_soilc
            ...
            if (use_biochar) then ! Sabrina
                ...
                if (d>0 .and. d<=Biochar_Tyear) then
                   if ((mcsec>= 8*3600 .and. mcsec <= 16*3600) .and. (jday==decomp_cascade_con%Biochar_day(d)) .and. (kyr==decomp_cascade_con%Biochar_yr(d))) then
                       nf_soil%decomp_npools_sourcesink_col(c,j,i_met_bio) =((1-bio_Rfrac)*(decomp_cascade_con%Biochar_Amt(d)/8))/bio_met_cn 
                       nf_soil%decomp_npools_sourcesink_col(c,j,i_lig_bio) =(bio_Rfrac*(decomp_cascade_con%Biochar_Amt(d)/8))/bio_rec_cn
                   end if
                end if
              ! print *, "CNNUpdate met bio sink, lig bio sink",nf_soil%decomp_npools_sourcesink_col(c,j,i_met_bio),nf_soil%decomp_npools_sourcesink_col(c,j,i_lig_bio)
            end if
