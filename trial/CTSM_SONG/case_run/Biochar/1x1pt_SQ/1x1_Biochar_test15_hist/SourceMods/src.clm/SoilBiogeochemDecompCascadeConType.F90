module SoilBiogeochemDecompCascadeConType
 ...
 !
  ...
  type, public :: decomp_cascade_type
     ...
     integer           , pointer  :: Biochar_yr(:)          !Sabrina
     integer           , pointer  :: Biochar_day(:)
     real(r8)          , pointer  :: Biochar_Amt(:)
...
contains
  ...
  !------------------------------------------------------------------------
  subroutine init_decomp_cascade_constants(use_century_decomp,use_biochar,Biochar_Tyear )
    !
    ! !DESCRIPTION:
    ! Initialize decomposition cascade state
    !------------------------------------------------------------------------
    ! !ARGUMENTS:
    logical, intent(IN) :: use_century_decomp, use_biochar
    integer, intent(IN) :: Biochar_Tyear
    ! !LOGAL VARIABLES:
    ...
    allocate(decomp_cascade_con%spinup_factor(1:ndecomp_pools))
        ! Sabrina Mod
    if (use_biochar) then
         write (5000,*) "Biochar_Tyear", Biochar_Tyear,use_biochar
         allocate(decomp_cascade_con%Biochar_Amt(1:Biochar_Tyear))
         allocate(decomp_cascade_con%Biochar_yr(1:Biochar_Tyear))
         allocate(decomp_cascade_con%Biochar_day(1:Biochar_Tyear))    
    end if
    ...
    decomp_cascade_con%spinup_factor(1:ndecomp_pools)                     = nan
        ! Sabrina Mod
    if (use_biochar) then
         decomp_cascade_con%Biochar_Amt(1:Biochar_Tyear)                     = 0._r8
         decomp_cascade_con%Biochar_yr(1:Biochar_Tyear)                      = 0
         decomp_cascade_con%Biochar_day(1:Biochar_Tyear)                     = 0 
    end if
