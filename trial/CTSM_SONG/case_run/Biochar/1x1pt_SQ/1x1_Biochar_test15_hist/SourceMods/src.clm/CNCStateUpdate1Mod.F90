module CNCStateUpdate1Mod
  ...
  use clm_varctl                         , only : use_fates, use_cn, iulog,use_biochar, init_bio_yr, Biochar_Tyear ! Sabrina
  !
  implicit none
  private
  !
...
contains
  ...
  subroutine CStateUpdate1( num_soilc, filter_soilc, num_soilp, filter_soilp, &
    ...
    !
    ! !LOCAL VARIABLES:
    integer  :: c,p,j,k,l,d ! indices Sabrina
    integer  :: fp,fc     ! filter indices
    real(r8) :: dt        ! radiation time step (seconds)
    real(r8) :: check_cpool
    real(r8) :: cpool_delta
    real(r8) :: bio_Rfrac ! Sabrina
    real(r8) :: bio_met_cn
    real(r8) :: bio_rec_cn
    integer  :: jday
    integer  :: kmo
                ...
                if (use_biochar) then ! Sabrina
                   ...
                   if (d>0 .and. d<= Biochar_Tyear) then
                        if ((mcsec >= 8*3600 .and. mcsec <= 16*3600) .and.(jday==decomp_cascade_con%Biochar_day(d)) .and. (kyr==decomp_cascade_con%Biochar_yr(d))) then
                           cf_soil%decomp_cpools_sourcesink_col(c,j,i_met_bio) =(1-bio_Rfrac)*(decomp_cascade_con%Biochar_Amt(d)/8)
                           cf_soil%decomp_cpools_sourcesink_col(c,j,i_lig_bio)=bio_Rfrac*(decomp_cascade_con%Biochar_Amt(d)/8)
                        end if
                   end if
                 ! print *, "CNCUpdate met bio sink, lig bio sink",cf_soil%decomp_cpools_sourcesink_col(c,j,i_met_bio),cf_soil%decomp_cpools_sourcesink_col(c,j,i_lig_bio)
                end if
