module SoilBiogeochemDecompCascadeCNMod
  ...
  use clm_varpar                         , only : i_met_lit, i_cel_lit, i_lig_lit, i_cwd, i_met_bio, i_lig_bio ! Sabrina        
  use clm_varctl                         , only : iulog, spinup_state, anoxia, use_lch4, use_vertsoilc, use_fates, use_biochar
  ...
  type, private :: params_type
     ...
     real(r8):: rf_l4s1_cn      !Sabrina respiration frac met bio to SOM1
     real(r8):: rf_l5s3_cn      !Sabrina resp frac lig bio to SOM3
     ...
     real(r8) :: k_l4_cn        !decom rate met biochar Sabrina
     real(r8) :: k_l5_cn        !decomp rate lig biochar 
...
contains
  ...
  subroutine readParams ( ncid )
    ...
    if (use_biochar) then ! Sabrina
        params_inst%rf_l4s1_cn = 0.39_r8
        params_inst%rf_l5s3_cn = 0.29_r8
        params_inst%k_l4_cn = 0.000410958_r8
        params_inst%k_l5_cn = 0.00001598_r8
    end if 
  ...
  subroutine init_decompcascade_cn(bounds, soilbiogeochem_state_inst)
    ...
    real(r8) :: cn_s3
    real(r8) :: cn_s4
    real(r8) :: rf_l4s1      !Sabrina resp frac met bio to SOM1
    real(r8) :: rf_l5s3      !resp frac lig bio to SOM3
    ...
    associate(                                                                                        &
      ...
      rf_s3s4=params_inst%rf_s3s4_cn
        if (use_biochar) then
          rf_l4s1=params_inst%rf_l4s1_cn    !Sabrina
          rf_l5s3=params_inst%rf_l5s3_cn
        end if
      ...
      !-------------------  list of pools an their attributes  ------------

      ! Calc biochar CN ratios:
      bio_Rfrac = 0.96_r8
      bio_rec_cn = 203._r8
      bio_met_cn = (150*(1-bio_Rfrac))/(1-(150*(bio_Rfrac/bio_rec_cn)))
      ...
      if (use_biochar) then ! Sabrina
        i_metbio = i_met_bio
        floating_cn_ratio_decomp_pools(i_metbio) = .true.
        decomp_cascade_con%decomp_pool_name_restart(i_metbio) = 'metbio'
        decomp_cascade_con%decomp_pool_name_history(i_metbio) = 'METBIO'
        decomp_cascade_con%decomp_pool_name_long(i_metbio) = 'Metabolic Biochar'
        decomp_cascade_con%decomp_pool_name_short(i_metbio) = 'MB'
        is_litter(i_metbio) = .true.
        is_soil(i_metbio) = .false.
        is_cwd(i_metbio) = .false.
        initial_cn_ratio(i_metbio) = bio_met_cn
        initial_stock(i_metbio) = 0._r8
        is_metabolic(i_metbio) = .true.
        is_cellulose(i_metbio) = .false.
        is_lignin(i_metbio) = .false. 
                
       ! print *, "unbalance check 1", i_metbio,decomp_cascade_con%decomp_pool_name_history(i_metbio)

        i_ligbio = i_lig_bio
        floating_cn_ratio_decomp_pools(i_ligbio) = .true.
        decomp_cascade_con%decomp_pool_name_restart(i_ligbio) = 'ligbio'
        decomp_cascade_con%decomp_pool_name_history(i_ligbio) = 'LIGBIO'
        decomp_cascade_con%decomp_pool_name_long(i_ligbio) = 'Lignin Biochar'
        decomp_cascade_con%decomp_pool_name_short(i_ligbio) = 'LB'
        is_litter(i_ligbio) = .true.
        is_soil(i_ligbio) = .false.
        is_cwd(i_ligbio) = .false.
        initial_cn_ratio(i_ligbio) = bio_rec_cn
        initial_stock(i_ligbio) = 0._r8
        is_metabolic(i_ligbio) = .false.
        is_cellulose(i_ligbio) = .false.
        is_lignin(i_ligbio) = .true.

       ! print *, "unbalance check 2", i_ligbio,decomp_cascade_con%decomp_pool_name_history(i_ligbio)
      end if 
      ...
      if (.not. use_fates) then
       ...
       ! print *, "unbalance 3", i_cwd,decomp_cascade_con%decomp_pool_name_history(i_cwd)
      end if

      if ( use_fates ) then ! Sabrina
         i_soil1 = 4
      else if (use_biochar) then
         i_soil1 = 7
      else 
         i_soil1 = 5
      endif
      ...
      if (use_biochar) then ! Sabrina
        spinup_factor(i_metbio) = 1._r8
        spinup_factor(i_ligbio) = 1._r8
      end if
      ...
      if (use_biochar) then ! Sabrina
        i_l4s1 = 4
        decomp_cascade_con%cascade_step_name(i_l4s1) = 'L4S1'
        rf_decomp_cascade(bounds%begc:bounds%endc,1:nlevdecomp,i_l4s1) = rf_l4s1
        cascade_donor_pool(i_l4s1) = i_metbio
        cascade_receiver_pool(i_l4s1) = i_soil1
        pathfrac_decomp_cascade(bounds%begc:bounds%endc,1:nlevdecomp,i_l4s1) = 1.0_r8
        
        i_l5s3 = 5
        decomp_cascade_con%cascade_step_name(i_l5s3) = 'L5S3'
        rf_decomp_cascade(bounds%begc:bounds%endc,1:nlevdecomp,i_l5s3) = rf_l5s3
        cascade_donor_pool(i_l5s3) = i_ligbio
        cascade_receiver_pool(i_l5s3) = i_soil3
        pathfrac_decomp_cascade(bounds%begc:bounds%endc,1:nlevdecomp,i_l5s3) =1.0_r8
   ...
   subroutine decomp_rate_constants_cn(bounds, &
     ...
     associate(                                                           &
       ...
       k_l3=params_inst%k_l3_cn
        if (use_biochar) then ! Sabrina
          k_l4=params_inst%k_l4_cn
          k_l5=params_inst%k_l5_cn
        end if 
       ...
       if (use_biochar) then ! Sabrina
        k_l4 = 1.0_r8-exp(-k_l4*dtd)
        k_l5 = 1.0_r8-exp(-k_l5*dtd)
       end if
       ...
       if (use_vertsoilc) then
          do j = 1,nlevdecomp
             do fc = 1,num_soilc
               ...
               if (use_biochar) then ! Sabrina
                decomp_k(c,j,i_metbio) = k_l4 * t_scalar(c,j) * w_scalar(c,j) *depth_scalar(c,j) * o_scalar(c,j) / dt
                decomp_k(c,j,i_ligbio) = k_l5 * t_scalar(c,j) *w_scalar(c,j)*depth_scalar(c,j)*o_scalar(c,j)/dt 
               end if 
             end do
          end do
       else
          do j = 1,nlevdecomp
             do fc = 1,num_soilc
               ...
               if (use_biochar) then ! Sabrina
                decomp_k(c,j,i_metbio) = k_l4*t_scalar(c,j)* w_scalar(c,j)*o_scalar(c,j)/dt
                decomp_k(c,j,i_ligbio) = k_l5*t_scalar(c,j)*w_scalar(c,j)*o_scalar(c,j)/dt
               end if 
             end do
          end do
       end if
