module clm_varpar
  ...
  use clm_varctl   , only: use_biochar ! Sabrina Mod
  ...
  integer, public   :: i_met_bio  ! Sabrina
  integer, public   :: i_lig_bio 
...
!          nlevsoi     =  29 ! 10x10 + 9x100 + 10x300 = 4e3mm = 4m
    ...
    if ( use_fates ) then
       ...
       if (use_biochar) then ! Sabrina
         i_met_bio = i_cwd + 1
         i_lig_bio = i_met_bio +1
         ndecomp_pools = 10
         ndecomp_cascade_transitions = 11
       end if
    endif
