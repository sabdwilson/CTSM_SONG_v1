...
contains
  ...
  subroutine CropPhenology(num_pcropp, filter_pcropp                     , &
    ...
    use clm_varctl       , only : use_fert_fix ! Sabrina Mod 
    ...
    real(r8) ndays_on2(num_pcropp) ! number of days to fertilize for fixed fertilization
    real(r8) fert_fix(num_pcropp) ! fixed fert rate
    integer fertday(num_pcropp) ! fertilization day
    integer fertyr(num_pcropp) ! fertilization year
    !------------------------------------------------------------------------
    ...
    associate(                                                                   & 
      ...
      ! Sabrina - deleted original fertilizer loop
      ...
      do fp = 1, num_pcropp
         ...
         fertday(fp) = 0 ! Sabrina Mod start
         fertyr(fp) = 0
         fert_fix(fp) = 0._r8
         ndays_on2(fp) = 0._r8
         
        write(999997,*)use_fert_fix,p,fp,fertday(fp),fertyr(fp) 
         if (p == 2) then
                fertday(fp) = 135       
                fertyr(fp) = 2014
         end if
         ...
         ! Sabrina
         if (use_fertilizer .and. use_fert_fix) then
                   fert_fix(fp) = 224
                   ndays_on2(fp) = 1._r8
                   ndays_on = 0._r8
         else if (use_fertilizer .and. .not. use_fert_fix) then
                   ndays_on2(fp) = 0._r8
                   ndays_on = 20._r8
         else 
                   ndays_on2(fp) = 0._r8
       ...
       write(999998,*)use_fert_fix,use_fertilizer,p,fp,fertday(fp),fertyr(fp)                 
         ! ---------------------------------
         ...
         if (croplive(p)) then
            ...
            if (peaklai(p) >= 1) then
             ...
             write(2005,*) kyr,jday,p,leafout(p),huileaf(p),hui(p),huigrain(p),&
                           idpp,mxmat(ivt(p)),a5tmin(p)
            if (leafout(p) >= huileaf(p) .and. hui(p) < huigrain(p) .and. idpp < mxmat(ivt(p))) then
               ...
               if (abs(onset_counter(p)) > 1.e-6_r8) then
                  ...
                  onset_counter(p) = dt
                   ...
                   if (use_fertilizer .and. use_fert_fix) then
                        if (jday == fertday(fp) .and. kyr == fertyr(fp)) then
                                fert_counter(p) = ndays_on2(fp) * secspday
                                fert(p) = fert_fix(fp) / fert_counter(p)
                        end if
                   else if (use_fertilizer .and. .not. use_fert_fix) then
      ...
      write(999999,*)jday,kyr,dt,dtrad,fert(p),fert_counter(p)
