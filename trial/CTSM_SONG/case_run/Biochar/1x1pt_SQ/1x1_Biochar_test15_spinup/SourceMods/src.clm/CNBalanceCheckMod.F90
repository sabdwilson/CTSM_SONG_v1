module CNBalanceCheckMod
  ...
  use clm_varctl                      , only : iulog, use_nitrif_denitrif,use_biochar, Biochar_Tyear, init_bio_yr!Sabrina
  ...
  use clm_time_manager                , only : get_curr_date, get_curr_calday !Sabrina
  use SoilBiogeochemDecompCascadeConType , only : decomp_cascade_con ! Sabrina
...
contains
  ...
  subroutine CBalanceCheck(this, bounds, num_soilc, filter_soilc, &
    ...
    real(r8) :: dt             ! radiation time step (seconds)i
    real(r8) :: k       ! Sabrina (index of biochar_tyear)
    ...
    real(r8) :: biochar_c_input_grc(bounds%begg:bounds%endg) ! Sabrina
    real(r8) :: hrv_xsmrpool_amount_left_to_dribble(bounds%begg:bounds%endg)
    real(r8) :: dwt_conv_cflux_amount_left_to_dribble(bounds%begg:bounds%endg)
    integer  ::  jday ! Sabrina
    integer  ::  kyr
    integer  ::  kmo
    integer  ::  kda
    integer  ::  mcsec
    real(r8) :: biochar_c(bounds%begc:bounds%endc)
    real(r8) :: biochar_c_input(bounds%begc:bounds%endc)
    !-----------------------------------------------------------------------
    ...
    associate(                                                                            & 
      ...
      biochar_c_input_grc(bounds%begg:bounds%endg) = 0._r8

      err_found = .false.
      do fc = 1,num_soilc
         c = filter_soilc(fc)
       ! print *, "biochar issue:", num_soilc,fc,c
        if (use_biochar) then
         biochar_c_input(c) = 0._r8
         biochar_c(c) = 0._r8 ! Sabrina
        end if
        !print *, "biochar c input(C)", biochar_c_input(c),c
         ! calculate the total column-level carbon storage, for mass conservation check
         col_endcb(c) = totcolc(c)

         ! calculate total column-level inputs
         call get_curr_date(kyr,kmo,kda,mcsec) ! Sabrina
         jday = get_curr_calday()
        ! print *, "jday and kyr are (before c loop)", jday, kyr,mcsec
       
         col_cinputs = gpp(c)
        
         if (use_biochar) then
           if (init_bio_yr /= 0) then
             k = kyr - init_bio_yr +1
           else
             k = 1
           end if
         ! print *, "k value is", k
           if (k>0 .and. k<=Biochar_Tyear) then
             if ((mcsec >= (8*3600) .and. mcsec <=(16*3600)) .and. (jday == decomp_cascade_con%Biochar_day(k)) .and. (kyr ==decomp_cascade_con%Biochar_yr(k))) then
                biochar_c(c) = decomp_cascade_con%Biochar_Amt(k)/8
                col_cinputs = col_cinputs +  biochar_c(c)/dt
                biochar_c_input(c) = biochar_c(c)/dt
          !      print*, "biochar addition to cpinputs",col_cinputs,biochar_c(c), c,fc
           !     print *, "biochar_c_input is", biochar_c_input(c)
             end if
           ! print*, "col value after biochar timestep is", col_cinputs,mcsec
           ! print *, "biochar_c(c) is ", biochar_c(c),decomp_cascade_con%Biochar_Amt(k), jday, decomp_cascade_con%Biochar_day(k),kyr,decomp_cascade_con%Biochar_yr(k)    
           end if
         end if 
         ...
         if (abs(col_errcb(c)) > 1e-7_r8) then
          ...
          if (abs(col_errcb(c)) > 1e-8_r8) then
            write(iulog,*) 'cbalance warning at c =', c, col_errcb(c), col_endcb(c),col_cinputs,col_coutputs
         end if
      ...
      if (use_biochar) then
       call c2g( bounds = bounds, &
         carr = biochar_c_input(bounds%begc:bounds%endc), &
         garr = biochar_c_input_grc(bounds%begg:bounds%endg), &
         c2l_scale_type = 'unity', &
         l2g_scale_type = 'unity')
      end if
      err_found = .false.
      do g = bounds%begg, bounds%endg
         ...
         if (use_biochar) then
             grc_cinputs = grc_cinputs + biochar_c_input_grc(g)
         
         ! print *, "biochar c grid is", biochar_c_input_grc(g)
         end if 
  ...
  subroutine NBalanceCheck(this, bounds, num_soilc, filter_soilc, &
    ...
    integer :: kda
    integer :: mcsec
    real(r8):: biochar_n(bounds%begc:bounds%endc)
    real(r8):: biochar_cn(bounds%begc:bounds%endc) ! total biochar cn ratio of both pools
    ...
    associate(                                                                             & 
      ...
      do fc = 1,num_soilc
        ...
        if (use_biochar) then 
         ...
         call get_curr_date(kyr,kmo,kda,mcsec) ! Sabrina
         jday = get_curr_calday()
        ! print *, "jday and kyr are (before n loop) ", jday,kyr,mcsec
         ...
         if (use_crop) then
           ...
           if (use_biochar) then
             ...
             if (k>0 .and. k<=Biochar_Tyear) then
                if ((mcsec >= (8*3600) .and. mcsec <=(16*3600)) .and. (jday == decomp_cascade_con%Biochar_day(k)) .and. (kyr == decomp_cascade_con%Biochar_yr(k))) then
                   biochar_n(c) = (decomp_cascade_con%Biochar_Amt(k)/8) / biochar_cn(c)
                   col_ninputs(c) = col_ninputs(c) +  biochar_n(c)/dt
                   
                end if
         !     print*, "biochar_n(c) is", biochar_n(c),decomp_cascade_con%Biochar_Amt(k), biochar_cn(c), jday, kyr
      ...
      do g = bounds%begg, bounds%endg
        ...
        ! print *, "grid error balance factors", grc_ninputs(g), grc_noutputs(g),grc_endnb(g), grc_begnb(g)        
