module clm_instMod
  ...
  use clm_varctl      , only : use_century_decomp,use_biochar, Biochar_Tyear, use_crop, snow_cover_fraction_method, paramfile
...
contains
  ...
  subroutine clm_instInit(bounds)
    ...
    if (use_cn .or. use_fates ) then
       ...
       call init_decomp_cascade_constants(use_century_decomp,use_biochar,Biochar_Tyear )
       if (use_century_decomp) then
        ...
        if (use_biochar) then
         ! print*, "1biochar tyear is", Biochar_Tyear, "before open happens"
          open (unit = 50,file='/home/u8/sdwilson/CTSM_SONG/case_run/Biochar/1x1pt_ISU/Biochar_Tyear.txt',status = 'old') ! Sabrina
          !   print*, "2biochar year is", Biochar_Tyear,"iostat is", iostat
             do i=1, Biochar_Tyear
                read(50,*,IOSTAT=iostat) decomp_cascade_con%Biochar_Amt(i),decomp_cascade_con%Biochar_yr(i),decomp_cascade_con%Biochar_day(i)
           !     print*, "3biochar yr is", Biochar_Tyear, "iostat is", iostat
                if (iostat /= 0) then
                 EXIT
                end if
            !  print*,"check Biochar input", decomp_cascade_con%Biochar_Amt(i), decomp_cascade_con%Biochar_yr(i), decomp_cascade_con%Biochar_day(i)
             end do
          close (50)
        end if
       end if
