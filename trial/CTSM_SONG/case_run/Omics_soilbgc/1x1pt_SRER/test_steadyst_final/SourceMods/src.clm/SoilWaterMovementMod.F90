    ...
    associate(& 
      ...
      do j = 1, nlevsoi
         do fc = 1, num_hydrologyc
            ...
            ! Tianyi Hu added here for lower boundary condition !
            s1 = max(s1, 0.04_r8)
            ! ************************************************* !
      ...
      do fc = 1,num_hydrologyc
         ...
         if(jwt(c) < nlevsoi) then
            ...
            ! Tianyi Hu changed here for lower boundary condition !
            !s_node = max(h2osoi_vol(c,jwt(c)+1)/watsat(c,jwt(c)+1), 0.01_r8)
            s_node = max(h2osoi_vol(c,jwt(c)+1)/watsat(c,jwt(c)+1), 0.04_r8)
            ! ************************************************* !
...
!            s1 = max(h2osoi_vol(c,jwt+1)/watsat(c,jwt+1), 0.01_r8)
            ...
            s1 = max(vwc_liq(c,jwt+1)/watsat(c,jwt+1), 0.04_r8)
            ! ************************************************* !
            s1 = min(1._r8, s1)
