module SoilStateInitTimeConstMod
  ...
  !
  ! !USES
  use shr_kind_mod  , only : r8 => shr_kind_r8, SHR_KIND_CL !Tianyi added SHR_KIND_CL
  ...
  !Tianyi for dynamic B profile
  integer, parameter,          private :: name_len = SHR_KIND_CL
  character(len=name_len),     private :: B_profile_file
  character(len=name_len),     private :: Watsat_profile_file
  character(len=name_len),     private :: Sucsat_profile_file
...
contains
  ...
  subroutine ReadNL( nlfilename )
    ...
    !modified namelist by Tianyi for dynamic update
    !namelist / clm_soilstate_inparm / organic_frac_squared
    namelist / clm_soilstate_inparm / &
         organic_frac_squared, Watsat_profile_file, Sucsat_profile_file, B_profile_file
    !***********************!
  ...
  subroutine SoilStateInitTimeConst(bounds, soilstate_inst, nlfilename) 
    ...
    use clm_varctl          , only : use_cn, use_lch4, use_fates, dynroot_scheme ! Tianyi Hu
...
! remove roots below bedrock layer
    ...
    !==========Tianyi Hu added for Song dynroot===========
    if (.not. use_fates) then
    !if ( (.not. use_fates) .and. (dynroot_scheme == 1) ) then
    ...
    do c = begc, endc
       ...
       if (lun%itype(l)==istwet .or. lun%itype(l)==istice_mec .or. &
         ...
         !====Tianyi Hu use experiment Watsat value====
          open(unit=15, &
               file=trim(Watsat_profile_file), &
               status='old', action='read')
          read(15,*) soilstate_inst%watsat_col(c,:)
          close(15)
          soilstate_inst%watsat_col(c,:) = params_inst%watsat_adjustfactor * soilstate_inst%watsat_col(c,:)
          write(1999,*) 'Read new data '
          write(1999,*) soilstate_inst%watsat_col(c,:)
         !====Tianyi Hu use experiment Sucsat value====
          open(unit=15, &
               file=trim(Sucsat_profile_file), &
               status='old', action='read')
          read(15,*) soilstate_inst%sucsat_col(c,:)
          close(15)
          soilstate_inst%sucsat_col(c,:) = params_inst%sucsat_adjustfactor * soilstate_inst%sucsat_col(c,:)
          write(1999,*) 'Read new data '
          write(1999,*) soilstate_inst%sucsat_col(c,:)       
         !====Tianyi Hu use experiment B value====
          open(unit=15, &
               file=trim(B_profile_file), &
               status='old', action='read')
          read(15,*) soilstate_inst%bsw_col(c,:)
          close(15)
          soilstate_inst%bsw_col(c,:) = params_inst%bsw_adjustfactor * soilstate_inst%bsw_col(c,:)
          write(1999,*) 'Read new data '
          write(1999,*) soilstate_inst%bsw_col(c,:)
         !========================================
