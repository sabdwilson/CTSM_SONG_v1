...
case_revised_src=1 #1-copy the case code from identified folder, need to give the folder pathway
...
export OUTPUT_folder="/groups/chopinsong/CTSM_sabrina"
...
export CASE_NAME="Biochar_test21"
export Project_folder="Biochar"
export Run_folder="1x1pt_SQ"
export Case_folder="${CTSM_folder}/case_run/${Project_folder}/${Run_folder}"
#(1)if using revised codei from a specific case (e.g., specific code that implemented PSIImax-temperature
...
then
	export CASE_SRC="${CTSM_folder}/case_run/${Project_folder}/1x1pt_SQ/1x1_Biochar_test15_spinup/SourceMods/src.clm"
...
export Surface_data="${OUTPUT_folder}/PTCLM/mydatafile"
export Surface_data_spinup="${Surface_data}/SQ/1x1pt_SQ"
export Surface_data_final="${Surface_data}/SQ/1x1pt_SQ"
export Surface_data_hist="${Surface_data}/SQ/1x1pt_SQ"
export fsurf_data="surfdata_1x1pt_SQ_hist_78pfts_CMIP6_simyr2000_c231130.nc"
export fatmdomain="domain.lnd.1x1pt_SQ_navy.231130.nc"
export Forcing="/home/u8/sdwilson/data/SQ"

#XML Options
export RUN_STARTDATE_spinup="1980-01-01"
export STOP_N_spinup=280	
export RUN_TYPE_spinup=startup
export STOP_OPTION_spinup=nyears
export DATM_CLMNCEP_YR_START_spinup=1980
export DATM_CLMNCEP_YR_END_spinup=2012
export DATM_CLMNCEP_YR_ALIGN_spinup=1980
export JOB_WALLCLOCK_TIME_spinup=02:30:00
export walltime_spinup=02:30:00

export RUN_STARTDATE_final="1980-01-01"
export STOP_OPTION_final=nyears
export STOP_N_final=10
export RUN_TYPE_final=startup
export DATM_CLMNCEP_YR_START_final=1980
export DATM_CLMNCEP_YR_END_final=1990
export DATM_CLMNCEP_YR_ALIGN_final=1980
export JOB_WALLCLOCK_TIME_final=02:30:00
export walltime_final=02:30:00

export RUN_STARTDATE_hist="2010-01-01"
export STOP_OPTION_hist=nyears
export STOP_N_hist=3
export RUN_TYPE_hist=startup
export DATM_CLMNCEP_YR_START_hist=2010
export DATM_CLMNCEP_YR_END_hist=2012
export DATM_CLMNCEP_YR_ALIGN_hist=2010
export JOB_WALLCLOCK_TIME_hist=00:30:00
export walltime_hist=00:30:00

export BLDNML_OPTS="-bgc bgc -mask navy -crop"
...
for v in "${Run_type[@]}"
do
    if [ ! -d ${Case_folder}/1x1_${CASE_NAME}_${v} ];then
    echo "==================Creating ${v} case==========================="
    surfdata="Surface_data_${v}"
    echo "create_newcase --user-mods-dir ${!surfdata} --case ${Case_folder}/1x1_${CASE_NAME}_${v} \
    --compset ${compset} --res CLM_USRDAT --machine ${machine} -q ${queue}"
    ./create_newcase --user-mods-dir ${!surfdata} --case ${Case_folder}/1x1_${CASE_NAME}_${v} \
        --compset ${compset} --res CLM_USRDAT --machine ${machine} -q ${queue}
    ...
    if (( case_revised_src == 1 ))
    then
    	cp -v ${CASE_SRC}/* ${Case_folder}/1x1_${CASE_NAME}_${v}/SourceMods/src.clm
    fi 
    echo "========================================================================"
    fi
done
...
cd ${Case_folder}/1x1_${CASE_NAME}_spinup
echo $(pwd)
...
hist_fincl1 = 'GPP:A','TWS:A','TOTLITC:A','SMINN:A','TSOI:A'
hist_fincl2 = 'TOTSOMC:A'
...
cd ${Case_folder}/1x1_${CASE_NAME}_final
echo $(pwd)
cp -v /${OUTPUT_folder}/${Project_folder}/${Run_folder}/${CASE_NAME}/1x1_${CASE_NAME}_spinup/run/1x1_*clm2.r.* ./
Restart_f=$(basename -a \
$(ls /${OUTPUT_folder}/${Project_folder}/${Run_folder}/${CASE_NAME}/1x1_${CASE_NAME}_spinup/run/1x1_*clm2.r.*))
echo "${Restart_f}"
...
finidat='${Case_folder}/1x1_${CASE_NAME}_final/${Restart_f}'
EOF
#paramfile="${CASE_SRC}/${paramfile}"
...
cd ${Case_folder}/1x1_${CASE_NAME}_hist
echo $(pwd)
cp -v /${OUTPUT_folder}/${Project_folder}/${Run_folder}/${CASE_NAME}/1x1_${CASE_NAME}_final/run/1x1_*clm2.r.* ./
Restart_f=$(basename -a \
$(ls /${OUTPUT_folder}/${Project_folder}/${Run_folder}/${CASE_NAME}/1x1_${CASE_NAME}_final/run/1x1_*clm2.r.*))
echo "${Restart_f}"
...
/${OUTPUT_folder}/${Project_folder}/${Run_folder}/${CASE_NAME}/1x1_${CASE_NAME}_hist/run/
...
cat >> user_nl_clm <<EOF
hist_empty_htapes = .true.
hist_fincl1 = 'GPP:A','SR:A','ER:A','NEE:A','QSOIL:A','QVEGE:A','QVEGT:A','TLAI:A','SOIL3N:A',
              'TOTVEGC:A','LITTERC_HR:A','TOTSOMC:A','TWS:A','SOIL1C:A','SOIL1N:A','SOIL3C:A'
	      'TOTLITN:A','SOILC_HR:A','TOTSOMN:A','TOTECOSYSC:A','TOTECOSYSN:A',
	      'TOTLITC:A','GRAINC:A','F_N2O_DENIT:A','HR:A','SMINN:A','SOIL2C:A','SOIL2N:A'
hist_fincl2 = 'GPP:A','TLAI:A'
hist_fincl3 = 'TSOI:A','H2OSOI:A','SMP:A'
hist_fincl4 = 'VEGWP:A'
hist_fincl5 = 'EFLX_LH_TOT:A','QFLX_EVAP_TOT:A','RAIN_FROM_ATM:A','FGEV:A','FCEV:A','FCTR:A',
              'QDRIP:A','QTOPSOIL:A','QINFL:A','QOVER:A','QDRAI:A','QRUNOFF:A'
hist_fincl6 = 'LEAFC_LOSS:A','LEAFC_ALLOC:A','FROOTC_ALLOC:A','FROOTC_LOSS:A','LEAFC_XFER_TO_LEAFC:A',
              'FROOTC_XFER_TO_FROOTC:A','PSNSUN_TO_CPOOL:A','PSNSHADE_TO_CPOOL:A','CPOOL_TO_LEAFC:A',
              'CPOOL_TO_LEAFC_STORAGE:A','CPOOL_TO_FROOTC:A','CPOOL_TO_FROOTC_STORAGE:A','CPOOL_TO_LIVESTEMC:A',
              'CPOOL_TO_LIVESTEMC_STORAGE:A','CPOOL_TO_GRESP_STORAGE:A','CPOOL_LEAF_GR:A','CPOOL_LEAF_STORAGE_GR:A',
              'TRANSFER_LEAF_GR:A','CPOOL_FROOT_GR:A','CPOOL_FROOT_STORAGE_GR:A','TRANSFER_FROOT_GR:A',
              'LEAFC_STORAGE_TO_XFER:A','FROOTC_STORAGE_TO_XFER:A','GRESP_STORAGE_TO_XFER:A','CURRENT_GR:A',
              'TRANSFER_GR:A','STORAGE_GR:A','AVAILC:A','PLANT_CALLOC:A',
              'LEAFC:A','FROOTC:A','LEAFN:A','FROOTN:A'
hist_fincl7 = 'EFLX_LH_TOT:A','FSH_G:A','TSOI:A','H2OSOI:A','EFLX_SOIL_GRND:A'
hist_dov2xy = .true.,.false.,.true.,.false.,.true.,.false.,.true.
hist_type1d_pertape = ' ', 'PFTS', ' ', 'PFTS', ' ', 'PFTS', ' '
hist_nhtfrq = -24,-24,-1,-1,-24,-1,-1
hist_mfilt  = 100000,100000,2400000,2400000,100000,2400000,2400000
use_init_interp = .true.
use_fertilizer  = .false.
use_fert_fix 	= .false.
use_vertsoilc = .false.
...
do_transient_crops = .false.
do_transient_pfts  = .false.
flanduse_timeseries= '/groups/chopinsong/CTSM_sabrina/PTCLM/mydatafile/SQ/1x1pt_SQ/landuse.timeseries_1x1pt_SQ_hist_78pfts_CMIP6_simyr1850-2015_c231113.nc'
constrain_stress_deciduous_onset=.false.
soil_layerstruct_predefined = '10SL_3.5m'
finidat='${Case_folder}/1x1_${CASE_NAME}_hist/${Restart_f}'
...
streams = "datm.streams.txt.CLM1PT.CLM_USRDAT 2010 2010 2012",
