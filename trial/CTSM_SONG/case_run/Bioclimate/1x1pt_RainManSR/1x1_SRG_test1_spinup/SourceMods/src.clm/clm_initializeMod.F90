...
contains
  ...
  subroutine initialize2( )
    ...
    if(masterproc) write(40007,*)"hist_printflds_before"
    call shr_sys_flush(40007)
    call hist_printflds()
    if(masterproc) write(40007,*)"hist_printflds_after"
    call shr_sys_flush(40007)
    ! ------------------------------------------------------------------------
    ...
    do nc = 1,nclumps
       call get_clump_bounds(nc, bounds_clump)
       if(masterproc) write(40007,*)"dyn_hwcontent_set_baselines_before",nc,bounds_clump
       call shr_sys_flush(40007)
    ...
    if (use_cn) then
       if(masterproc) write(40007,*)"Init2_before"
       call shr_sys_flush(40007)
       call bgc_vegetation_inst%Init2(bounds_proc, NLFilename)
       if(masterproc) write(40007,*)"Init2_after"
       call shr_sys_flush(40007)
    ...
    if (nsrest == nsrContinue ) then
       if(masterproc) write(40007,*)"htapes_fieldlist_before"
       call shr_sys_flush(40007)
       call htapes_fieldlist()
       if(masterproc) write(40007,*)"htapes_fieldlist_after"
       call shr_sys_flush(40007)
    end if
    ...
    write(iulog,*)'finidat_interp_source',trim(finidat_interp_source)
    write(iulog,*)'nsrStartup',nsrStartup,nsrContinue,nsrBranch,nsrest
    if (nsrest == nsrStartup) then
    ...
    if (nsrest == nsrStartup .and. finidat_interp_source /= ' ') then
       ...
       if(masterproc) write(40007,*)"clm_initializaMod: restFile_write_before"
       call shr_sys_flush(40007)
       call restFile_write(bounds_proc, finidat_interp_dest, writing_finidat_interp_dest_file=.true.)
       if(masterproc) write(40007,*)"clm_initializaMod: restFile_write_after"
       call shr_sys_flush(40007)
