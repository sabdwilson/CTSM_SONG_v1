...
contains
  ...
  subroutine Restart(this, bounds, ncid, flag, &
    ...
    if (flag == 'read' ) then
     ...
     if (myrank == 0) write(40007,*) "cal_h2osoi_vol_col",nlevs,nlevurb,nlevgrnd
    ...
    !=======================add H2OSOI variable to the restart output======================         
    if (flag == 'write' .or. flag == 'define' ) then
    call MPI_COMM_RANK(MPI_COMM_WORLD,myrank,ierr)
    if (myrank == 0)write(40007,*) "before_restartvar",myrank, ierr, this%info%fname('H2OSOI')
    call shr_sys_flush(40007)
    call restartvar(ncid=ncid, flag=flag, &
         varname=this%info%fname('H2OSOI'), &
         xtype=ncd_double,  &
         dim1name='column', dim2name='levgrnd', switchdim=.true., &
         long_name=this%info%lname('volumetric soil water (natural vegetated and crop landunits only'), &
         units='mm3/mm3', &
         interpinic_flag='interp', readvar=readvar, data=this%h2osoi_vol_col)
    endif
    !======================================================================================
