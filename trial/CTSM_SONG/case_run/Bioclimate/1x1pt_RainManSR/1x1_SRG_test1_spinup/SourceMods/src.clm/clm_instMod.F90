...
contains
  ...
  subroutine clm_instRest(bounds, ncid, flag, writing_finidat_interp_dest_file)
    ...
    use shr_sys_mod, only : shr_sys_flush !add by Yang Song
    !
    ! !DESCRIPTION:
    ...
    if(masterproc) write(40007,*) "RESTART_WATER_VARIABLE: RESTART_before"
    ...
    if(masterproc) write(40007,*) "RESTART_WATER_VARIABLE: RESTART_after"
    call shr_sys_flush(40007)
