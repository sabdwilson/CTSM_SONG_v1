...
#include "dynamic_vector_typedef.inc"
  ...
  integer(SHR_KIND_IN),save :: debug = 1        ! edit/turn-on for debug write statements
...
999 continue
  ...
  subroutine shr_stream_findBounds(strm,mDateIn,        secIn,               &
    ...
    if(strm%nFiles>1)then
      write(300000,*) mDateIn,strm%nFiles, yrFirst,yrAlign,mYear,nYears,dYear,dDateIn, rDateIn,dDateF, dDateL
    endif
    if(strm%yearAlign ==1750)then
      write(300001,*) mDateIn,strm%nFiles,yrFirst,yrAlign,mYear,nYears,dYear,dDateIn, rDateIn,dDateF, dDateL
    endif
    ...
    if (strm%found_gvd) then
       ...
       write(s_logunit,*) 'tcx fbd3 ',rDateIn,rDatelvd,rDategvd,k,n
       call shr_sys_flush(s_logunit)
    if(strm%nFiles>1)then
      write(300002,*) mDateIn,rDateIn,rDatelvd,rDategvd,k,n,strm%found_lvd,strm%found_gvd
    endif
    if(strm%yearAlign ==1750)then
      write(300003,*) mDateIn,rDateIn,rDatelvd,rDategvd,k,n,strm%found_lvd,strm%found_gvd
    endif
    !-----------------------------------------------------------
    ! dateIn < rDatelvd
    !   limit -> abort
    ...
    if (rDateIn < rDatelvd) then
       ...
       if (.not.cycle) then
          ...
          fileUB = strm%file(k_ub)%name
                   write(s_logunit,*)'tcx fb1 ',n_lb,mDateLB,secLB,n_ub,mDateUB,secUB,yy,mm,dd
                   call shr_sys_flush(s_logunit)
          return
       endif
       ...
       if (cycle) then
          ...
          secUB   = strm%file(k_ub)%secs(n_ub)
          fileUB  = strm%file(k_ub)%name
                   write(s_logunit,*)'tcx fb2 ',n_lb,mDateLB,secLB,n_ub,mDateUB,secUB,yy,mm,dd
                   call shr_sys_flush(s_logunit)
          return
       endif
       ...
       if (.not.cycle) then
          ...
          secUB   = 0
          fileUB  = strm%file(k_ub)%name
                   write(s_logunit,*)'tcx fb3 ',n_lb,mDateLB,secLB,n_ub,mDateUB,secUB,yy,mm,dd
                   call shr_sys_flush(s_logunit)
          return
       endif
       ...
       if (cycle) then
          ...
          secUB   = strm%file(k_ub)%secs(n_ub)
          fileUB  = strm%file(k_ub)%name
                   write(s_logunit,*)'tcx fb4 ',n_lb,mDateLB,secLB,n_ub,mDateUB,secUB,yy,mm,dd
                   call shr_sys_flush(s_logunit)
          return
       endif
       ...
       C:    do k=strm%k_lvd,strm%nFiles
          ...
          if (strm%found_gvd .and. rDateIn >= rDategvd) then
             ...
             if (.not.cycle) then
                ...
                secUB   = 0
                fileUB  = strm%file(k_ub)%name
                               write(s_logunit,*)'tcx fb5 ',n_lb,mDateLB,secLB,n_ub,mDateUB,secUB,yy,mm,dd
                               call shr_sys_flush(s_logunit)
                return
             endif
             ...
             if (cycle) then
                ...
                secUB   = strm%file(k_ub)%secs(n_ub)
                fileUB  = strm%file(k_ub)%name
                               write(s_logunit,*)'tcx fb6 ',n_lb,mDateLB,secLB,n_ub,mDateUB,secUB,yy,mm,dd
                               call shr_sys_flush(s_logunit)
                return
             endif
          ...
          if ( rDate1 < rDateIn ) then
             ...
             do n=1,strm%file(k)%nt
                ...
                if ( rDate2 <= rDateIn ) then
                   ...
                   secUB = strm%file(k_ub)%secs(n_ub)
                   fileUB = strm%file(k_ub)%name
                                    write(s_logunit,*)'tcx fb7 ',k,n,n_lb,mDateLB,secLB,n_ub,mDateUB,secUB,yy,mm,dd
                                     call shr_sys_flush(s_logunit)
                   return
                endif
             enddo
    ...
    if (strm%offset /= 0) then
       ...
       do n = 1,num
          ...
          call shr_cal_advDateInt(offin,'seconds',din,sin,dout,sout,calendar)
                  write(s_logunit,*) 'tcx debug rtc1 ',k,n,nt,strm%offset,din,sin,dout,sout
