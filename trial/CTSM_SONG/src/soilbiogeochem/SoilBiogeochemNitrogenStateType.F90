...
contains
  ...
  subroutine InitHistory(this, bounds)
   ...
   ! print *, "init ntrunc spval", this%ntrunc_col(begc:endc)
       ...
       if (lun%itype(l) == istsoil .or. lun%itype(l) == istcrop) then
          ...
          do j = 1, nlevdecomp
             do k = 1, ndecomp_pools
               ...
               ! print *, "check npools_vr_col calc",decomp_cpools_vr_col(c,j,k), decomp_cascade_con%initial_cn_ratio(k),this%decomp_npools_vr_col(c,j,k)
             end do
          ...
          do k = 1, ndecomp_pools
           ...
           ! print *, "check npools_col", this%decomp_npools_col(c,k),decomp_cpools_col(c,k), decomp_cascade_con%initial_cn_ratio(k)
          end do
       ...
       do k = 1, ndecomp_pools
          ...
          do c = bounds%begc, bounds%endc
             ...
             do j = 1, nlevdecomp
              ...
              ! print *, "exit_spinup condition test decomp_npools_vr",this%decomp_npools_vr_col(c,j,k)
             end do
          end do
       end do
    end if
  ...
  subroutine SetValues ( this, num_column, filter_column, value_column )
    ...
    do j = 1,nlevdecomp_full
       ...
       ! print *, "value column", this%ntrunc_vr_col(i,j)
       end do
    end do
    ...
    do k = 1, ndecomp_pools
       ...
       ! print *, "column decomp npools", this%decomp_npools_col(i,k)
       end do
    end do
  ...
  subroutine Summary(this, bounds, num_allc, filter_allc)
   ...
   do l = 1, ndecomp_pools
      ...
      do j = 1, nlevdecomp
         do fc = 1,num_allc
          ...
          ! print*, "vert integ decomp npools", this%decomp_npools_col(c,l),this%decomp_npools_vr_col(c,j,l), dzsoi_decomp(j)
         end do
      end do
   end do
      ...
      if ( decomp_cascade_con%is_soil(l) ) then
         do fc = 1,num_allc
            ...
            this%totsomn_col(c) = this%totsomn_col(c) + &
                 this%decomp_npools_col(c,l)
          ! print*,"totsomn_col(c), and decomp pool",this%totsomn_col(c),this%decomp_npools_col(c,l)
         end do
      end if
   end do
   ...
   do j = 1, nlevdecomp
    ...
    ! print *, "diagnose ntrunc", this%ntrunc_col(c), this%ntrunc_vr_col(c,j),dzsoi_decomp(j)
   end do
