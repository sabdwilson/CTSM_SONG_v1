         ...
         do j = 1,nlevdecomp
            ...
            do k = 1, ndecomp_pools
               ...
               if (abs(cs%decomp_cpools_vr_col(c,j,k)) < ccrit) then
                 ...
                 ! print *, "cc check", cc, cs%decomp_cpools_vr_col(c,j,k)
                  if (use_cn) then
                     cn = cn + ns%decomp_npools_vr_col(c,j,k)
                     ns%decomp_npools_vr_col(c,j,k) = 0._r8
                  
                 ! print *, "check cn and npools", cn,ns%decomp_npools_vr_col(c,j,k)
                  end if
                  if ( use_c13 ) then
           ...
           ! print *, "cc and ctrunc check",cc, cs%ctrunc_vr_col(c,j)
            if (use_cn) then
          ...
          ! print *,"diagnose ntrunc part 2", ns%ntrunc_vr_col(c,j), cn
         end do
