...
! FUN Ends 
  ...
  subroutine Summary_carbonflux(this, &
    ...
    do fp = 1,num_soilp
       ...
       if ( use_crop .and. patch%itype(p) >= npcropmin )then
          this%ar_patch(p) =           &
               this%mr_patch(p)      + &
               this%gr_patch(p)
         ! print *, "xsmrpool first loop: ar, mr,gr", this%ar_patch(p),this%mr_patch(p),this%gr_patch(p)
          if ( .not. this%dribble_crophrv_xsmrpool_2atm ) this%ar_patch(p) = this%ar_patch(p) + &
                                             this%xsmrpool_to_atm_patch(p) ! xsmr... is -ve (slevis)
         ! print *, "xsmrpool second loop: ar, xsmrpool",this%ar_patch(p),this%xsmrpool_to_atm_patch(p)
       else         
             this%ar_patch(p) =           &
                  this%mr_patch(p)      + &
                  this%gr_patch(p)         
    ...
    if (use_crop .and. this%dribble_crophrv_xsmrpool_2atm) then
       ...
       ! print *, "after p2c", this%xsmrpool_to_atm_col(bounds%begc:bounds%endc)
    ...
    do fc = 1,num_soilc
       ...
       this%er_col(c) = &
            this%ar_col(c) + &
            soilbiogeochem_hr_col(c)
      ! print *,"CNVegCarbonFluxType er, ar, hr", this%er_col(c), this%ar_col(c), soilbiogeochem_hr_col(c)
