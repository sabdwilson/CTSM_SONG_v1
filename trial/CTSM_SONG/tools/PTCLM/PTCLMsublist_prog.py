...
#----------------------------------------------------------------------------------------
   ...
   inputdir_def = "/xdisk/chopinsong/chopinsong/CTSM_input"
   inputdir     = os.getenv("DIN_LOC_ROOT", inputdir_def )
   sitelistcsv  = "US-UMB" #"US-CHATS,US-FPe,CA-Let,US-NR1,CA-Man,BR-Sa1,BR-Sa3"
   mach         = "UA-puma"
   wall         = "01:00:00"
   parse_args   = False
   ptclm_opts   = ""
...
class test_PTCLMsublist_prog(unittest.TestCase):
   ...
   def test_init( self ):
     ...
     checkstring = "qsub  -o PTCLM_US-UMB."+jobid+".stdout.out  -N PTCLM_US-UMB  -l walltime=02:00:00  " + \
                   "-W group_list=chopinsong -l select=1:ncpus=28:mem=168gb -q standard " + \
