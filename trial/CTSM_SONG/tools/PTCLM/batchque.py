...
# Revised by Yang Song: add batch information for UA-ocelote and UA-puma HPC
#----------------------------------------------------------------------------------------
   # Class data
   setup     = False
   mach      = ""
   ...
   opts    = { 'yellowstone':"-n 1 -R 'span[ptile=15]' -q geyser -N -a poe ", \
               ...
               'UA-ocelote' :"-l select=1:ncpus=28:mem=168gb -q standard ",
	       'UA-puma'    :"--nodes=1 --ntasks=28 --mem-per-cpu=4gb --partition=standard" }
   # batch submission command
   bsub      = { 'yellowstone':"bsub",   'cheyenne':"qsub"         , 'edison':"qsub"	,'UA-ocelote':"qsub", 'UA-puma':"sbatch " }
   # Option to give file for standard output
   bs_stdout = { 'yellowstone':" -oo ",  'cheyenne':" -o "         , 'edison':" -o "	,'UA-ocelote':"-o",'UA-puma':" -o " }
   # Option to give job name to use
   bs_jobnam = { 'yellowstone':" -J ",   'cheyenne':" -N "         , 'edison':" -N "	,'UA-ocelote':"-N",'UA-puma':" --job-name="   }
   # Option to give current directory to use
   bs_curdir = { 'yellowstone':" -cwd ", 'cheyenne':""             , 'edison':" -d "	,'UA-ocelote':"",'UA-puma':""   }
   # Option to give account name to use
   bs_accnt  = { 'yellowstone':" -P ",   'cheyenne':" -A "         , 'edison':""	,'UA-ocelote':"-W group_list=",'UA-puma':" --account="    }
   # If jobcommand needs to be script file
   bs_script = { 'yellowstone':False,    'cheyenne':True           , 'edison':True	,'UA-ocelote':True    ,'UA-puma':False  }
   # Option to give wallclock time to use
   bs_wtime  = { 'yellowstone':" -W ",   'cheyenne':" -l walltime=", 'edison':" -l walltime=",	'UA-ocelote':" -l walltime=", 'UA-puma':" --time=" }

   def Initialize( self, prog, mach="", account="" ):
...
class test_batchque(unittest.TestCase):
   ...
   def test_submit( self ):
       ...
       startname = { 'ys':'yellowstone', 'cheyenne':'cheyenne', 'edison':'edison','UA-ocelote':'UA-ocelote','UA-puma':'UA-puma' }
       mach      = ""
