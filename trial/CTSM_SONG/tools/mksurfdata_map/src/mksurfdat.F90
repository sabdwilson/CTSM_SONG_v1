...
!EOP
    ...
    character(len=256) :: line              ! full land use string add by YangSong
    character(len=256)   :: year_str          ! string of year add by YangSong
    character(len=4)   :: year_str2
    character(len=256) :: string            ! string read in
    INTEGER  :: idx,i                         ! the position of the last ">" add by Yang Song
    integer  :: t1                          ! timer
...
!-----------------------------------------------------------------------
    ...
    print*, "finish mkpftInit"
    allocate ( elevclass(nglcec+1) )
    call mkglcmecInit (elevclass)
    print*,"finish mkglcmecInit"
    print*,"mksrf_furban=",mksrf_furban
    call mkurbanInit (mksrf_furban) !add test by Yang Song
    print*,"finish mkurbanInit"
    if ( all_veg )then
    ...
    print*, "mkharvest_data=",mksrf_fhrvtyp
    ...
    print*, "mksrf_furban",map_furban,mksrf_furban
    ...
    if ( .not. all_urban .and. .not. all_veg )then
       ...
       ! furbtopo.
      print*, "elevation",map_furbtopo,mksrf_furbtopo
    ...
    if (mksrf_fdynuse /= ' ') then
       ...
       write(6,*) "test1_landuse", mksrf_fdynuse, nfdyn      
       pctnatpft_max = pctnatpft
       pctcft_max = pctcft
       write(6,*) "test2_landuse"
       ntim = 0
       do 
          ! Read input pft data
           !read(nfdyn, '(A195,1x,I4)', iostat=ier) string, year
           !===================================================================
           !Origininal code is not flexible in format to
           !read text file, and ouput 0 year. The code is improved by Yang Song
           !to enable reading strings with different length and split year and
           !crop management informaiton 
           !====================================================================
           read(nfdyn, '(A)',iostat=ier) line
           write(6,*) "test3_line=",line 
           if (ier /= 0) exit
           ...
           string=line(1:idx)
           year_str=trim(line(idx+1:))
           idx=0
           do i=1, LEN_TRIM(year_str), 1
              if(year_str(i:i) >= "0".and.year_str(i:i)<='9' )then
                  idx = i
                  exit
              endif
           enddo
           year_str2=year_str(idx:(idx+3))

           WRITE(6,*)'YEAR_str2=',year_str2,'string=',string
           read(year_str2,*) year
          ...
          if ( all_veg )then
             fname = ' '
             fhrvname  = ' '
             write(6,*) "test3_landuse:in the loop of all_veg:mkfpt_parse_oride"
             call mkpft_parse_oride(string)
             write(6,*) "test3_landuse:in the loop of all_veg:mkharvest_parse_oride"
	     ...
	     write(6,*)'input pft dynamic dataset for year ', year, ' is : ', trim(fname)
             ...
             !=========================================================
             !The following code is improved by Yang Song for flexiblly reading
             !string and split year number and crop management string
             !===============================================================
             read(nfdyn, '(A)',iostat=ier) line
             write(6,*) "test3_line(not_all_veg)=",line
             if (ier /= 0) exit
