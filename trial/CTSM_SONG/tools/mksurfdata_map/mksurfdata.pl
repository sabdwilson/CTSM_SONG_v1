...
my $CSMDATA = "/xdisk/chopinsong/chopinsong/CTSM_inputdata";  #revised by Yang Song #"/glade/p/cesm/cseg/inputdata";
...
#-----------------------------------------------------------------------------------------------
   ...
   print $cfh <<"EOF";
##! /bin/bash
#export CSMDATA = $CSMDATA
EOF
#========changed from tsh to bash by Yang Song for UA-ocelote========
##! /bin/csh -f
##set CSMDATA = $CSMDATA
#====================================================================
   ...
   foreach my $res ( @hresols ) {
      ...
      foreach my $typ ( @typlist ) {
         ...
         my $hgrid = `$hgrid_cmd`;
         print "hgrid_test:$typ,$lmask,$hgrid,$mopts,$typ \n";
         if ($debug) {
	 ...
	 if ( $opts{'hgrid'} eq "usrspec" ) {
         ...
         print "map_typ2:$map{$typ} \n"; 
         if ( $map{$typ} !~ /[^ ]+/ ) {
            print "could not find a mapping file loop /n";
      ...
      RCP: foreach my $ssp_rcp ( @rcpaths ) {
         ...
         SIM_YEAR: foreach my $sim_year ( @years ) {
            ...
            print "cwd:$cwd\n";
            print "$exedir/mksurfdata_map < $cwd/$namelist_fname\n";
            if ( ! $opts{'debug'} ) {
               #Tianyi's revision
               #system( "$exedir/mksurfdata_map < $cwd/$namelist_fname" );
