...
!! adding phosphorus by Xiaojuan Yang

  private :: init_pft_phostate_type
  private :: init_pft_phoflux_type
  private :: init_column_phostate_type
  private :: init_column_phoflux_type
...
!------------------------------------------------------------------------
    ...
    ! phosphorus balance structures (pft and column levels)

    call init_phosphorus_balance_type(begp, endp, clm3%g%l%c%p%pphobal)
    call init_phosphorus_balance_type(begc, endc, clm3%g%l%c%cphobal)
   ...
   ! pft phosphorus state variables at the pft level and averaged to the column

    call init_pft_phostate_type(begp, endp, clm3%g%l%c%p%pphos)
    call init_pft_phostate_type(begc, endc, clm3%g%l%c%cphos%pphos_a)
    ...
    ! pft phosphorus flux variables at pft level and averaged to column

    call init_pft_phoflux_type(begp, endp, clm3%g%l%c%p%pphof)
    call init_pft_phoflux_type(begc, endc, clm3%g%l%c%cphof%pphof_a)
    ...
    ! column phosphorus state variables at column level and averaged to
    ! the landunit and gridcell and model

    call init_column_phostate_type(begc, endc, clm3%g%l%c%cphos)
   ...
   ! column phosphorus flux variables at column level

    call init_column_phoflux_type(begc, endc, clm3%g%l%c%cphof)
...
!BOP
!  
! !IROUTINE: init_phosphorus_balance_type
!
! !INTERFACE:
  subroutine init_phosphorus_balance_type(beg, end, phobal)
!
! !DESCRIPTION:
! Initialize phosphorus balance variables
!
! !ARGUMENTS:
    implicit none
    integer, intent(in) :: beg, end
    type(phosphorus_balance_type), intent(inout):: phobal
!
! !REVISION HISTORY:
! Created by Peter Thornton, 12/11/2003
!
...
!------------------------------------------------------------------------
    ...
    allocate(phobal%begpb(beg:end))
    allocate(phobal%endpb(beg:end))
    allocate(phobal%errpb(beg:end))

    phobal%begpb(beg:end) = nan
    phobal%endpb(beg:end) = nan
    phobal%errpb(beg:end) = nan
    
  end subroutine init_phosphorus_balance_type
...
!------------------------------------------------------------------------
    ...
    !! add phosphorus
    allocate(pftcon%leafcp(0:numpft))
    allocate(pftcon%lflitcp(0:numpft))
    allocate(pftcon%frootcp(0:numpft))
    allocate(pftcon%livewdcp(0:numpft))
    allocate(pftcon%deadwdcp(0:numpft))
    allocate(pftcon%graincp(0:numpft))
    ...
    !! the values of these constants are given here temperarily
    !! will be read in pftvarcon.F90 in later version  
!    pftcon%leafcp(:) = 1000._r8
!    pftcon%lflitcp(:) = 2000._r8
!    pftcon%frootcp(:) = 1500._r8
!    pftcon%livewdcp(:) = 3000._r8
!    pftcon%deadwdcp(:) = 3000._r8
!    pftcon%graincp(:) = 500._r8
...
#endif
    !! add phosphorus: Xiaojuan Yang 11/06/2011
    allocate(pepv%p_allometry(beg:end))
    allocate(pepv%plant_pdemand(beg:end))
    allocate(pepv%tempmax_retransp(beg:end))
    allocate(pepv%annmax_retransp(beg:end))
    allocate(pepv%avail_retransp(beg:end))
    allocate(pepv%plant_palloc(beg:end))
...
#endif
    !! add variables for phosphorus: 11/06/2011, Xiaojuan Yang  
    pepv%p_allometry(beg:end) = nan
    pepv%plant_pdemand(beg:end) = nan
    pepv%tempmax_retransp(beg:end) = nan
    pepv%annmax_retransp(beg:end) = nan
    pepv%avail_retransp(beg:end) = nan
    pepv%plant_palloc(beg:end) = nan
...
!BOP
!
! !IROUTINE: init_pft_phostate_type
!
! !INTERFACE:
  subroutine init_pft_phostate_type(beg, end, pphos)
!
! !DESCRIPTION:
! Initialize pft phosphorus state
!
! !USES:
    use surfrdMod, only : crop_prog
! !ARGUMENTS:
    implicit none
    integer, intent(in) :: beg, end
    type (pft_phostate_type), intent(inout):: pphos !pft phosphorus state
!
! !REVISION HISTORY:
! Created by Peter Thornton
!
...
!------------------------------------------------------------------------
    if ( crop_prog )then
       allocate(pphos%grainp(beg:end))
       allocate(pphos%grainp_storage(beg:end))
       allocate(pphos%grainp_xfer(beg:end))
    end if
    allocate(pphos%leafp(beg:end))
    allocate(pphos%leafp_storage(beg:end))
    allocate(pphos%leafp_xfer(beg:end))
    allocate(pphos%frootp(beg:end))
    allocate(pphos%frootp_storage(beg:end))
    allocate(pphos%frootp_xfer(beg:end))
    allocate(pphos%livestemp(beg:end))
    allocate(pphos%livestemp_storage(beg:end))
    allocate(pphos%livestemp_xfer(beg:end))
    allocate(pphos%deadstemp(beg:end))
    allocate(pphos%deadstemp_storage(beg:end))
    allocate(pphos%deadstemp_xfer(beg:end))
    allocate(pphos%livecrootp(beg:end))
    allocate(pphos%livecrootp_storage(beg:end))
    allocate(pphos%livecrootp_xfer(beg:end))
    allocate(pphos%deadcrootp(beg:end))
    allocate(pphos%deadcrootp_storage(beg:end))
    allocate(pphos%deadcrootp_xfer(beg:end))
    allocate(pphos%retransp(beg:end))
    allocate(pphos%ppool(beg:end))
    allocate(pphos%pft_ptrunc(beg:end))
    allocate(pphos%dispvegp(beg:end))
    allocate(pphos%storvegp(beg:end))
    allocate(pphos%totvegp(beg:end))
    allocate(pphos%totpftp(beg:end))

    if ( crop_prog )then
       pphos%grainp(beg:end)         = nan
       pphos%grainp_storage(beg:end) = nan
       pphos%grainp_xfer(beg:end)    = nan
    end if
    pphos%leafp(beg:end) = nan
    pphos%leafp_storage(beg:end) = nan
    pphos%leafp_xfer(beg:end) = nan
    pphos%frootp(beg:end) = nan
    pphos%frootp_storage(beg:end) = nan
    pphos%frootp_xfer(beg:end) = nan
    pphos%livestemp(beg:end) = nan
    pphos%livestemp_storage(beg:end) = nan
    pphos%livestemp_xfer(beg:end) = nan
    pphos%deadstemp(beg:end) = nan
    pphos%deadstemp_storage(beg:end) = nan
    pphos%deadstemp_xfer(beg:end) = nan
    pphos%livecrootp(beg:end) = nan
    pphos%livecrootp_storage(beg:end) = nan
    pphos%livecrootp_xfer(beg:end) = nan
    pphos%deadcrootp(beg:end) = nan
    pphos%deadcrootp_storage(beg:end) = nan
    pphos%deadcrootp_xfer(beg:end) = nan
    pphos%retransp(beg:end) = nan
    pphos%ppool(beg:end) = nan
    pphos%pft_ptrunc(beg:end) = nan
    pphos%dispvegp(beg:end) = nan
    pphos%storvegp(beg:end) = nan
    pphos%totvegp(beg:end) = nan
    pphos%totpftp(beg:end) = nan

  end subroutine init_pft_phostate_type
...
!BOP
!
! !IROUTINE: init_pft_phoflux_type
!
! !INTERFACE:
  subroutine init_pft_phoflux_type(beg, end, pphof)
!   
! !DESCRIPTION:
! Initialize pft phosphorus flux variables 
...
!------------------------------------------------------------------------
    ...
    allocate(pphof%m_leafp_to_litter(beg:end)) 
    allocate(pphof%m_frootp_to_litter(beg:end))
    allocate(pphof%m_leafp_storage_to_litter(beg:end))
    allocate(pphof%m_frootp_storage_to_litter(beg:end))
    allocate(pphof%m_livestemp_storage_to_litter(beg:end))
    allocate(pphof%m_deadstemp_storage_to_litter(beg:end))
    allocate(pphof%m_livecrootp_storage_to_litter(beg:end))
    allocate(pphof%m_deadcrootp_storage_to_litter(beg:end))
    allocate(pphof%m_leafp_xfer_to_litter(beg:end))
    allocate(pphof%m_frootp_xfer_to_litter(beg:end))
    allocate(pphof%m_livestemp_xfer_to_litter(beg:end))
    allocate(pphof%m_deadstemp_xfer_to_litter(beg:end))
    allocate(pphof%m_livecrootp_xfer_to_litter(beg:end))
    allocate(pphof%m_deadcrootp_xfer_to_litter(beg:end))
    allocate(pphof%m_livestemp_to_litter(beg:end))
    allocate(pphof%m_deadstemp_to_litter(beg:end))
    allocate(pphof%m_livecrootp_to_litter(beg:end))
    allocate(pphof%m_deadcrootp_to_litter(beg:end))
    allocate(pphof%m_retransp_to_litter(beg:end))
    allocate(pphof%hrv_leafp_to_litter(beg:end))
    allocate(pphof%hrv_frootp_to_litter(beg:end))
    allocate(pphof%hrv_leafp_storage_to_litter(beg:end))
    allocate(pphof%hrv_frootp_storage_to_litter(beg:end))
    allocate(pphof%hrv_livestemp_storage_to_litter(beg:end))
    allocate(pphof%hrv_deadstemp_storage_to_litter(beg:end))
    allocate(pphof%hrv_livecrootp_storage_to_litter(beg:end))
    allocate(pphof%hrv_deadcrootp_storage_to_litter(beg:end))
    allocate(pphof%hrv_leafp_xfer_to_litter(beg:end))
    allocate(pphof%hrv_frootp_xfer_to_litter(beg:end))
    allocate(pphof%hrv_livestemp_xfer_to_litter(beg:end))
    allocate(pphof%hrv_deadstemp_xfer_to_litter(beg:end))
    allocate(pphof%hrv_livecrootp_xfer_to_litter(beg:end))
    allocate(pphof%hrv_deadcrootp_xfer_to_litter(beg:end))
    allocate(pphof%hrv_livestemp_to_litter(beg:end))
    allocate(pphof%hrv_deadstemp_to_prod10p(beg:end))
    allocate(pphof%hrv_deadstemp_to_prod100p(beg:end))
    allocate(pphof%hrv_livecrootp_to_litter(beg:end))
    allocate(pphof%hrv_deadcrootp_to_litter(beg:end))
    allocate(pphof%hrv_retransp_to_litter(beg:end))
    allocate(pphof%m_leafp_to_fire(beg:end))
    allocate(pphof%m_frootp_to_fire(beg:end))
    allocate(pphof%m_leafp_storage_to_fire(beg:end))
    allocate(pphof%m_frootp_storage_to_fire(beg:end))
    allocate(pphof%m_livestemp_storage_to_fire(beg:end))
    allocate(pphof%m_deadstemp_storage_to_fire(beg:end))
    allocate(pphof%m_livecrootp_storage_to_fire(beg:end))
    allocate(pphof%m_deadcrootp_storage_to_fire(beg:end))
    allocate(pphof%m_leafp_xfer_to_fire(beg:end))
    allocate(pphof%m_frootp_xfer_to_fire(beg:end))
    allocate(pphof%m_livestemp_xfer_to_fire(beg:end))
    allocate(pphof%m_deadstemp_xfer_to_fire(beg:end))
    allocate(pphof%m_livecrootp_xfer_to_fire(beg:end))
    allocate(pphof%m_deadcrootp_xfer_to_fire(beg:end))
    allocate(pphof%m_livestemp_to_fire(beg:end))
    allocate(pphof%m_deadstemp_to_fire(beg:end))
    allocate(pphof%m_deadstemp_to_litter_fire(beg:end))
    allocate(pphof%m_livecrootp_to_fire(beg:end))
    allocate(pphof%m_deadcrootp_to_fire(beg:end))
    allocate(pphof%m_deadcrootp_to_litter_fire(beg:end))
    allocate(pphof%m_retransp_to_fire(beg:end))
    allocate(pphof%leafp_xfer_to_leafp(beg:end))
    allocate(pphof%frootp_xfer_to_frootp(beg:end))
    allocate(pphof%livestemp_xfer_to_livestemp(beg:end))
    allocate(pphof%deadstemp_xfer_to_deadstemp(beg:end))
    allocate(pphof%livecrootp_xfer_to_livecrootp(beg:end))
    allocate(pphof%deadcrootp_xfer_to_deadcrootp(beg:end))
    allocate(pphof%leafp_to_litter(beg:end))
    allocate(pphof%leafp_to_retransp(beg:end))
    allocate(pphof%frootp_to_litter(beg:end))
    allocate(pphof%retransp_to_ppool(beg:end))
    allocate(pphof%sminp_to_ppool(beg:end))
    allocate(pphof%ppool_to_leafp(beg:end))
    allocate(pphof%ppool_to_leafp_storage(beg:end))
    allocate(pphof%ppool_to_frootp(beg:end))
    allocate(pphof%ppool_to_frootp_storage(beg:end))
    allocate(pphof%ppool_to_livestemp(beg:end))
    allocate(pphof%ppool_to_livestemp_storage(beg:end))
    allocate(pphof%ppool_to_deadstemp(beg:end))
    allocate(pphof%ppool_to_deadstemp_storage(beg:end))
    allocate(pphof%ppool_to_livecrootp(beg:end))
    allocate(pphof%ppool_to_livecrootp_storage(beg:end))
    allocate(pphof%ppool_to_deadcrootp(beg:end))
    allocate(pphof%ppool_to_deadcrootp_storage(beg:end))
    allocate(pphof%leafp_storage_to_xfer(beg:end))
    allocate(pphof%frootp_storage_to_xfer(beg:end))
    allocate(pphof%livestemp_storage_to_xfer(beg:end))
    allocate(pphof%deadstemp_storage_to_xfer(beg:end))
    allocate(pphof%livecrootp_storage_to_xfer(beg:end))
    allocate(pphof%deadcrootp_storage_to_xfer(beg:end))
    allocate(pphof%livestemp_to_deadstemp(beg:end))
    allocate(pphof%livestemp_to_retransp(beg:end))
    allocate(pphof%livecrootp_to_deadcrootp(beg:end))
    allocate(pphof%livecrootp_to_retransp(beg:end))
    allocate(pphof%pdeploy(beg:end))
    allocate(pphof%pft_pinputs(beg:end))
    allocate(pphof%pft_poutputs(beg:end))
    allocate(pphof%wood_harvestp(beg:end))
    allocate(pphof%pft_fire_ploss(beg:end))
    if ( crop_prog )then
       allocate(pphof%grainp_xfer_to_grainp(beg:end))
       allocate(pphof%livestemp_to_litter(beg:end))
       allocate(pphof%grainp_to_food(beg:end))
       allocate(pphof%ppool_to_grainp(beg:end))
       allocate(pphof%ppool_to_grainp_storage(beg:end))
       allocate(pphof%grainp_storage_to_xfer(beg:end))
    end if

    pphof%m_leafp_to_litter(beg:end) = nan
    pphof%m_frootp_to_litter(beg:end) = nan
    pphof%m_leafp_storage_to_litter(beg:end) = nan
    pphof%m_frootp_storage_to_litter(beg:end) = nan
    pphof%m_livestemp_storage_to_litter(beg:end) = nan
    pphof%m_deadstemp_storage_to_litter(beg:end) = nan
    pphof%m_livecrootp_storage_to_litter(beg:end) = nan
    pphof%m_deadcrootp_storage_to_litter(beg:end) = nan
    pphof%m_leafp_xfer_to_litter(beg:end) = nan
    pphof%m_frootp_xfer_to_litter(beg:end) = nan
    pphof%m_livestemp_xfer_to_litter(beg:end) = nan
    pphof%m_deadstemp_xfer_to_litter(beg:end) = nan
    pphof%m_livecrootp_xfer_to_litter(beg:end) = nan
    pphof%m_deadcrootp_xfer_to_litter(beg:end) = nan
    pphof%m_livestemp_to_litter(beg:end) = nan
    pphof%m_deadstemp_to_litter(beg:end) = nan
    pphof%m_livecrootp_to_litter(beg:end) = nan
    pphof%m_deadcrootp_to_litter(beg:end) = nan
    pphof%m_retransp_to_litter(beg:end) = nan
    pphof%hrv_leafp_to_litter(beg:end) = nan
    pphof%hrv_frootp_to_litter(beg:end) = nan
    pphof%hrv_leafp_storage_to_litter(beg:end) = nan
    pphof%hrv_frootp_storage_to_litter(beg:end) = nan
    pphof%hrv_livestemp_storage_to_litter(beg:end) = nan
    pphof%hrv_deadstemp_storage_to_litter(beg:end) = nan
    pphof%hrv_livecrootp_storage_to_litter(beg:end) = nan
    pphof%hrv_deadcrootp_storage_to_litter(beg:end) = nan
    pphof%hrv_leafp_xfer_to_litter(beg:end) = nan
    pphof%hrv_frootp_xfer_to_litter(beg:end) = nan
    pphof%hrv_livestemp_xfer_to_litter(beg:end) = nan
    pphof%hrv_deadstemp_xfer_to_litter(beg:end) = nan
    pphof%hrv_livecrootp_xfer_to_litter(beg:end) = nan
    pphof%hrv_deadcrootp_xfer_to_litter(beg:end) = nan
    pphof%hrv_livestemp_to_litter(beg:end) = nan
    pphof%hrv_deadstemp_to_prod10p(beg:end) = nan
    pphof%hrv_deadstemp_to_prod100p(beg:end) = nan
    pphof%hrv_livecrootp_to_litter(beg:end) = nan
    pphof%hrv_deadcrootp_to_litter(beg:end) = nan
    pphof%hrv_retransp_to_litter(beg:end) = nan
    pphof%m_leafp_to_fire(beg:end) = nan
    pphof%m_frootp_to_fire(beg:end) = nan
    pphof%m_leafp_storage_to_fire(beg:end) = nan
    pphof%m_frootp_storage_to_fire(beg:end) = nan
    pphof%m_livestemp_storage_to_fire(beg:end) = nan
    pphof%m_deadstemp_storage_to_fire(beg:end) = nan
    pphof%m_livecrootp_storage_to_fire(beg:end) = nan
    pphof%m_deadcrootp_storage_to_fire(beg:end) = nan
    pphof%m_leafp_xfer_to_fire(beg:end) = nan
    pphof%m_frootp_xfer_to_fire(beg:end) = nan
    pphof%m_livestemp_xfer_to_fire(beg:end) = nan
    pphof%m_deadstemp_xfer_to_fire(beg:end) = nan
    pphof%m_livecrootp_xfer_to_fire(beg:end) = nan
    pphof%m_deadcrootp_xfer_to_fire(beg:end) = nan
    pphof%m_livestemp_to_fire(beg:end) = nan
    pphof%m_deadstemp_to_fire(beg:end) = nan
    pphof%m_deadstemp_to_litter_fire(beg:end) = nan
    pphof%m_livecrootp_to_fire(beg:end) = nan
    pphof%m_deadcrootp_to_fire(beg:end) = nan
    pphof%m_deadcrootp_to_litter_fire(beg:end) = nan
    pphof%m_retransp_to_fire(beg:end) = nan
    pphof%leafp_xfer_to_leafp(beg:end) = nan
    pphof%frootp_xfer_to_frootp(beg:end) = nan
    pphof%livestemp_xfer_to_livestemp(beg:end) = nan
    pphof%deadstemp_xfer_to_deadstemp(beg:end) = nan
    pphof%livecrootp_xfer_to_livecrootp(beg:end) = nan
    pphof%deadcrootp_xfer_to_deadcrootp(beg:end) = nan
    pphof%leafp_to_litter(beg:end) = nan
    pphof%leafp_to_retransp(beg:end) = nan
    pphof%frootp_to_litter(beg:end) = nan
    pphof%retransp_to_ppool(beg:end) = nan
    pphof%sminp_to_ppool(beg:end) = nan
    pphof%ppool_to_leafp(beg:end) = nan
    pphof%ppool_to_leafp_storage(beg:end) = nan
    pphof%ppool_to_frootp(beg:end) = nan
    pphof%ppool_to_frootp_storage(beg:end) = nan
    pphof%ppool_to_livestemp(beg:end) = nan
    pphof%ppool_to_livestemp_storage(beg:end) = nan
    pphof%ppool_to_deadstemp(beg:end) = nan
    pphof%ppool_to_deadstemp_storage(beg:end) = nan
    pphof%ppool_to_livecrootp(beg:end) = nan
    pphof%ppool_to_livecrootp_storage(beg:end) = nan
    pphof%ppool_to_deadcrootp(beg:end) = nan
    pphof%ppool_to_deadcrootp_storage(beg:end) = nan
    pphof%leafp_storage_to_xfer(beg:end) = nan
    pphof%frootp_storage_to_xfer(beg:end) = nan
    pphof%livestemp_storage_to_xfer(beg:end) = nan
    pphof%deadstemp_storage_to_xfer(beg:end) = nan
    pphof%livecrootp_storage_to_xfer(beg:end) = nan
    pphof%deadcrootp_storage_to_xfer(beg:end) = nan
    pphof%livestemp_to_deadstemp(beg:end) = nan
    pphof%livestemp_to_retransp(beg:end) = nan
    pphof%livecrootp_to_deadcrootp(beg:end) = nan
    pphof%livecrootp_to_retransp(beg:end) = nan
    pphof%pdeploy(beg:end) = nan
    pphof%pft_pinputs(beg:end) = nan
    pphof%pft_poutputs(beg:end) = nan
    pphof%wood_harvestp(beg:end) = nan
    pphof%pft_fire_ploss(beg:end) = nan
    if ( crop_prog )then
       pphof%grainp_xfer_to_grainp(beg:end) = nan
       pphof%livestemp_to_litter(beg:end) = nan
       pphof%grainp_to_food(beg:end) = nan
       pphof%ppool_to_grainp(beg:end) = nan
       pphof%ppool_to_grainp_storage(beg:end) = nan
       pphof%grainp_storage_to_xfer(beg:end) = nan
    end if

  end subroutine init_pft_phoflux_type
...
!------------------------------------------------------------------------
    ...
    allocate(cps%isoiorder(beg:end))  !* soil order, cannot be averaged up
...
!   allocate(cps%xirrig(beg:end))
    ...
    allocate(cps%fpi_p(beg:end))   !! add phosphorus
    allocate(cps%fpg(beg:end))
    allocate(cps%fpg_p(beg:end))   !! add phosphorus
...
!   cps%xirrig(beg:end) = 0._r8
    ...
    cps%fpi(beg:end) = nan
    cps%fpi_p(beg:end) = nan   !! add phosphorus
    cps%fpg(beg:end) = nan
    cps%fpg_p(beg:end) = nan
...
!BOP
!
! !IROUTINE: init_column_phostate_type
!
! !INTERFACE:
  subroutine init_column_phostate_type(beg, end, cphos)
!
! !DESCRIPTION:
! Initialize column phosphorus state variables
!
! !ARGUMENTS:
    implicit none
    integer, intent(in) :: beg, end
...
!
! !REVISION HISTORY:
! Added by Xiaojuan Yang
!
!EOP
!------------------------------------------------------------------------

    allocate(cphos%cwdp(beg:end))
    allocate(cphos%litr1p(beg:end))
    allocate(cphos%litr2p(beg:end))
    allocate(cphos%litr3p(beg:end))
    allocate(cphos%soil1p(beg:end))
    allocate(cphos%soil2p(beg:end))
    allocate(cphos%soil3p(beg:end))
    allocate(cphos%soil4p(beg:end))
    allocate(cphos%solutionp(beg:end))
    allocate(cphos%labilep(beg:end))
    allocate(cphos%secondp(beg:end))
    allocate(cphos%occlp(beg:end))
    allocate(cphos%primp(beg:end))
    allocate(cphos%col_ptrunc(beg:end))
    allocate(cphos%seedp(beg:end))
    allocate(cphos%prod10p(beg:end))
    allocate(cphos%prod100p(beg:end))
    allocate(cphos%totprodp(beg:end))
    allocate(cphos%totlitp(beg:end))
    allocate(cphos%totsomp(beg:end))
    allocate(cphos%totecosysp(beg:end))
    allocate(cphos%totcolp(beg:end))

    cphos%cwdp(beg:end) = nan
    cphos%litr1p(beg:end) = nan
    cphos%litr2p(beg:end) = nan
    cphos%litr3p(beg:end) = nan
    cphos%soil1p(beg:end) = nan
    cphos%soil2p(beg:end) = nan
    cphos%soil3p(beg:end) = nan
    cphos%soil4p(beg:end) = nan
    cphos%solutionp(beg:end) = nan
    cphos%labilep(beg:end) = nan
    cphos%secondp(beg:end) = nan
    cphos%occlp(beg:end) = nan
    cphos%primp(beg:end) = nan
    cphos%col_ptrunc(beg:end) = nan
    cphos%seedp(beg:end) = nan
    cphos%prod10p(beg:end) = nan
    cphos%prod100p(beg:end) = nan
    cphos%totprodp(beg:end) = nan
    cphos%totlitp(beg:end) = nan
    cphos%totsomp(beg:end) = nan
    cphos%totecosysp(beg:end) = nan
    cphos%totcolp(beg:end) = nan

  end subroutine init_column_phostate_type
...
!BOP
!
! !IROUTINE: init_column_phoflux_type
!
! !INTERFACE:
  subroutine init_column_phoflux_type(beg, end, cphof)
!
! !DESCRIPTION:
! Initialize column sphosphorus flux variables
!
! !USES:
    use surfrdMod , only : crop_prog
! !ARGUMENTS:
...
!------------------------------------------------------------------------
    ...
    allocate(cphof%primp_to_labilep(beg:end))
    allocate(cphof%labilep_to_secondp(beg:end))
    allocate(cphof%secondp_to_labilep(beg:end))
    allocate(cphof%secondp_to_occlp(beg:end))
    allocate(cphof%m_leafp_to_litr1p(beg:end))
    allocate(cphof%m_leafp_to_litr2p(beg:end))
    allocate(cphof%m_leafp_to_litr3p(beg:end))
    allocate(cphof%m_frootp_to_litr1p(beg:end))
    allocate(cphof%m_frootp_to_litr2p(beg:end))
    allocate(cphof%m_frootp_to_litr3p(beg:end))
    allocate(cphof%m_leafp_storage_to_litr1p(beg:end))
    allocate(cphof%m_frootp_storage_to_litr1p(beg:end))
    allocate(cphof%m_livestemp_storage_to_litr1p(beg:end))
    allocate(cphof%m_deadstemp_storage_to_litr1p(beg:end))
    allocate(cphof%m_livecrootp_storage_to_litr1p(beg:end))
    allocate(cphof%m_deadcrootp_storage_to_litr1p(beg:end))
    allocate(cphof%m_leafp_xfer_to_litr1p(beg:end))
    allocate(cphof%m_frootp_xfer_to_litr1p(beg:end))
    allocate(cphof%m_livestemp_xfer_to_litr1p(beg:end))
    allocate(cphof%m_deadstemp_xfer_to_litr1p(beg:end))
    allocate(cphof%m_livecrootp_xfer_to_litr1p(beg:end))
    allocate(cphof%m_deadcrootp_xfer_to_litr1p(beg:end))
    allocate(cphof%m_livestemp_to_cwdp(beg:end))
    allocate(cphof%m_deadstemp_to_cwdp(beg:end))
    allocate(cphof%m_livecrootp_to_cwdp(beg:end))
    allocate(cphof%m_deadcrootp_to_cwdp(beg:end))
    allocate(cphof%m_retransp_to_litr1p(beg:end))
    allocate(cphof%hrv_leafp_to_litr1p(beg:end))
    allocate(cphof%hrv_leafp_to_litr2p(beg:end))
    allocate(cphof%hrv_leafp_to_litr3p(beg:end))
    allocate(cphof%hrv_frootp_to_litr1p(beg:end))
    allocate(cphof%hrv_frootp_to_litr2p(beg:end))
    allocate(cphof%hrv_frootp_to_litr3p(beg:end))
    allocate(cphof%hrv_livestemp_to_cwdp(beg:end))
    allocate(cphof%hrv_deadstemp_to_prod10p(beg:end))
    allocate(cphof%hrv_deadstemp_to_prod100p(beg:end))
    allocate(cphof%hrv_livecrootp_to_cwdp(beg:end))
    allocate(cphof%hrv_deadcrootp_to_cwdp(beg:end))
    allocate(cphof%hrv_retransp_to_litr1p(beg:end))
    allocate(cphof%hrv_leafp_storage_to_litr1p(beg:end))
    allocate(cphof%hrv_frootp_storage_to_litr1p(beg:end))
    allocate(cphof%hrv_livestemp_storage_to_litr1p(beg:end))
    allocate(cphof%hrv_deadstemp_storage_to_litr1p(beg:end))
    allocate(cphof%hrv_livecrootp_storage_to_litr1p(beg:end))
    allocate(cphof%hrv_deadcrootp_storage_to_litr1p(beg:end))
    allocate(cphof%hrv_leafp_xfer_to_litr1p(beg:end))
    allocate(cphof%hrv_frootp_xfer_to_litr1p(beg:end))
    allocate(cphof%hrv_livestemp_xfer_to_litr1p(beg:end))
    allocate(cphof%hrv_deadstemp_xfer_to_litr1p(beg:end))
    allocate(cphof%hrv_livecrootp_xfer_to_litr1p(beg:end))
    allocate(cphof%hrv_deadcrootp_xfer_to_litr1p(beg:end))
    allocate(cphof%m_deadstemp_to_cwdp_fire(beg:end))
    allocate(cphof%m_deadcrootp_to_cwdp_fire(beg:end))
    allocate(cphof%m_litr1p_to_fire(beg:end))
    allocate(cphof%m_litr2p_to_fire(beg:end))
    allocate(cphof%m_litr3p_to_fire(beg:end))
    allocate(cphof%m_cwdp_to_fire(beg:end))
    if ( crop_prog )then
       allocate(cphof%grainp_to_litr1p(beg:end))
       allocate(cphof%grainp_to_litr2p(beg:end))
       allocate(cphof%grainp_to_litr3p(beg:end))
       allocate(cphof%livestemp_to_litr1p(beg:end))
       allocate(cphof%livestemp_to_litr2p(beg:end))
       allocate(cphof%livestemp_to_litr3p(beg:end))
    end if
    allocate(cphof%leafp_to_litr1p(beg:end))
    allocate(cphof%leafp_to_litr2p(beg:end))
    allocate(cphof%leafp_to_litr3p(beg:end))
    allocate(cphof%frootp_to_litr1p(beg:end))
    allocate(cphof%frootp_to_litr2p(beg:end))
    allocate(cphof%frootp_to_litr3p(beg:end))
    allocate(cphof%cwdp_to_litr2p(beg:end))
    allocate(cphof%cwdp_to_litr3p(beg:end))
    allocate(cphof%litr1p_to_soil1p(beg:end))
    allocate(cphof%sminp_to_soil1p_l1(beg:end))
    allocate(cphof%litr2p_to_soil2p(beg:end))
    allocate(cphof%sminp_to_soil2p_l2(beg:end))
    allocate(cphof%litr3p_to_soil3p(beg:end))
    allocate(cphof%sminp_to_soil3p_l3(beg:end))
    allocate(cphof%soil1p_to_soil2p(beg:end))
    allocate(cphof%sminp_to_soil2p_s1(beg:end))
    allocate(cphof%soil2p_to_soil3p(beg:end))
    allocate(cphof%sminp_to_soil3p_s2(beg:end))
    allocate(cphof%soil3p_to_soil4p(beg:end))
    allocate(cphof%sminp_to_soil4p_s3(beg:end))
    allocate(cphof%soil4p_to_sminp(beg:end))
    allocate(cphof%soil1p_to_sminp_biochem(beg:end))
    allocate(cphof%soil2p_to_sminp_biochem(beg:end))
    allocate(cphof%soil3p_to_sminp_biochem(beg:end))
    allocate(cphof%soil4p_to_sminp_biochem(beg:end))
!    allocate(cphof%sminp_to_denit_l1s1(beg:end))
!    allocate(cphof%sminp_to_denit_l2s2(beg:end))
!    allocate(cphof%sminp_to_denit_l3s3(beg:end))
!    allocate(cphof%sminp_to_denit_s1s2(beg:end))
!    allocate(cphof%sminn_to_denit_s2s3(beg:end))
!    allocate(cphof%sminn_to_denit_s3s4(beg:end))
!    allocate(cphof%sminn_to_denit_s4(beg:end))
!    allocate(cphof%sminn_to_denit_excess(beg:end))
    allocate(cphof%sminp_leached(beg:end))
    allocate(cphof%dwt_seedp_to_leaf(beg:end))
    allocate(cphof%dwt_seedp_to_deadstem(beg:end))
    allocate(cphof%dwt_conv_pflux(beg:end))
    allocate(cphof%dwt_prod10p_gain(beg:end))
    allocate(cphof%dwt_prod100p_gain(beg:end))
    allocate(cphof%dwt_frootp_to_litr1p(beg:end))
    allocate(cphof%dwt_frootp_to_litr2p(beg:end))
    allocate(cphof%dwt_frootp_to_litr3p(beg:end))
    allocate(cphof%dwt_livecrootp_to_cwdp(beg:end))
    allocate(cphof%dwt_deadcrootp_to_cwdp(beg:end))
    allocate(cphof%dwt_ploss(beg:end))
    allocate(cphof%prod10p_loss(beg:end))
    allocate(cphof%prod100p_loss(beg:end))
    allocate(cphof%product_ploss(beg:end))
    allocate(cphof%potential_immob_p(beg:end))
    allocate(cphof%actual_immob_p(beg:end))
    allocate(cphof%sminp_to_plant(beg:end))
    allocate(cphof%supplement_to_sminp(beg:end))
    allocate(cphof%gross_pmin(beg:end))
    allocate(cphof%net_pmin(beg:end))
    allocate(cphof%biochem_pmin(beg:end))
!    allocate(cphof%denit(beg:end))
    allocate(cphof%col_pinputs(beg:end))
    allocate(cphof%col_poutputs(beg:end))
    allocate(cphof%col_fire_ploss(beg:end))

!    cphof%ndep_to_sminn(beg:end) = nan
!    cphof%nfix_to_sminn(beg:end) = nan
    cphof%m_leafp_to_litr1p(beg:end) = nan
    cphof%m_leafp_to_litr2p(beg:end) = nan
    cphof%m_leafp_to_litr3p(beg:end) = nan
    cphof%m_frootp_to_litr1p(beg:end) = nan
    cphof%m_frootp_to_litr2p(beg:end) = nan
    cphof%m_frootp_to_litr3p(beg:end) = nan
    cphof%m_leafp_storage_to_litr1p(beg:end) = nan
    cphof%m_frootp_storage_to_litr1p(beg:end) = nan
    cphof%m_livestemp_storage_to_litr1p(beg:end) = nan
    cphof%m_deadstemp_storage_to_litr1p(beg:end) = nan
    cphof%m_livecrootp_storage_to_litr1p(beg:end) = nan
    cphof%m_deadcrootp_storage_to_litr1p(beg:end) = nan
    cphof%m_leafp_xfer_to_litr1p(beg:end) = nan
    cphof%m_frootp_xfer_to_litr1p(beg:end) = nan
    cphof%m_livestemp_xfer_to_litr1p(beg:end) = nan
    cphof%m_deadstemp_xfer_to_litr1p(beg:end) = nan
    cphof%m_livecrootp_xfer_to_litr1p(beg:end) = nan
    cphof%m_deadcrootp_xfer_to_litr1p(beg:end) = nan
    cphof%m_livestemp_to_cwdp(beg:end) = nan
    cphof%m_deadstemp_to_cwdp(beg:end) = nan
    cphof%m_livecrootp_to_cwdp(beg:end) = nan
    cphof%m_deadcrootp_to_cwdp(beg:end) = nan
    cphof%m_retransp_to_litr1p(beg:end) = nan
    cphof%hrv_leafp_to_litr1p(beg:end) = nan
    cphof%hrv_leafp_to_litr2p(beg:end) = nan
    cphof%hrv_leafp_to_litr3p(beg:end) = nan
    cphof%hrv_frootp_to_litr1p(beg:end) = nan
    cphof%hrv_frootp_to_litr2p(beg:end) = nan
    cphof%hrv_frootp_to_litr3p(beg:end) = nan
    cphof%hrv_livestemp_to_cwdp(beg:end) = nan
    cphof%hrv_deadstemp_to_prod10p(beg:end) = nan
    cphof%hrv_deadstemp_to_prod100p(beg:end) = nan
    cphof%hrv_livecrootp_to_cwdp(beg:end) = nan
    cphof%hrv_deadcrootp_to_cwdp(beg:end) = nan
    cphof%hrv_retransp_to_litr1p(beg:end) = nan
    cphof%hrv_leafp_storage_to_litr1p(beg:end) = nan
    cphof%hrv_frootp_storage_to_litr1p(beg:end) = nan
    cphof%hrv_livestemp_storage_to_litr1p(beg:end) = nan
    cphof%hrv_deadstemp_storage_to_litr1p(beg:end) = nan
    cphof%hrv_livecrootp_storage_to_litr1p(beg:end) = nan
    cphof%hrv_deadcrootp_storage_to_litr1p(beg:end) = nan
    cphof%hrv_leafp_xfer_to_litr1p(beg:end) = nan
    cphof%hrv_frootp_xfer_to_litr1p(beg:end) = nan
    cphof%hrv_livestemp_xfer_to_litr1p(beg:end) = nan
    cphof%hrv_deadstemp_xfer_to_litr1p(beg:end) = nan
    cphof%hrv_livecrootp_xfer_to_litr1p(beg:end) = nan
    cphof%hrv_deadcrootp_xfer_to_litr1p(beg:end) = nan
    cphof%m_deadstemp_to_cwdp_fire(beg:end) = nan
    cphof%m_deadcrootp_to_cwdp_fire(beg:end) = nan
    cphof%m_litr1p_to_fire(beg:end) = nan
    cphof%m_litr2p_to_fire(beg:end) = nan
    cphof%m_litr3p_to_fire(beg:end) = nan
    cphof%m_cwdp_to_fire(beg:end) = nan
    if ( crop_prog )then
       cphof%grainp_to_litr1p(beg:end)    = nan
       cphof%grainp_to_litr2p(beg:end)    = nan
       cphof%grainp_to_litr3p(beg:end)    = nan
       cphof%livestemp_to_litr1p(beg:end) = nan
       cphof%livestemp_to_litr2p(beg:end) = nan
       cphof%livestemp_to_litr3p(beg:end) = nan
    end if
    cphof%leafp_to_litr1p(beg:end) = nan
    cphof%leafp_to_litr2p(beg:end) = nan
    cphof%leafp_to_litr3p(beg:end) = nan
    cphof%frootp_to_litr1p(beg:end) = nan
    cphof%frootp_to_litr2p(beg:end) = nan
    cphof%frootp_to_litr3p(beg:end) = nan
    cphof%cwdp_to_litr2p(beg:end) = nan
    cphof%cwdp_to_litr3p(beg:end) = nan
    cphof%litr1p_to_soil1p(beg:end) = nan
    cphof%sminp_to_soil1p_l1(beg:end) = nan
    cphof%litr2p_to_soil2p(beg:end) = nan
    cphof%sminp_to_soil2p_l2(beg:end) = nan
    cphof%litr3p_to_soil3p(beg:end) = nan
    cphof%sminp_to_soil3p_l3(beg:end) = nan
    cphof%soil1p_to_soil2p(beg:end) = nan
    cphof%sminp_to_soil2p_s1(beg:end) = nan
    cphof%soil2p_to_soil3p(beg:end) = nan
    cphof%sminp_to_soil3p_s2(beg:end) = nan
    cphof%soil3p_to_soil4p(beg:end) = nan
    cphof%sminp_to_soil4p_s3(beg:end) = nan
    cphof%soil4p_to_sminp(beg:end) = nan
    cphof%soil1p_to_sminp_biochem(beg:end) = nan
    cphof%soil2p_to_sminp_biochem(beg:end) = nan
    cphof%soil3p_to_sminp_biochem(beg:end) = nan
    cphof%soil4p_to_sminp_biochem(beg:end) = nan
!    cphof%sminn_to_denit_l1s1(beg:end) = nan
!    cphof%sminn_to_denit_l2s2(beg:end) = nan
!    cphof%sminn_to_denit_l3s3(beg:end) = nan
!    cphof%sminn_to_denit_s1s2(beg:end) = nan
!    cphof%sminn_to_denit_s2s3(beg:end) = nan
!    cphof%sminn_to_denit_s3s4(beg:end) = nan
!    cphof%sminn_to_denit_s4(beg:end) = nan
!    cphof%sminn_to_denit_excess(beg:end) = nan
    cphof%sminp_leached(beg:end) = nan
    cphof%dwt_seedp_to_leaf(beg:end) = nan
    cphof%dwt_seedp_to_deadstem(beg:end) = nan
    cphof%dwt_conv_pflux(beg:end) = nan
    cphof%dwt_prod10p_gain(beg:end) = nan
    cphof%dwt_prod100p_gain(beg:end) = nan
    cphof%dwt_frootp_to_litr1p(beg:end) = nan
    cphof%dwt_frootp_to_litr2p(beg:end) = nan
    cphof%dwt_frootp_to_litr3p(beg:end) = nan
    cphof%dwt_livecrootp_to_cwdp(beg:end) = nan
    cphof%dwt_deadcrootp_to_cwdp(beg:end) = nan
    cphof%dwt_ploss(beg:end) = nan
    cphof%prod10p_loss(beg:end) = nan
    cphof%prod100p_loss(beg:end) = nan
    cphof%product_ploss(beg:end) = nan
    cphof%potential_immob_p(beg:end) = nan
    cphof%actual_immob_p(beg:end) = nan
    cphof%sminp_to_plant(beg:end) = nan
    cphof%supplement_to_sminp(beg:end) = nan
    cphof%gross_pmin(beg:end) = nan
    cphof%net_pmin(beg:end) = nan
    cphof%biochem_pmin(beg:end) = nan
!    cphof%denit(beg:end) = nan
    cphof%col_pinputs(beg:end) = nan
    cphof%col_poutputs(beg:end) = nan
    cphof%col_fire_ploss(beg:end) = nan

  end subroutine init_column_phoflux_type
