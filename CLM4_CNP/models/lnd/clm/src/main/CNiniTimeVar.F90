...
!  add phosphorus
   real(r8), pointer :: leafcp(:)    ! leaf C:N (gC/gN)
   real(r8), pointer :: deadwdcp(:)  ! dead wood (xylem and heartwood) C:N (gC/gN)
!
...
!! variables for phosphorus
   ...
   real(r8), pointer :: tempmax_retransp(:)   ! temporary max of retranslocated N pool (gN/m2)
   ...
   real(r8), pointer :: totecosysp(:)         ! (gP/m2) total ecosystem , incl veg
...
!   add phosphorus  
   ...
   do c = begc, endc
      ...
      if (itypelun(l) == istsoil .or. itypelun(l) == istcrop) then
         ...
         soil2c(c) = 0._r8
         soil3c(c) = 0._r8
         soil4c(c) = 1000._r8    !DMR 5/19/2011
         col_ctrunc(c) = 0._r8
         totlitc(c)    = 0._r8
         totsomc(c)    = 0._r8
         totecosysc(c) = 0._r8
         ...
         cwdn(c)   = cwdc(c) / 200._r8
         litr1n(c) = litr1c(c) / 50._r8
         litr2n(c) = litr2c(c) / 50._r8
         litr3n(c) = litr3c(c) / 50._r8
         ...
         ! add column phosphorus state variables
         cwdp(c)   = cwdn(c) / 20._r8
         litr1p(c) = litr1n(c) / 20._r8
         litr2p(c) = litr2n(c) / 20._r8
         litr3p(c) = litr3n(c) / 20._r8
         soil1p(c) = soil1n(c) / 30._r8
         soil2p(c) = soil2n(c) / 30._r8
         soil3p(c) = soil3n(c) / 50._r8
         soil4p(c) = soil4n(c) / 50._r8
         solutionp(c) = 0.05_r8
         labilep(c) = 5._r8
         secondp(c) = 30._r8
         occlp(c) = 180._r8
	 ...
	 clm3%g%l%c%cnf%dwt_nloss(c) = 0._r8
         ...
         !! add phosphorus   
	 clm3%g%l%c%cphof%dwt_seedp_to_leaf(c) = 0._r8
	 clm3%g%l%c%cphof%dwt_seedp_to_deadstem(c) = 0._r8
	 clm3%g%l%c%cphof%dwt_conv_pflux(c) = 0._r8
	 clm3%g%l%c%cphof%dwt_prod10p_gain(c) = 0._r8
	 clm3%g%l%c%cphof%prod10p_loss(c) = 0._r8
	 clm3%g%l%c%cphof%dwt_prod100p_gain(c) = 0._r8
	 clm3%g%l%c%cphof%prod100p_loss(c) = 0._r8
	 clm3%g%l%c%cphof%dwt_frootp_to_litr1p(c) = 0._r8
	 clm3%g%l%c%cphof%dwt_frootp_to_litr2p(c) = 0._r8
	 clm3%g%l%c%cphof%dwt_frootp_to_litr3p(c) = 0._r8
	 clm3%g%l%c%cphof%dwt_livecrootp_to_cwdp(c) = 0._r8
	 clm3%g%l%c%cphof%dwt_deadcrootp_to_cwdp(c) = 0._r8
	 clm3%g%l%c%cphof%dwt_ploss(c) = 0._r8
      end if
   end do
   ...
   do p = begp, endp
      ...
      if (itypelun(l) == istsoil .or. itypelun(l) == istcrop) then
         ...
         ! add phosphorus state variables
         if (ivt(p) == noveg) then
            leafp(p) = 0._r8
            leafp_storage(p) = 0._r8
         else
            leafp(p) = leafc(p) / leafcp(ivt(p))
            leafp_storage(p) = leafc_storage(p) / leafcp(ivt(p))
         end if

         leafp_xfer(p) = 0._r8
         frootp(p) = 0._r8
         frootp_storage(p) = 0._r8
         frootp_xfer(p) = 0._r8
         livestemp(p) = 0._r8
         livestemp_storage(p) = 0._r8
         ...
         if (woody(ivt(p)) == 1._r8) then
            deadstemp(p) = deadstemc(p) / deadwdcp(ivt(p))
         else
            deadstemp(p) = 0._r8
         end if

         deadstemp_storage(p) = 0._r8
         deadstemp_xfer(p) = 0._r8
         livecrootp(p) = 0._r8
         livecrootp_storage(p) = 0._r8
         livecrootp_xfer(p) = 0._r8
         deadcrootp(p) = 0._r8
         deadcrootp_storage(p) = 0._r8
         ...
         alphapsnsha(p) = 0._r8
#endif
         !! phosphorus :: 11/06/2011, X. YANG
         p_allometry(p) = 0._r8
         plant_pdemand(p) = 0._r8
         tempmax_retransp(p) = 0._r8
         annmax_retransp(p) = 0._r8
         avail_retransp(p) = 0._r8
         plant_palloc(p) = 0._r8
