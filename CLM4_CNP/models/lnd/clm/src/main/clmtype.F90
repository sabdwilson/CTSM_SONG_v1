...
!----------------------------------------------------
! phosphorus balance structure
!----------------------------------------------------
type, public :: phosphorus_balance_type
   real(r8), pointer :: begpb(:)         !phosphorus mass, beginning of time step (gN/m**2)
   real(r8), pointer :: endpb(:)         !phosphorus mass, end of time step (gN/m**2)
   real(r8), pointer :: errpb(:)         !phosphorus balance error for the timestep (gN/m**2)
end type phosphorus_balance_type
...
type, public :: pft_epc_type
   ...
   ! add phosphorus 
   real(r8), pointer :: leafcp(:)    !leaf C:P (gC/gP)
   real(r8), pointer :: lflitcp(:)      !leaf litter C:P (gC/gP)
   real(r8), pointer :: frootcp(:)      !fine root C:P (gC/gP)
   real(r8), pointer :: livewdcp(:)     !live wood (phloem and ray parenchyma) C:P (gC/gP)
   real(r8), pointer :: deadwdcp(:)     !dead wood (xylem and heartwood) C:P (gC/gP)
   real(r8), pointer :: graincp(:)      !grain C:N (gC/gN) for prognostic crop model
...
type, public :: pft_epv_type
   ...
   !! add variables for phosphorus
   real(r8), pointer :: p_allometry(:)          !N allocation index (DIM)
   real(r8), pointer :: plant_pdemand(:)        !N flux required to support initial GPP (gN/m2/s)
   real(r8), pointer :: tempmax_retransp(:)     !temporary annual max of retranslocated N pool (gN/m2)
   real(r8), pointer :: annmax_retransp(:)      !annual max of retranslocated N pool (gN/m2)
   real(r8), pointer :: avail_retransp(:)       !N flux available from retranslocation pool (gN/m2/s)
   real(r8), pointer :: plant_palloc(:)         !total allocated N flux (gN/m2/s)
...
!----------------------------------------------------
! pft phosphorus state variables structure
!----------------------------------------------------
type, public :: pft_phostate_type
   ! variables for prognostic crop model
   real(r8), pointer :: grainp(:)             ! (gP/m2) grain P 
   real(r8), pointer :: grainp_storage(:)     ! (gP/m2) grain P storage
   real(r8), pointer :: grainp_xfer(:)        ! (gP/m2) grain P transfer
   !
   ...
   real(r8), pointer :: pft_ptrunc(:)         ! (gP/m2) pft-level sink for P truncation
   ! summary (diagnostic) state variables, not involved in mass balance
...
!----------------------------------------------------
! pft phosphorus flux variables structure
!----------------------------------------------------
type, public :: pft_phoflux_type
   ! new variables for CN code
   ! gap mortality fluxes
   ...
   ! fire mortality fluxes
   real(r8), pointer :: m_leafp_to_fire(:)                  ! leaf P fire loss (gP/m2/s)
   real(r8), pointer :: m_leafp_storage_to_fire(:)          ! leaf P storage fire loss (gP/m2/s)
   real(r8), pointer :: m_leafp_xfer_to_fire(:)             ! leaf P transfer fire loss (gP/m2/s)
   real(r8), pointer :: m_frootp_to_fire(:)                 ! fine root P fire loss (gP/m2/s)
   real(r8), pointer :: m_frootp_storage_to_fire(:)         ! fine root P storage fire loss (gP/m2/s)
   real(r8), pointer :: m_frootp_xfer_to_fire(:)            ! fine root P transfer fire loss (gP/m2/s)
   real(r8), pointer :: m_livestemp_to_fire(:)              ! live stem P fire loss (gP/m2/s)
   real(r8), pointer :: m_livestemp_storage_to_fire(:)      ! live stem P storage fire loss (gP/m2/s)
   real(r8), pointer :: m_livestemp_xfer_to_fire(:)         ! live stem P transfer fire loss (gP/m2/s)
   real(r8), pointer :: m_deadstemp_to_fire(:)              ! dead stem P fire loss (gP/m2/s)
   real(r8), pointer :: m_deadstemp_to_litter_fire(:)       ! dead stem P fire mortality to litter (gP/m2/s)
   real(r8), pointer :: m_deadstemp_storage_to_fire(:)      ! dead stem P storage fire loss (gP/m2/s)
   real(r8), pointer :: m_deadstemp_xfer_to_fire(:)         ! dead stem P transfer fire loss (gP/m2/s)
   real(r8), pointer :: m_livecrootp_to_fire(:)             ! live coarse root P fire loss (gP/m2/s)
   real(r8), pointer :: m_livecrootp_storage_to_fire(:)     ! live coarse root P storage fire loss (gP/m2/s)
   real(r8), pointer :: m_livecrootp_xfer_to_fire(:)        ! live coarse root P transfer fire loss (gP/m2/s)
   real(r8), pointer :: m_deadcrootp_to_fire(:)             ! dead coarse root P fire loss (gP/m2/s)
   real(r8), pointer :: m_deadcrootp_to_litter_fire(:)      ! dead coarse root P fire mortality to litter (gP/m2/s)
   real(r8), pointer :: m_deadcrootp_storage_to_fire(:)     ! dead coarse root P storage fire loss (gP/m2/s)
   real(r8), pointer :: m_deadcrootp_xfer_to_fire(:)        ! dead coarse root P transfer fire loss (gP/m2/s)
   real(r8), pointer :: m_retransp_to_fire(:)               ! retranslocated N pool fire loss (gP/m2/s)
   ! phenology fluxes from transfer pool                     
   ...
   real(r8), pointer :: leafp_to_retransp(:)                ! leaf P to retranslocated N pool (gP/m2/s)
   ...
   real(r8), pointer :: pft_pinputs(:)                      ! total P inputs to pft-level (gP/m2/s)
   real(r8), pointer :: pft_poutputs(:)                     ! total P outputs from pft-level (gP/m2/s)
   real(r8), pointer :: wood_harvestp(:)                    ! total P losses to wood product pools (gP/m2/s)
   ! new variables for fire code 
   real(r8), pointer :: pft_fire_ploss(:)                   ! total pft-level fire P loss (gP/m2/s) 
end type pft_phoflux_type
...
!! add variables for phosphorus code
   integer, pointer ::  isoiorder(:)     ! soil order
   real(r8), pointer :: fpi_p(:)
   real(r8), pointer :: fpg_p(:)
   ! new variables for fire code
...
type, public :: column_nstate_type
 ...
 !----------------------------------------------------
 ! column phosphorus state variables structure !----------------------------------------------------
 type, public :: column_phostate_type
   type(pft_phostate_type):: pphos_a              !pft-level nitrogen state variables averaged to the column
   ! BGC variables
   real(r8), pointer :: cwdp(:)               ! (gP/m2) coarse woody debris P
   real(r8), pointer :: litr1p(:)             ! (gP/m2) litter labile P
   real(r8), pointer :: litr2p(:)             ! (gP/m2) litter cellulose P
   real(r8), pointer :: litr3p(:)             ! (gP/m2) litter lignin P
   ...
   real(r8), pointer :: solutionp(:)              ! (gP/m2) soil solution P
   ...
   real(r8), pointer :: col_ptrunc(:)         ! (gP/m2) column-level sink for N truncation
   ! wood product pools, for dynamic landcover
   real(r8), pointer :: seedp(:)              ! (gP/m2) column-level pool for seeding new PFTs
   real(r8), pointer :: prod10p(:)            ! (gP/m2) wood product P pool, 10-year lifespan
   real(r8), pointer :: prod100p(:)           ! (gP/m2) wood product P pool, 100-year lifespan
   real(r8), pointer :: totprodp(:)           ! (gP/m2) total wood product P
    ! summary (diagnostic) state variables, not involved in mass balance
   real(r8), pointer :: totlitp(:)            ! (gP/m2) total litter nitrogen
   real(r8), pointer :: totsomp(:)            ! (gP/m2) total soil organic matter nitrogen
   real(r8), pointer :: totecosysp(:)         ! (gP/m2) total ecosystem nitrogen, incl veg 
   real(r8), pointer :: totcolp(:)            ! (gP/m2) total column nitrogen, incl veg
end type column_phostate_type
...
!----------------------------------------------------
! column phosphorus flux variables structure
!----------------------------------------------------
type, public :: column_phoflux_type
   type(pft_phoflux_type):: pphof_a        !pft-level nitrogen flux variables averaged to the column
   ! new variables for phosphorus code
   ! inorganic P transformation fluxes
   real(r8), pointer :: primp_to_labilep(:)                ! P from primary mineral to labile 
   real(r8), pointer :: labilep_to_secondp(:)              ! labile P to secondary mineral P 
   real(r8), pointer :: secondp_to_labilep(:)              ! the desorption of secondary mineral P to labile P
   real(r8), pointer :: secondp_to_occlp(:)                ! the occlusion of secondary P to occluded P

   ! column-level gap mortality fluxes
   real(r8), pointer :: m_leafp_to_litr1p(:)               ! leaf P mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: m_leafp_to_litr2p(:)               ! leaf P mortality to litter 2 P (gP/m2/s)
   real(r8), pointer :: m_leafp_to_litr3p(:)               ! leaf P mortality to litter 3 P (gP/m2/s)
   real(r8), pointer :: m_frootp_to_litr1p(:)              ! fine root P mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: m_frootp_to_litr2p(:)              ! fine root P mortality to litter 2 P (gP/m2/s)
   real(r8), pointer :: m_frootp_to_litr3p(:)              ! fine root P mortality to litter 3 P (gP/m2/s)
   real(r8), pointer :: m_livestemp_to_cwdp(:)             ! live stem P mortality to coarse woody debris P (gP/m2/s)
   real(r8), pointer :: m_deadstemp_to_cwdp(:)             ! dead stem P mortality to coarse woody debris P (gP/m2/s)
   real(r8), pointer :: m_livecrootp_to_cwdp(:)            ! live coarse root P mortality to coarse woody debris P (gP/m2/s)
   real(r8), pointer :: m_deadcrootp_to_cwdp(:)            ! dead coarse root P mortality to coarse woody debris P (gP/m2/s)
   real(r8), pointer :: m_retransp_to_litr1p(:)            ! retranslocated P pool mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: m_leafp_storage_to_litr1p(:)       ! leaf P storage mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: m_frootp_storage_to_litr1p(:)      ! fine root P storage mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: m_livestemp_storage_to_litr1p(:)   ! live stem P storage mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: m_deadstemp_storage_to_litr1p(:)   ! dead stem P storage mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: m_livecrootp_storage_to_litr1p(:)  ! live coarse root P storage mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: m_deadcrootp_storage_to_litr1p(:)  ! dead coarse root P storage mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: m_leafp_xfer_to_litr1p(:)          ! leaf P transfer mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: m_frootp_xfer_to_litr1p(:)         ! fine root P transfer mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: m_livestemp_xfer_to_litr1p(:)      ! live stem P transfer mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: m_deadstemp_xfer_to_litr1p(:)      ! dead stem P transfer mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: m_livecrootp_xfer_to_litr1p(:)     ! live coarse root P transfer mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: m_deadcrootp_xfer_to_litr1p(:)     ! dead coarse root P transfer mortality to litter 1 P (gP/m2/s)
   ! column-level harvest fluxes
   real(r8), pointer :: hrv_leafp_to_litr1p(:)               ! leaf P harvest mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: hrv_leafp_to_litr2p(:)               ! leaf P harvest mortality to litter 2 P (gP/m2/s)
   real(r8), pointer :: hrv_leafp_to_litr3p(:)               ! leaf P harvest mortality to litter 3 P (gP/m2/s)
   real(r8), pointer :: hrv_frootp_to_litr1p(:)              ! fine root P harvest mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: hrv_frootp_to_litr2p(:)              ! fine root P harvest mortality to litter 2 P (gP/m2/s)
   real(r8), pointer :: hrv_frootp_to_litr3p(:)              ! fine root P harvest mortality to litter 3 P (gP/m2/s)
   real(r8), pointer :: hrv_livestemp_to_cwdp(:)             ! live stem P harvest mortality to coarse woody debris P (gP/m2/s)
   ...
   real(r8), pointer :: hrv_livecrootp_to_cwdp(:)            ! live coarse root P harvest mortality to coarse woody debris P (gP/m2/s)
   real(r8), pointer :: hrv_deadcrootp_to_cwdp(:)            ! dead coarse root P harvest mortality to coarse woody debris P (gP/m2/s)
   real(r8), pointer :: hrv_retransp_to_litr1p(:)            ! retranslocated P pool harvest mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: hrv_leafp_storage_to_litr1p(:)       ! leaf P storage harvest mortality to litter 1 N (gP/m2/s)
   real(r8), pointer :: hrv_frootp_storage_to_litr1p(:)      ! fine root P storage harvest mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: hrv_livestemp_storage_to_litr1p(:)   ! live stem P storage harvest mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: hrv_deadstemp_storage_to_litr1p(:)   ! dead stem P storage harvest mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: hrv_livecrootp_storage_to_litr1p(:)  ! live coarse root P storage harvest mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: hrv_deadcrootp_storage_to_litr1p(:)  ! dead coarse root P storage harvest mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: hrv_leafp_xfer_to_litr1p(:)          ! leaf P transfer harvest mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: hrv_frootp_xfer_to_litr1p(:)         ! fine root P transfer harvest mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: hrv_livestemp_xfer_to_litr1p(:)      ! live stem P transfer harvest mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: hrv_deadstemp_xfer_to_litr1p(:)      ! dead stem P transfer harvest mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: hrv_livecrootp_xfer_to_litr1p(:)     ! live coarse root P transfer harvest mortality to litter 1 P (gP/m2/s)
   real(r8), pointer :: hrv_deadcrootp_xfer_to_litr1p(:)     ! dead coarse root P transfer harvest mortality to litter 1 P (gP/m2/s)
   ! column-level fire fluxes
   real(r8), pointer :: m_deadstemp_to_cwdp_fire(:)        ! dead stem P to coarse woody debris P by fire (gP/m2/s)
   real(r8), pointer :: m_deadcrootp_to_cwdp_fire(:)       ! dead coarse root P to to woody debris P by fire (gP/m2/s)
   real(r8), pointer :: m_litr1p_to_fire(:)                ! litter 1 P fire loss (gP/m2/s)
   real(r8), pointer :: m_litr2p_to_fire(:)                ! litter 2 P fire loss (gP/m2/s)
   real(r8), pointer :: m_litr3p_to_fire(:)                ! litter 3 P fire loss (gP/m2/s)
   real(r8), pointer :: m_cwdp_to_fire(:)                  ! coarse woody debris P fire loss (gP/m2/s)
   ! litterfall fluxes
   real(r8), pointer :: livestemp_to_litr1p(:)   ! livestem P litterfall to litter 1 P (gP/m2/s)
   real(r8), pointer :: livestemp_to_litr2p(:)   ! livestem P litterfall to litter 2 P (gP/m2/s)
   real(r8), pointer :: livestemp_to_litr3p(:)   ! livestem P litterfall to litter 3 P (gP/m2/s)
   real(r8), pointer :: leafp_to_litr1p(:)       ! leaf P litterfall to litter 1 P (gP/m2/s)
   real(r8), pointer :: leafp_to_litr2p(:)       ! leaf P litterfall to litter 2 P (gP/m2/s)
   real(r8), pointer :: leafp_to_litr3p(:)       ! leaf P litterfall to litter 3 P (gP/m2/s)
   real(r8), pointer :: frootp_to_litr1p(:)      ! fine root P litterfall to litter 1 P (gP/m2/s)
   real(r8), pointer :: frootp_to_litr2p(:)      ! fine root P litterfall to litter 2 P (gP/m2/s)
   real(r8), pointer :: frootp_to_litr3p(:)      ! fine root P litterfall to litter 3 P (gP/m2/s)
   ! litterfall fluxes for prognostic crop model            
   real(r8), pointer :: grainp_to_litr1p(:)      ! grain P litterfall to litter 1 P (gP/m2/s)
   real(r8), pointer :: grainp_to_litr2p(:)      ! grain P litterfall to litter 2 P (gP/m2/s)
   real(r8), pointer :: grainp_to_litr3p(:)      ! grain P litterfall to litter 3 P (gP/m2/s)
   ! decomposition fluxes
   real(r8), pointer :: cwdp_to_litr2p(:)        ! decomp. of coarse woody debris P to litter 2 P (gP/m2/s)
   real(r8), pointer :: cwdp_to_litr3p(:)        ! decomp. of coarse woody debris P to litter 3 P (gP/m2/s)
   real(r8), pointer :: litr1p_to_soil1p(:)      ! decomp. of litter 1 P to SOM 1 P (gP/m2/s)
   real(r8), pointer :: sminp_to_soil1p_l1(:)    ! mineral P flux for decomp. of litter 1 to SOM 1 (gP/m2/s)
   real(r8), pointer :: litr2p_to_soil2p(:)      ! decomp. of litter 2 P to SOM 2 P (gP/m2/s)
   real(r8), pointer :: sminp_to_soil2p_l2(:)    ! mineral P flux for decomp. of litter 2 to SOM 2 (gP/m2/s)
   real(r8), pointer :: litr3p_to_soil3p(:)      ! decomp. of litter 3 P to SOM 3 P (gP/m2/s)
   real(r8), pointer :: sminp_to_soil3p_l3(:)    ! mineral P flux for decomp. of litter 3 to SOM 3 (gP/m2/s)
   real(r8), pointer :: soil1p_to_soil2p(:)      ! decomp. of SOM 1 P to SOM 2 P (gP/m2/s)
   real(r8), pointer :: sminp_to_soil2p_s1(:)    ! mineral P flux for decomp. of SOM 1 to SOM 2 (gP/m2/s)
   real(r8), pointer :: soil2p_to_soil3p(:)      ! decomp. of SOM 2 P to SOM 3 P (gP/m2/s)
   real(r8), pointer :: sminp_to_soil3p_s2(:)    ! mineral P flux for decomp. of SOM 2 to SOM 3 (gP/m2/s)
   real(r8), pointer :: soil3p_to_soil4p(:)      ! decomp. of SOM 3 P to SOM 4 P (gP/m2/s)
   real(r8), pointer :: sminp_to_soil4p_s3(:)    ! mineral P flux for decomp. of SOM 3 to SOM 4 (gP/m2/s)
   real(r8), pointer :: soil4p_to_sminp(:)       ! P mineralization for decomp. of SOM 4 (gP/m2/s)

!! biochemical mineraliation of soil organic phosphorus
...
!!!Xiaojuan Yang, there is no gaseous output for phosphorus
   ! denitrification fluxes
!   real(r8), pointer :: sminn_to_denit_l1s1(:)   ! denitrification for decomp. of litter 1 to SOM 1 (gN/m2/s) 
!   real(r8), pointer :: sminn_to_denit_l2s2(:)   ! denitrification for decomp. of litter 2 to SOM 2 (gN/m2/s)
!   real(r8), pointer :: sminn_to_denit_l3s3(:)   ! denitrification for decomp. of litter 3 to SOM 3 (gN/m2/s)
...
!   real(r8), pointer :: sminn_to_denit_excess(:) ! denitrification from excess mineral N pool (gN/m2/s)
   ...
   ! dynamic landcover fluxes
   real(r8), pointer :: dwt_seedp_to_leaf(:)      ! (gP/m2/s) seed source to PFT-level
   real(r8), pointer :: dwt_seedp_to_deadstem(:)  ! (gP/m2/s) seed source to PFT-level
   real(r8), pointer :: dwt_conv_pflux(:)         ! (gP/m2/s) conversion N flux (immediate loss to atm)
   ...
   real(r8), pointer :: dwt_frootp_to_litr1p(:)   ! (gP/m2/s) fine root to litter due to landcover change
   real(r8), pointer :: dwt_frootp_to_litr2p(:)   ! (gP/m2/s) fine root to litter due to landcover change
   real(r8), pointer :: dwt_frootp_to_litr3p(:)   ! (gP/m2/s) fine root to litter due to landcover change
   real(r8), pointer :: dwt_livecrootp_to_cwdp(:) ! (gP/m2/s) live coarse root to CWD due to landcover change
   real(r8), pointer :: dwt_deadcrootp_to_cwdp(:) ! (gP/m2/s) dead coarse root to CWD due to landcover change
   ...
   ! summary (diagnostic) flux variables, not involved in mass balance
   real(r8), pointer :: potential_immob_p(:)       ! potential P immobilization (gP/m2/s)
   real(r8), pointer :: actual_immob_p(:)          ! actual P immobilization (gP/m2/s)
   real(r8), pointer :: sminp_to_plant(:)        ! plant uptake of soil mineral P (gP/m2/s)
   real(r8), pointer :: supplement_to_sminp(:)   ! supplemental P supply (gP/m2/s)
   real(r8), pointer :: gross_pmin(:)            ! gross rate of P mineralization (gP/m2/s)
   real(r8), pointer :: net_pmin(:)              ! net rate of P mineralization (gP/m2/s)
   real(r8), pointer :: biochem_pmin(:)              ! rate of biochemical P mineralization (gP/m2/s)
!   real(r8), pointer :: denit(:)                 ! total rate of denitrification (gP/m2/s)
...
!----------------------------------------------------
! gridcell phosphorus state variables structure
!----------------------------------------------------
type, public :: gridcell_phostate_type
   type(column_phostate_type):: cphos_a            !column-level phosphorus state variables averaged to gridcell
end type gridcell_phostate_type
...
!----------------------------------------------------
! gridcell phosphorus flux variables structure
!----------------------------------------------------
type, public :: gridcell_phoflux_type
   type(column_phoflux_type):: cphof_a             !column-level phosphorus flux variables averaged to gridcell
end type gridcell_phoflux_type
...
type, public :: pft_type
   ...
   type(phosphorus_balance_type) :: pphobal !nitrogen balance structure
   ...
   type(pft_phostate_type) :: pphos         !pft phosphorus state
   ...
   type(pft_phoflux_type)  :: pphof         !pft phosphorus flux
...
type, public :: column_type
   ...
   type(phosphorus_balance_type) :: cphobal !nitrogen balance structure
   ...
   type(column_phostate_type) :: cphos      !column phosphorus state
   ...
   type(column_phoflux_type) :: cphof       !column phosphorus flux
