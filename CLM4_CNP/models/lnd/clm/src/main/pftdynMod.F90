...
! ! PRIVATE TYPES
  ...
  integer :: ntimes
  logical :: do_harvest
  logical :: do_harvest_new    ! temporary logic variable
                               ! added by JFM for harvest at the first time step, 09/01/11
...
! !USES:
    use shr_kind_mod, only : r8 => shr_kind_r8
    use shr_const_mod,only : SHR_CONST_PDB, SHR_CONST_C14
    use decompMod   , only : get_proc_bounds
    use clm_varcon  , only : istsoil
    use clm_varcon  , only : istcrop
...
! modified by JFM between this line and line 2805, 09/01/11
  ! ============================================================================
  ! make the dynpft happen at first time step of year
  ! NOTE: mon, day, and sec correspond to nstep+1
  ! ============================================================================
   call get_curr_date(year, mon, day, sec)

   nstep = get_nstep()
   dtime = get_step_size()
!   write(iulog,*)'mao1 = ', year, mon, day, sec, nstep, dtime

   if (do_harvest) then

      ! by choosing different mon, day and sec here, we actually can let harvest happen at any time step
      if (mon==1 .and. day==1 .and. sec==0) then 
!         write(iulog,*)'mao2 = ', year, mon, day, sec, nstep
         do_harvest_new = .true.
      else
         do_harvest_new = .false.
      end if
   else
      do_harvest_new = .false.
   end if
!   write(iulog,*)'mao3 = ', do_harvest, do_harvest_new
   ...
   do fp = 1,num_soilp
      ...
      if (ivt(p) > noveg .and. ivt(p) < nbrdlf_evr_shrub) then
         ...
         if (do_harvest_new) then
            am = harvest(g)

            ! let all the harvest of the year happen only at this time step
            m  = am/dtime
         else
            m = 0._r8
         end if   
