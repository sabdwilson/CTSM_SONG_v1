...
! !USES:
  ...
  !! add Balance check for P -XYANG
  use CNBalanceCheckMod   , only : BeginCBalance, BeginNBalance,BeginPBalance, &
                                   CBalanceCheck, NBalanceCheck,PBalanceCheck
...
!$OMP PARALLEL DO PRIVATE (nc,g,begg,endg,begl,endl,begc,endc,begp,endp)
  do nc = 1,nclumps
     ...
     call BeginPBalance(begc, endc, filter(nc)%num_soilc, filter(nc)%soilc)
...
!$OMP PARALLEL DO PRIVATE (nc,l,c,begg,endg,begl,endl,begc,endc,begp,endp)
  do nc = 1,nclumps
     ...
     if (nstep > 2) then
        ...
        call PBalanceCheck(begc, endc, filter(nc)%num_soilc, filter(nc)%soilc)
