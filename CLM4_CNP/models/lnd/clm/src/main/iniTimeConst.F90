...
! !USES:
  ...
  use pftvarcon   , only : noveg, ntree, roota_par, rootb_par,  &
                           ...
                           leafcp,lflitcp,frootcp,livewdcp,deadwdcp,graincp                           
...
!
  !! soil order
  integer , pointer :: isoiorder(:)         ! soil order class
!
...
!EOP
  ...
  !!  add varibales for reading in soil order for each grod cell : X.YANG
  integer,pointer :: soilorder_rdin(:)  ! read in - soil order
...
!------------------------------------------------------------------------
  ...
  !! X.YANG
  allocate( soilorder_rdin(begg:endg) ) 
  ...
  !! add reading in soil order data
  call ncd_io(ncid=ncid, varname='SOIL_ORDER', flag='read', data=soilorder_rdin, dim1name=grlnd, readvar=readvar)
  if (.not. readvar) call endrun( trim(subname)//' ERROR: SOIL ORDER NOT on surfdata file' ) 
  ...
  ! --------------------------------------------------------------------
   ...
   do m = 0,numpft
      ...
      !!! XYANG: add P 
      pftcon%leafcp(m) = leafcp(m)
      pftcon%lflitcp(m) = lflitcp(m)
      pftcon%frootcp(m) = frootcp(m)
      pftcon%livewdcp(m) = livewdcp(m)
      pftcon%deadwdcp(m) = deadwdcp(m)
      pftcon%graincp(m) = graincp(m)
