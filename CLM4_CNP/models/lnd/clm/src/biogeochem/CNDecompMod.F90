...
! !USES:
   ...
   !DMR 7/5/11 - add model parameters
   use pftvarcon       , only: cn_s1_in, cn_s2_in, cn_s3_in, cn_s4_in, &
	rf_l1s1_in, rf_l2s2_in, rf_l3s3_in, rf_s1s2_in, rf_s2s3_in, rf_s3s4_in, &
	cwd_fcel_in, k_l1_in, k_l2_in, k_l3_in, k_s1_in, k_s2_in, k_s3_in, k_s4_in, &
	k_frag_in, q10_hr, minpsi_hr, dnp,cp_s1_new_in,np_s1_new_in,np_s2_new_in,&
        np_s3_new_in,np_s4_new_in
...
!! add phosphorus
   ...
   real(r8), pointer :: potential_immob_p(:)
...
! !OTHER LOCAL VARIABLES:
   ...
   !real(r8):: dnp          !denitrification proportion  !DMR 7/5/11 - now a parameter
   integer :: nlevdecomp ! bottom layer to consider for decomp controls
   real(r8):: spinup_scalar         !multiplier for AD_SPINUP algorithm

!! add phosphorus
   real(r8):: cp_l1(lbc:ubc)        !C:P for litter 1
   real(r8):: cp_l2(lbc:ubc)        !C:P for litter 2
   real(r8):: cp_l3(lbc:ubc)        !C:P for litter 3
   real(r8):: cp_s1_new        !C:P for newly formed SOM 1
   real(r8):: cp_s2_new        !C:P for newly formed SOM 2
   real(r8):: cp_s3_new        !C:P for newly formed SOM 3
   real(r8):: cp_s4_new        !C:P for newly formed SOM 4
   real(r8):: np_s1_new        !N:P for newly formed SOM 1
   real(r8):: np_s2_new        !N:P for newly formed SOM 2
   real(r8):: np_s3_new        !N:P for newly formed SOM 3
   real(r8):: np_s4_new        !N:P for newly formed SOM 4
   real(r8):: cp_s1(lbc:ubc)        !C:P for SOM 1
   real(r8):: cp_s2(lbc:ubc)        !C:P for SOM 2
   real(r8):: cp_s3(lbc:ubc)        !C:P for SOM 3
   real(r8):: cp_s4(lbc:ubc)        !C:P for SOM 4
   real(r8):: cwdp_loss    !fragmentation rate for CWD nitrogen (gN/m2/s)
   real(r8):: pmpf_l1s1(lbc:ubc)    !potential mineral N flux, litter 1 -> SOM 1
   real(r8):: pmpf_l2s2(lbc:ubc)    !potential mineral N flux, litter 2 -> SOM 2
   real(r8):: pmpf_l3s3(lbc:ubc)    !potential mineral N flux, litter 3 -> SOM 3
   real(r8):: pmpf_s1s2(lbc:ubc)    !potential mineral N flux, SOM 1 -> SOM 2
   real(r8):: pmpf_s2s3(lbc:ubc)    !potential mineral N flux, SOM 2 -> SOM 3
   real(r8):: pmpf_s3s4(lbc:ubc)    !potential mineral N flux, SOM 3 -> SOM 4
   real(r8):: pmpf_s4(lbc:ubc)      !potential mineral N flux, SOM 4
   real(r8):: immob_p(lbc:ubc)        !potential N immobilization
...
!! add phosphorus
   ...
   potential_immob_p       => clm3%g%l%c%cphof%potential_immob_p
   ...
   gross_pmin            => clm3%g%l%c%cphof%gross_pmin
   net_pmin              => clm3%g%l%c%cphof%net_pmin
   ...
   !cn_s3 = 10.0_r8
   !cn_s4 = 10.0_r8
   cn_s1 = cn_s1_in           !DMR 7/5/11
   cn_s2 = cn_s2_in
   cn_s3 = cn_s3_in
   cn_s4 = cn_s4_in

!! add phosphorus(C:P ratios)

   !! define the CP ratio of newly formed soil organic matter 
    
   np_s1_new = np_s1_new_in
   np_s2_new = np_s2_new_in
   np_s3_new = np_s3_new_in
   np_s4_new = np_s4_new_in

   cp_s1_new = 12.0_r8*np_s1_new
   cp_s2_new = 12.0_r8*np_s2_new
   cp_s3_new = 10.0_r8*np_s3_new
   cp_s4_new = 10.0_r8*np_s4_new
   ...
   !cwd_fcel = 0.76_r8
   !cwd_flig = 0.24_r8
   cwd_fcel = cwd_fcel_in
   cwd_flig = 1.0 - cwd_fcel    !DMR 7/5/11
   ...
   !DMR 7/5/11 - get these parameters from pft-physiology
   k_l1 = k_l1_in    
   k_l2 = k_l2_in 
   k_l3 = k_l3_in
   k_s1 = k_s1_in
...
!   k_s4 = k_s4 * 70._r8 !spinup_scalar
   ...
   ! DMR 7/5/11 - q10 is now a model parameter (q10_hr)
   t_scalar(:) = 0._r8
   do j = 1,nlevdecomp
      do fc = 1,num_soilc
         c = filter_soilc(fc)
         t_scalar(c)=t_scalar(c) + (q10_hr**((t_soisno(c,j)-(SHR_CONST_TKFRZ+25._r8))/10._r8))*fr(c,j)
      end do
   end do
...
!! add phosphorus(set initial values for potential mineral P fluxes)
   pmpf_l1s1(:) = 0._r8
   pmpf_l2s2(:) = 0._r8
   pmpf_l3s3(:) = 0._r8
   pmpf_s1s2(:) = 0._r8
...
!!    add phosphorus  ! calculate litter compartment C:P ratios

      !!! initialize soil CP ratio as new soil ratio temperory
      cp_s1(c)=cp_s1_new
      cp_s2(c)=cp_s2_new
      cp_s3(c)=cp_s3_new
      cp_s4(c)=cp_s4_new

      if (litr1p(c) > 0._r8) cp_l1(c) = litr1c(c)/litr1p(c)
      if (litr2p(c) > 0._r8) cp_l2(c) = litr2c(c)/litr2p(c)
      if (litr3p(c) > 0._r8) cp_l3(c) = litr3c(c)/litr3p(c)
      if (soil1p(c) > 0._r8) cp_s1(c) = soil1c(c)/soil1p(c)
      if (soil2p(c) > 0._r8) cp_s2(c) = soil2c(c)/soil2p(c)
      if (soil3p(c) > 0._r8) cp_s3(c) = soil3c(c)/soil3p(c)
      if (soil4p(c) > 0._r8) cp_s4(c) = soil4c(c)/soil4p(c)

      !!! test if som1-som2,som2-som3,som3-som4 
!      cp_s2_new=cp_s1(c)
!      cp_s4_new=cp_s3(c)
      ...
      !!LITTERBAG EXPERIMENT : MODIFY DECOMPSOTION RATE, TESTING
!      ck_l1 = ck_l1 *0.3_r8
!      ck_l2 = ck_l2 *0.3_r8
!      ck_l2 = ck_l2 *0.3_r8
!      ck_s1 = ck_s1 *0.3_r8
...
!!    add phosphorus
      cwdp_loss = cwdp(c) * ck_frag / dt
      cwdp_to_litr2p(c) = cwdp_loss * cwd_fcel
      cwdp_to_litr3p(c) = cwdp_loss * cwd_flig
...
!!       add phosphorus
         ratio = 0._r8
         if (litr1p(c) > 0._r8) ratio = cp_s1_new/cp_l1(c)
         pmpf_l1s1(c) = (plitr1c_loss(c) * (1.0_r8 - rf_l1s1 - ratio))/cp_s1_new
...
!!       add phosphorus
         ratio = 0._r8
         if (litr2p(c) > 0._r8) ratio = cp_s2_new/cp_l2(c)
         pmpf_l2s2(c) = (plitr2c_loss(c) * (1.0_r8 - rf_l2s2 - ratio))/cp_s2_new
...
!!       add phosphorus
         ratio = 0._r8
         if (litr3p(c) > 0._r8) ratio = cp_s3_new/cp_l3(c)
         pmpf_l3s3(c) = (plitr3c_loss(c) * (1.0_r8 - rf_l3s3 - ratio))/cp_s3_new
...
!!       add phosphorus
         ratio = 0._r8
         if (soil1p(c) > 0._r8) ratio = cp_s2_new/cp_s1(c)
         pmpf_s1s2(c) = (psoil1c_loss(c) * (1.0_r8 - rf_s1s2 - ratio))/cp_s2_new
      end if
...
!!       add phosphorus
         ratio = 0._r8
         if (soil2p(c) > 0._r8) ratio = cp_s3_new/cp_s2(c)
         pmpf_s2s3(c) = (psoil2c_loss(c) * (1.0_r8 - rf_s2s3 - ratio))/cp_s3_new
      end if
...
!!       add phosphorus
         ratio = 0._r8
         if (soil3p(c) > 0._r8) ratio = cp_s4_new/cp_s3(c)
         pmpf_s3s4(c) = (psoil3c_loss(c) * (1.0_r8 - rf_s3s4 - ratio))/cp_s4_new
      end if
...
!!       add phosphorus
         if(soil4p(c) > 0._r8) pmpf_s4(c) = -psoil4c_loss(c)/cp_s4(c)
      end if
...
!!       add phosphorus
      if (pmpf_l1s1(c) > 0._r8) then
         immob_p(c) = immob_p(c) + pmpf_l1s1(c)
      else
         gross_pmin(c) = gross_pmin(c) - pmpf_l1s1(c)
      end if
...
!!       add phosphorus
      if (pmpf_l2s2(c) > 0._r8) then
         immob_p(c) = immob_p(c) + pmpf_l2s2(c)
      else
         gross_pmin(c) = gross_pmin(c) - pmpf_l2s2(c)
      end if
...
!!       add phosphorus
      if (pmpf_l3s3(c) > 0._r8) then
         immob_p(c) = immob_p(c) + pmpf_l3s3(c)
      else
         gross_pmin(c) = gross_pmin(c) - pmpf_l3s3(c)
      end if
...
!!       add phosphorus
      if (pmpf_s1s2(c) > 0._r8) then
         immob_p(c) = immob_p(c) + pmpf_s1s2(c)
      else
         gross_pmin(c) = gross_pmin(c) - pmpf_s1s2(c)
      end if
...
!!       add phosphorus
      if (pmpf_s2s3(c) > 0._r8) then
         immob_p(c) = immob_p(c) + pmpf_s2s3(c)
      else
         gross_pmin(c) = gross_pmin(c) - pmpf_s2s3(c)
      end if
...
!!       add phosphorus
      if (pmpf_s3s4(c) > 0._r8) then
         immob_p(c) = immob_p(c) + pmpf_s3s4(c)
      else
         gross_pmin(c) = gross_pmin(c) - pmpf_s3s4(c)
      end if

      ! SOM 4
      gross_nmin(c) = gross_nmin(c) - pmnf_s4(c)
!!       add phosphorus
      gross_pmin(c) = gross_pmin(c) - pmpf_s4(c)


      potential_immob(c) = immob(c)
      potential_immob_p(c) = immob_p(c)
   ...
   do fc = 1,num_soilc
      ...
      if (litr1c(c) > 0._r8) then
         if (pmnf_l1s1(c) > 0._r8 .and. pmpf_l1s1(c) > 0._r8 ) then
            plitr1c_loss(c) = plitr1c_loss(c) * min(fpi(c),fpi_p(c))
            pmnf_l1s1(c) = pmnf_l1s1(c) * min(fpi(c),fpi_p(c))
            pmpf_l1s1(c) = pmpf_l1s1(c) * min(fpi(c),fpi_p(c))

            sminn_to_denit_l1s1(c) = 0._r8
         elseif(pmnf_l1s1(c) > 0._r8 .and. pmpf_l1s1(c) <= 0._r8)then
            plitr1c_loss(c) = plitr1c_loss(c) * fpi(c)
            pmnf_l1s1(c) = pmnf_l1s1(c) * fpi(c)
            pmpf_l1s1(c) = pmpf_l1s1(c) * fpi(c)

            sminn_to_denit_l1s1(c) = 0._r8
         elseif(pmnf_l1s1(c) < 0._r8 .and. pmpf_l1s1(c) > 0._r8)then
            plitr1c_loss(c) = plitr1c_loss(c) * fpi_p(c)
            pmpf_l1s1(c) = pmpf_l1s1(c) * fpi_p(c)
            pmnf_l1s1(c) = pmnf_l1s1(c) * fpi_p(c)
...
!!       add phosphorus         
         if (litr1p(c) > 0._r8) litr1p_to_soil1p(c) = plitr1c_loss(c) / cp_l1(c)
         sminp_to_soil1p_l1(c) = pmpf_l1s1(c)
         net_pmin(c) = net_pmin(c) - pmpf_l1s1(c)
      ...
      if (litr2c(c) > 0._r8) then
         ...
         if (pmnf_l2s2(c) > 0._r8 .and. pmpf_l2s2(c) > 0._r8 ) then
            plitr2c_loss(c) = plitr2c_loss(c) * min(fpi(c),fpi_p(c))
            pmnf_l2s2(c) = pmnf_l2s2(c) * min(fpi(c),fpi_p(c))
            pmpf_l2s2(c) = pmpf_l2s2(c) * min(fpi(c),fpi_p(c))

            sminn_to_denit_l2s2(c) = 0._r8
         elseif(pmnf_l2s2(c) > 0._r8 .and. pmpf_l2s2(c) <= 0._r8)then
            plitr2c_loss(c) = plitr2c_loss(c) * fpi(c)
            pmnf_l2s2(c) = pmnf_l2s2(c) * fpi(c)
            pmpf_l2s2(c) = pmpf_l2s2(c) * fpi(c)

            sminn_to_denit_l2s2(c) = 0._r8
         elseif(pmnf_l2s2(c) < 0._r8 .and. pmpf_l2s2(c) > 0._r8)then
            plitr2c_loss(c) = plitr2c_loss(c) * fpi_p(c)
            pmpf_l2s2(c) = pmpf_l2s2(c) * fpi_p(c)
            pmnf_l2s2(c) = pmnf_l2s2(c) * fpi_p(c)
...
!!       add phosphorus         
         if (litr2p(c) > 0._r8) litr2p_to_soil2p(c) = plitr2c_loss(c) / cp_l2(c)
         sminp_to_soil2p_l2(c) = pmpf_l2s2(c)
         net_pmin(c) = net_pmin(c) - pmpf_l2s2(c)
      ...
      if (litr3c(c) > 0._r8) then
         ...
         if (pmnf_l3s3(c) > 0._r8 .and. pmpf_l3s3(c) > 0._r8 ) then
            plitr3c_loss(c) = plitr3c_loss(c) * min(fpi(c),fpi_p(c))
            pmnf_l3s3(c) = pmnf_l3s3(c) * min(fpi(c),fpi_p(c))
            pmpf_l3s3(c) = pmpf_l3s3(c) * min(fpi(c),fpi_p(c))

            sminn_to_denit_l3s3(c) = 0._r8
         elseif(pmnf_l3s3(c) > 0._r8 .and. pmpf_l3s3(c) <= 0._r8)then
            plitr3c_loss(c) = plitr3c_loss(c) * fpi(c)
            pmnf_l3s3(c) = pmnf_l3s3(c) * fpi(c)
            pmpf_l3s3(c) = pmpf_l3s3(c) * fpi(c)

            sminn_to_denit_l3s3(c) = 0._r8
         elseif(pmnf_l3s3(c) < 0._r8 .and. pmpf_l3s3(c) > 0._r8)then
            plitr3c_loss(c) = plitr3c_loss(c) * fpi_p(c)
            pmpf_l3s3(c) = pmpf_l3s3(c) * fpi_p(c)
            pmnf_l3s3(c) = pmnf_l3s3(c) * fpi_p(c)
...
!!       add phosphorus         
         if (litr3p(c) > 0._r8) litr3p_to_soil3p(c) = plitr3c_loss(c) / cp_l3(c)
         sminp_to_soil3p_l3(c) = pmpf_l3s3(c)
         net_pmin(c) = net_pmin(c) - pmpf_l3s3(c)

      end if
      
      ! SOM 1 fluxes (fast rate soil organic matter pool)
      if (soil1c(c) > 0._r8) then
         if (pmnf_s1s2(c) > 0._r8 .and. pmpf_s1s2(c) > 0._r8 ) then
            psoil1c_loss(c) = psoil1c_loss(c) *  min(fpi(c),fpi_p(c))
            pmnf_s1s2(c) = pmnf_s1s2(c) * min(fpi(c),fpi_p(c))
            pmpf_s1s2(c) = pmpf_s1s2(c) * min(fpi(c),fpi_p(c))

            sminn_to_denit_s1s2(c) = 0._r8
         elseif(pmnf_s1s2(c) > 0._r8 .and. pmpf_s1s2(c) <= 0._r8)then
            psoil1c_loss(c) = psoil1c_loss(c)  * fpi(c)
            pmnf_s1s2(c) = pmnf_s1s2(c) * fpi(c)
            pmpf_s1s2(c) = pmpf_s1s2(c) * fpi(c)

            sminn_to_denit_s1s2(c) = 0._r8
         elseif(pmnf_s1s2(c) < 0._r8 .and. pmpf_s1s2(c) > 0._r8)then
            psoil1c_loss(c) = psoil1c_loss(c)  * fpi_p(c)
            pmpf_s1s2(c) = pmpf_s1s2(c) * fpi_p(c)
            pmnf_s1s2(c) = pmnf_s1s2(c) * fpi_p(c)
...
!!       add phosphorus
         if (soil1p(c) > 0._r8) soil1p_to_soil2p(c) = psoil1c_loss(c) / cp_s1(c)
         sminp_to_soil2p_s1(c) = pmpf_s1s2(c)
         net_pmin(c) = net_pmin(c) - pmpf_s1s2(c)

      end if

      ! SOM 2 fluxes (medium rate soil organic matter pool)
      if (soil2c(c) > 0._r8) then
         if (pmnf_s2s3(c) > 0._r8 .and. pmpf_s2s3(c) > 0._r8 ) then
            psoil2c_loss(c) = psoil2c_loss(c) *  min(fpi(c),fpi_p(c))
            pmnf_s2s3(c) = pmnf_s2s3(c) * min(fpi(c),fpi_p(c))
            pmpf_s2s3(c) = pmpf_s2s3(c) * min(fpi(c),fpi_p(c))

            sminn_to_denit_s2s3(c) = 0._r8
         elseif(pmnf_s2s3(c) > 0._r8 .and. pmpf_s2s3(c) <= 0._r8)then
            psoil2c_loss(c) = psoil2c_loss(c)  * fpi(c)
            pmnf_s2s3(c) = pmnf_s2s3(c) * fpi(c)
            pmpf_s2s3(c) = pmpf_s2s3(c) * fpi(c)

            sminn_to_denit_s2s3(c) = 0._r8
         elseif(pmnf_s2s3(c) < 0._r8 .and. pmpf_s2s3(c) > 0._r8)then
            psoil2c_loss(c) = psoil2c_loss(c)  * fpi_p(c)
            pmpf_s2s3(c) = pmpf_s2s3(c) * fpi_p(c)
            pmnf_s2s3(c) = pmnf_s2s3(c) * fpi_p(c)
...
!!       add phosphorus
         if (soil2p(c) > 0._r8) soil2p_to_soil3p(c) = psoil2c_loss(c) / cp_s2(c)
         sminp_to_soil3p_s2(c) = pmpf_s2s3(c)
         net_pmin(c) = net_pmin(c) - pmpf_s2s3(c)
      ...
      if (soil3c(c) > 0._r8) then
         if (pmnf_s3s4(c) > 0._r8 .and. pmpf_s3s4(c) > 0._r8 ) then
            psoil3c_loss(c) = psoil3c_loss(c) *  min(fpi(c),fpi_p(c))
            pmnf_s3s4(c) = pmnf_s3s4(c) * min(fpi(c),fpi_p(c))
            pmpf_s3s4(c) = pmpf_s3s4(c) * min(fpi(c),fpi_p(c))

            sminn_to_denit_s3s4(c) = 0._r8
         elseif(pmnf_s3s4(c) > 0._r8 .and. pmpf_s3s4(c) <= 0._r8)then
            psoil3c_loss(c) = psoil3c_loss(c)  * fpi(c)
            pmnf_s3s4(c) = pmnf_s3s4(c) * fpi(c)
            pmpf_s3s4(c) = pmpf_s3s4(c) * fpi(c)

            sminn_to_denit_s3s4(c) = 0._r8
         elseif(pmnf_s3s4(c) < 0._r8 .and. pmpf_s3s4(c) > 0._r8)then
            psoil3c_loss(c) = psoil3c_loss(c)  * fpi_p(c)
            pmpf_s3s4(c) = pmpf_s3s4(c) * fpi_p(c)
            pmnf_s3s4(c) = pmnf_s3s4(c) * fpi_p(c)
...
!!       add phosphorus
         if (soil3p(c) > 0._r8) soil3p_to_soil4p(c) = psoil3c_loss(c) / cp_s3(c)
         sminp_to_soil4p_s3(c) = pmpf_s3s4(c)
         net_pmin(c) = net_pmin(c) - pmpf_s3s4(c)
...
!!       add phosphorus
         if (soil4p(c) > 0._r8) soil4p_to_sminp(c) = psoil4c_loss(c) / cp_s4(c)
         net_pmin(c) = net_pmin(c) - pmpf_s4(c)
