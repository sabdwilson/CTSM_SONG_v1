...
! ! Add phosphorus- X.YANG OCT24, 2012 
   character(len=*), parameter, public :: suplpAll=& ! Supplemental Nitrogen for all PFT's
                     'ALL'
   character(len=*), parameter, public :: suplpCrp=& ! Supplemental Nitrogen for prognostic Crop
                     'PROG_CROP_ONLY'
   character(len=*), parameter, public :: suplpNon=& ! No supplemental Nitrogen
                     'NONE'
   character(len=15), public :: suplphos = suplpNon ! Supplemental Nitrogen mode
! !PRIVATE DATA MEMBERS:
   ...
   real(r8), pointer:: col_plant_pdemand(:) !column-level plant P demand
   logical :: Carbon_only = .false.         ! Carbon only mode 
                                            ! (Nitrogen is prescribed NOT prognostic)
   logical :: CarbonNitrogen_only = .false.         ! CarbonNitrogen only mode 
                                            ! (Phosphorus is prescribed NOT prognostic)
   logical :: CarbonPhosphorus_only = .false.         ! CarbonPhosphorus only mode 
                                            ! (Nitrogen is prescribed NOT prognostic)
   logical :: crop_supln  = .false.         ! Prognostic crop receives supplemental Nitrogen
   logical :: crop_suplp  = .false.         ! Prognostic crop receives supplemental Nitrogen
!
! !REVISION HISTORY:
...
!-----------------------------------------------------------------------
   ...
   !! add phosphorus 
   allocate(col_plant_pdemand(lbc:ubc))
   col_plant_pdemand(:) = nan
...
!! Assuming Phophorus is not limiting during adspinup and exit_spinup --XYANG  OCT 24, 2012

#if (defined AD_SPINUP || defined EXIT_SPINUP)
    suplphos = suplpAll
#endif
   ...
   select case(suplnitro)
      ...
      case(suplnCrp)      !!! modification needed for crop case -- XYANG 
        select case (suplphos)
          case(suplpCrp)
             Carbon_only = .false.
             CarbonNitrogen_only=.false.
...
! !USES:
   ...
   use pftvarcon , only: npcropmin, declfact, bfact, aleaff, arootf, astemf, &
			 ...
			 bdnr_d, rc_npool, dayscrecover,frac_solp_in
...
!
   ...
   !!
   real(r8), pointer :: retransp(:)   ! (kgP/m2) plant pool of retranslocated P
   real(r8), pointer :: ppool(:)      ! (kgP/m2) plant pool of allocatable P
   
   ! column level
   real(r8), pointer :: sminn(:)      ! (kgN/m2) soil mineral N
   real(r8), pointer :: solutionp(:)      ! (kgP/m2) soil solution mineral P
   real(r8), pointer :: labilep(:)      ! (kgP/m2) soil labile mineral P
   ...
   !!
   real(r8), pointer :: graincp(:)    ! grain C:N (gC/gP)
!
...
!
   ...
   !!   phosphorus 
   real(r8), pointer :: p_allometry(:)           ! P allocation index (DIM)
   real(r8), pointer :: plant_pdemand(:)         ! N flux required to support initial GPP (gN/m2/s)
   real(r8), pointer :: tempmax_retransp(:)      ! temporary annual max of retranslocated P pool (gP/m2)
   real(r8), pointer :: avail_retransp(:)        ! N flux available from retranslocation pool (gN/m2/s)
   real(r8), pointer :: annmax_retransp(:)       ! annual max of retranslocated N pool
   ...
   real(r8), pointer :: ppool_to_leafp(:)                 ! allocation to leaf N (gN/m2/s)
   real(r8), pointer :: ppool_to_leafp_storage(:)         ! allocation to leaf N storage (gN/m2/s)
   real(r8), pointer :: ppool_to_frootp(:)                ! allocation to fine root N (gN/m2/s)
   real(r8), pointer :: ppool_to_frootp_storage(:)        ! allocation to fine root N storage (gN/m2/s)
   real(r8), pointer :: ppool_to_livestemp(:)
   real(r8), pointer :: ppool_to_livestemp_storage(:)
   ...
   real(r8), pointer :: fpi_p(:)                          ! fraction of potential immobilization (no units)
   real(r8), pointer :: fpg_p(:)                          ! fraction of potential gpp (no units)
   real(r8), pointer :: potential_immob_p(:)
   real(r8), pointer :: actual_immob_p(:)
...
! !OTHER LOCAL VARIABLES:
   ...
   !! add phosphorus :Xiaojuan Yang 
   integer :: plimit               !flag for N limitation
   real(r8):: cpl,cpfr,cplw,cpdw   !C:N ratios for leaf, fine root, and wood
   real(r8):: sum_pdemand          !total column N demand (gN/m2/s)
   real(r8):: cpg                  !C:N ratio for grain (= cnlw for now; slevis)
   real(r8):: frac_solp            !fraction of labile p as solution P(Parameter, can be changed depending on soil properties
...
!-----------------------------------------------------------------------
   ...
   leafcp                      => pftcon%leafcp
   frootcp                     => pftcon%frootcp
   livewdcp                    => pftcon%livewdcp
   deadwdcp                    => pftcon%deadwdcp
   graincp                     => pftcon%graincp
   ...
   !! add phosphorus
   p_allometry                 => clm3%g%l%c%p%pepv%p_allometry
   plant_pdemand               => clm3%g%l%c%p%pepv%plant_pdemand
   tempmax_retransp            => clm3%g%l%c%p%pepv%tempmax_retransp
   avail_retransp              => clm3%g%l%c%p%pepv%avail_retransp
   annmax_retransp             => clm3%g%l%c%p%pepv%annmax_retransp
   plant_palloc                => clm3%g%l%c%p%pepv%plant_palloc
   ...
   fpg_p                       => clm3%g%l%c%cps%fpg_p
   potential_immob_p           => clm3%g%l%c%cphof%potential_immob_p
   actual_immob_p              => clm3%g%l%c%cphof%actual_immob_p
   ...
   do fp=1,num_soilp
      ...
      ! determine P requirements: 11/06/2011, Xiaojuan added phosphorus  
      if (woody(ivt(p)) == 1.0_r8) then
...
!         print*,p,"cpl=",cpl,"f1=",f1,"cpfr=",cpfr,"f3=",f3,"f4=",f4,"f2=",f2,"cplw=",cplw,"cpdw=",cpdw
...
!         print*,p,"cpl=",cpl,"f1=",f1,"cpfr=",cpfr
      end if
      plant_ndemand(p) = availc(p)*(n_allometry(p)/c_allometry(p))
      plant_pdemand(p) = availc(p)*(p_allometry(p)/c_allometry(p))

!      print*,P,plant_pdemand(p),'plant P demand at pft level', p_allometry(p)
...
!      print*,tempmax_retransp(p),retransp(p),'retransp(p)'
...
!     print*,annmax_retransp(p),avail_retransp(p),'avail_retransp(p)'
      if (annsum_potential_gpp(p) > 0.0_r8) then
         avail_retransn(p) = (annmax_retransn(p)/2.0)*(gpp(p)/annsum_potential_gpp(p))/dt
         avail_retransp(p) = (annmax_retransp(p)/2.0)*(gpp(p)/annsum_potential_gpp(p))/dt
      else
...
!      print*,avail_retransp(p),'avail_retransp(p)'
...
!!    06/25/2012: Modification of NPOOL  - X.YANG  
!      retransn_to_npool(p)=retransn(p)/dt
!      retransp_to_ppool(p)=retransp(p)/dt    !! phosphorus

!      print*,retransp(p),'retransp(p)'
   ...
   call p2c(num_soilc,filter_soilc,plant_pdemand,col_plant_pdemand)   !! phosphorus
   ...
   do fc=1,num_soilc
      ...
      sum_pdemand = col_plant_pdemand(c) + potential_immob_p(c)
      ...
      else if ( ((.not. Carbon_only.and.(.not. CarbonPhosphorus_only)) .and.  (.not. crop_supln)) .or. &
      ...
      else if ( Carbon_only .or. CarbonPhosphorus_only .or. &
...
!     !! for phosphorus
      ...
      !! assign frac_solp 
      frac_solp = frac_solp_in
      if (sum_pdemand*dt < solutionp(c)) then
         ! P availability is not limiting immobilization of plant
         ! uptake, and both can proceed at their potential rates

         plimit = 0
         fpi_p(c) = 1.0_r8
         actual_immob_p(c) = potential_immob_p(c)
         sminp_to_plant(c) = col_plant_pdemand(c)

      else if ( ((.not. Carbon_only.and.(.not. CarbonNitrogen_only)) .and.  (.not. crop_supln)) .or. &
                (crop_supln .and. ( (itypelun(l) /= istcrop)  .or. &
                ((itypelun(l) == istcrop) .and. (ivt(pfti(c)) < npcropmin) )) ) )then

         ! P availability can not satisfy the sum of immobilization and
         ! plant growth demands, so these two demands compete for available
         ! soil labile resource.

         plimit = 1
         if (sum_pdemand > 0.0_r8) then
            actual_immob_p(c) = (solutionp(c)/dt)*(potential_immob_p(c) / sum_pdemand)
         else
            actual_immob_p(c) = 0.0_r8
         end if
         ...
         sminp_to_plant(c) = (solutionp(c)/dt) - actual_immob_p(c)
      else if ( Carbon_only .or. CarbonNitrogen_only .or.&
               (crop_supln .and. (itypelun(l) == istcrop) .and. &
                (ivt(pfti(c)) >= npcropmin)) )then
         ! this code block controls the addition of N to sminn pool
         ! to eliminate any N limitation, when Carbon_Only is set.  This lets the
         ...
         actual_immob_p(c) = potential_immob_p(c)
         sminp_to_plant(c) = col_plant_pdemand(c)
         supplement_to_sminp(c) = sum_pdemand - (solutionp(c)/dt)
      else
         call endrun( 'This else should NOT be able to happen' )
      end if


      if(nlimit == 1.and.plimit == 0)then
        actual_immob_p(c)=potential_immob_p(c) *fpi(c)
        sminp_to_plant(c) = col_plant_pdemand(c)
      elseif(nlimit == 0.and.plimit == 1)then 
        actual_immob(c)=potential_immob(c) *fpi_p(c)
        sminn_to_plant(c) = col_plant_ndemand(c)
      elseif(nlimit == 1.and.plimit == 1)then 
        if( fpi(c) <=fpi_p(c) )then
            actual_immob_p(c)=potential_immob_p(c) *fpi(c)
            sminp_to_plant(c) = min( (solutionp(c)/dt) - actual_immob_p(c),col_plant_pdemand(c) )
        else
            actual_immob(c)=potential_immob(c) *fpi_p(c)
            sminn_to_plant(c) = min( (sminn(c)/dt) - actual_immob(c),col_plant_ndemand(c) )
        endif
       endif
...
!      !! testing N balance without P dynamics
!         fpg_p(c) = 1.0_r8
!         fpi_p(c) = 1.0_r8
...
!      rc = 0.2       !!!MAGICAL NUMBER for P, need further thoughts here
!      r  = max(1._r8,rc/max(ppool(p),1e-9_r8))
!      plant_palloc(p) = ( plant_pdemand(p)+ retransp_to_ppool(p) )/r    !! phosphorus,NEED MODIFICATION
...
!      plant_calloc(p) = min( plant_nalloc(p) * (c_allometry(p)/n_allometry(p)),&
!                             plant_palloc(p) * (c_allometry(p)/p_allometry(p)) )
   ...
   call p2c(num_soilc,filter_soilc,sminn_to_npool,sminn_to_plant)
   call p2c(num_soilc,filter_soilc,sminp_to_ppool,sminp_to_plant)   !! phosphorus
