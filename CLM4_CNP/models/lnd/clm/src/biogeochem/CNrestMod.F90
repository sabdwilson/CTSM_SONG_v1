...
! !USES:
    ...
    use ncdio_pio
! add solution P related parameters to initialize solution pool based on labile pool
! Xiaojuan Yang - Nov., 01,2012
...
!EOP
    ...
    integer,pointer :: isoiorder(:)   ! pointer to column level soil order classes
...
!!  for phosphorus
    ! p_allometry
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='p_allometry', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='p_allometry', data=pptr%pepv%p_allometry, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar) 
       if (flag=='read' .and. .not. readvar) then
  	  if (is_restart()) call endrun
       end if	
    ...
    ! plant_pdemand
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='plant_pdemand', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='plant_pdemand', data=pptr%pepv%plant_pdemand, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar) 
       if (flag=='read' .and. .not. readvar) then
  	  if (is_restart()) call endrun
       end if	
...
!!  for phosphorus
    ! tempmax_retransp
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='tempmax_retransp', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='tempmax_retransp', data=pptr%pepv%tempmax_retransp, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar) 
       if (flag=='read' .and. .not. readvar) then
  	  if (is_restart()) call endrun
       end if	
    ...
    ! annmax_retransp
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='annmax_retransp', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='annmax_retransp', data=pptr%pepv%annmax_retransp, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar) 
       if (flag=='read' .and. .not. readvar) then
  	  if (is_restart()) call endrun
       end if	
    ...
    ! avail_retransp
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='avail_retransp', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='avail_retransp', data=pptr%pepv%avail_retransp, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar) 
       if (flag=='read' .and. .not. readvar) then
        if (is_restart()) call endrun
       end if 
    ...
    ! plant_palloc
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='plant_palloc', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='plant_palloc', data=pptr%pepv%plant_palloc, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar) 
       if (flag=='read' .and. .not. readvar) then
  	  if (is_restart()) call endrun
       end if	
    ...
    !--------------------------------
    ! pft phosphours state variables
    !--------------------------------
    
    ! leafp
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='leafp', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='leafp', data=pptr%pphos%leafp, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! leafp_storage 
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='leafp_storage', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='leafp_storage', data=pptr%pphos%leafp_storage, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! leafp_xfer
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='leafp_xfer', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='leafp_xfer', data=pptr%pphos%leafp_xfer, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! frootp
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='frootp', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='frootp', data=pptr%pphos%frootp, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! frootp_storage
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='frootp_storage', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='frootp_storage', data=pptr%pphos%frootp_storage, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! frootp_xfer
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='frootp_xfer', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='frootp_xfer', data=pptr%pphos%frootp_xfer, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! livestemp
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='livestemp', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='livestemp', data=pptr%pphos%livestemp, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! livestemp_storage
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='livestemp_storage', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='livestemp_storage', data=pptr%pphos%livestemp_storage, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! livestemp_xfer
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='livestemp_xfer', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='livestemp_xfer', data=pptr%pphos%livestemp_xfer, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! deadstemp
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='deadstemp', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='deadstemp', data=pptr%pphos%deadstemp, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    !deadstemp_storage 
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='deadstemp_storage', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='deadstemp_storage', data=pptr%pphos%deadstemp_storage, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    !deadstemp_xfer 
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='deadstemp_xfer', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='deadstemp_xfer', data=pptr%pphos%deadstemp_xfer, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! livecrootp
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='livecrootp', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='livecrootp', data=pptr%pphos%livecrootp, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! livecrootp_storage
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='livecrootp_storage', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='livecrootp_storage', data=pptr%pphos%livecrootp_storage, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    !livecrootp_xfer 
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='livecrootp_xfer', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='livecrootp_xfer', data=pptr%pphos%livecrootp_xfer, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! deadcrootp
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='deadcrootp', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='deadcrootp', data=pptr%pphos%deadcrootp, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! deadcrootp_storage
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='deadcrootp_storage', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='deadcrootp_storage', data=pptr%pphos%deadcrootp_storage, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! deadcrootp_xfer
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='deadcrootp_xfer', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='deadcrootp_xfer', data=pptr%pphos%deadcrootp_xfer, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    !retransp 
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='retransp', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='retransp', data=pptr%pphos%retransp, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! ppool
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='ppool', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='ppool', data=pptr%pphos%ppool, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! pft_ptrunc
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='pft_ptrunc', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='pft_ptrunc', data=pptr%pphos%pft_ptrunc, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! fpi_p
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='fpi_p', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='fpi_p', data=cptr%cps%fpi_p, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar) 
       if (flag=='read' .and. .not. readvar) then
  	  if (is_restart()) call endrun
       end if	
    ...
    ! fpg_p
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='fpg_p', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='fpg_p', data=cptr%cps%fpg_p, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar) 
       if (flag=='read' .and. .not. readvar) then
  	  if (is_restart()) call endrun
       end if	
    ...
    ! cwdp
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='cwdp', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='cwdp', data=cptr%cphos%cwdp, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then 
          if (is_restart()) call endrun
       end if   
    ...
    !litr1p 
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='litr1p', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='litr1p', data=cptr%cphos%litr1p, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! litr2p
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='litr2p', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='litr2p', data=cptr%cphos%litr2p, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! litr3p
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='litr3p', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='litr3p', data=cptr%cphos%litr3p, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! soil1p
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='soil1p', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='soil1p', data=cptr%cphos%soil1p, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! soil2p
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='soil2p', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='soil2p', data=cptr%cphos%soil2p, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! soil3p
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='soil3p', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='soil3p', data=cptr%cphos%soil3p, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! soil4p
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='soil4p', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='soil4p', data=cptr%cphos%soil4p, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! labilep
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='labilep', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='labilep', data=cptr%cphos%labilep, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! solutionp
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='solutionp', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='solutionp', data=cptr%cphos%solutionp, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
...
!!! to initialize solution P based on defined lamuir curve(Ks and smax) and labile P    
!   Xiaojuan Yang - Nov.1, 2012
...
!!!!!!!!!!!!!!!#if (defined EXIT_SPINUP)
    if(flag == 'read')then
     do c = begc, endc
      ...
      cptr%cphos%solutionp(c) = (cptr%cphos%labilep(c)*ks_sorption_c)/&
                             (smax_c-cptr%cphos%labilep(c))
     enddo 
    endif     
!!!!!!!!!!!!!!!#endif
    ...
    ! secondp
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='secondp', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='secondp', data=cptr%cphos%secondp, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! primp
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='primp', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='primp', data=cptr%cphos%primp, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! occlp
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='occlp', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='occlp', data=cptr%cphos%occlp, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! col_ptrunc
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='col_ptrunc', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='col_ptrunc', data=cptr%cphos%col_ptrunc, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! totcolp
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='totcolp', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='totcolp', data=cptr%cphos%totcolp, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! seedp
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='seedp', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='seedp', data=cptr%cphos%seedp, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! prod10p
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='prod10p', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='prod10p', data=cptr%cphos%prod10p, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    ! prod100p
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='prod100p', xtype=ncd_double,  &
            dim1name='column',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='prod100p', data=cptr%cphos%prod100p, &
            dim1name=namec, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun
       end if   
    ...
    if (flag == 'read') then
       ...
       do c = begc, endc
          ...
          clm3%g%l%c%cphos%soil1p(c) = clm3%g%l%c%cphos%soil1p(c) * m
          clm3%g%l%c%cphos%soil2p(c) = clm3%g%l%c%cphos%soil2p(c) * m
          clm3%g%l%c%cphos%soil3p(c) = clm3%g%l%c%cphos%soil3p(c) * m
          clm3%g%l%c%cphos%soil4p(c) = clm3%g%l%c%cphos%soil4p(c) * m
       end do
    end if
#endif
