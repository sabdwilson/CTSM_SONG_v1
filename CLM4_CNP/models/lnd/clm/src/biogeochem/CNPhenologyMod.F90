...
! !USES:
   ...
   use pftvarcon       , only: crit_dayl_in, ndays_on_in, ndays_off_in, &
			       fstor2tran_in, crit_onset_fdd_in, crit_onset_swi_in, &
			       soilpsi_on_in, crit_offset_fdd_in, crit_offset_swi_in, &
			       soilpsi_off_in, lwtop_ann_in, gddfunc_p1_in, &
                               gddfunc_p2_in
! !ARGUMENTS:
   implicit none
...
!------------------------------------------------------------------------
    ...
    !crit_dayl = 39300._r8        !DMR 7/5/11 - now a parameter
    crit_dayl = crit_dayl_in

    ! -----------------------------------------
    ! Constants for CNSeasonDecidPhenology and CNStressDecidPhenology
    ! -----------------------------------------
    !ndays_on  = 30._r8           !DMR 7/5/11 - now a parameter
    !ndays_off = 15._r8           !DMR 7/5/11 - now a parameter
    ndays_on  = ndays_on_in
    ndays_off = ndays_off_in

    ! transfer parameters
    !fstor2tran = 0.5_r8          !DMR 7/5/11 - now a parameter
    fstor2tran = fstor2tran_in
    ! -----------------------------------------
    ! Constants for CNStressDecidPhenology
    ! -----------------------------------------

    ! onset parameters
    !crit_onset_fdd = 15.0_r8     !DMR 7/5/11 - now a parameter
    crit_onset_fdd = crit_onset_fdd_in
    ...
    !crit_onset_swi = 15.0_r8      !DMR 7/5/11 - now a parameter
    !soilpsi_on     = -2.0_r8      !DMR 7/5/11 - now a parameter
     crit_onset_swi = crit_onset_swi_in
     soilpsi_on = soilpsi_on_in

    ! offset parameters
    !crit_offset_fdd = 15.0_r8     !DMR 7/5/11 - now a parameter
    !crit_offset_swi = 15.0_r8     !DMR 7/5/11 - now a parameter
    !soilpsi_off     = -2.0_r8     !DMR 7/5/11 - now a parameter
    crit_offset_fdd = crit_offset_fdd_in
    crit_offset_swi = crit_offset_swi_in
    soilpsi_off     = soilpsi_off_in
    gddfunc_p1      = gddfunc_p1_in
    gddfunc_p2      = gddfunc_p2_in
    ...
    lwtop = lwtop_ann_in / 31536000.0_r8   !DMR 7/5/11 - now a parameter
...
! local pointers to implicit in scalars
   ...
   real(r8), pointer :: leafp_storage(:)      ! (kgP/m2) leaf N storage
   real(r8), pointer :: frootp_storage(:)     ! (kgP/m2) fine root N storage
   real(r8), pointer :: livestemp_storage(:)  ! (kgP/m2) live stem N storage
   real(r8), pointer :: deadstemp_storage(:)  ! (kgP/m2) dead stem N storage
   real(r8), pointer :: livecrootp_storage(:) ! (kgP/m2) live coarse root N storage
...
! local pointers to implicit in/out scalars
   ...
   !!! add for phosphorus - Xiaojuan Yang
...
!-----------------------------------------------------------------------
   ...
   do fp = 1,num_soilp
      ...
      if (season_decid(ivt(p)) == 1._r8) then
         ...
         ! modified by DMR 7/5/11 to change constants into parameters
...
!
   ...
   !!! add phosphorus
   real(r8), pointer :: leafp_storage(:)      ! (kgP/m2) leaf P storage
   real(r8), pointer :: frootp_storage(:)     ! (kgP/m2) fine root P storage
   real(r8), pointer :: livestemp_storage(:)  ! (kgP/m2) live stem P storage
   real(r8), pointer :: deadstemp_storage(:)  ! (kgP/m2) dead stem P storage
   real(r8), pointer :: livecrootp_storage(:) ! (kgP/m2) live coarse root P storage
   real(r8), pointer :: deadcrootp_storage(:) ! (kgP/m2) dead coarse root P storage
...
!
   ...
   real(r8), pointer :: deadcrootn_xfer(:) ! (gN/m2) dead coarse root N transfer
   !!! add phosphorus
   real(r8), pointer :: leafp_xfer(:)      ! (kgP/m2) leaf N transfer
   real(r8), pointer :: frootp_xfer(:)     ! (kgP/m2) fine root N transfer
   real(r8), pointer :: livestemp_xfer(:)  ! (kgP/m2) live stem N transfer
   real(r8), pointer :: deadstemp_xfer(:)  ! (kgP/m2) dead stem N transfer
   real(r8), pointer :: livecrootp_xfer(:) ! (kgP/m2) live coarse root N transfer
   real(r8), pointer :: deadcrootp_xfer(:) ! (kgP/m2) dead coarse root N transfer
...
! local pointers to implicit in scalars
      ...
      real(r8), pointer :: leafcp(:)         ! leaf C:P (gC/gP)     !! add phosphorus
! local pointers to implicit out scalars
...
!!add phosphorus by Xiaojuan Yang
!
! !USES:
!
! !ARGUMENTS:
...
!  integer , pointer :: pcolumn(:)               ! pft's column index
   ...
   real(r8), pointer :: frootcn(:)               ! fine root C:N (gC/gP)
   real(r8), pointer :: livewdcp(:)              ! live wood C:P (gC/gP)
   real(r8), pointer :: graincp(:)               ! grain C:P (gC/gP)
...
!
   ...
   real(r8), pointer :: grainp_to_food(:)        ! grain P to food (gP/m2/s)
!
! local pointers to implicit out scalars
!
!
...
! 10/2/03: Created by Peter Thornton
! 10/24/03, Peter Thornton: migrated to vector data structures
! 01/31/2012, Xiaojuan Yang: add phosphorus 
!
! !LOCAL VARIABLES:
! local pointers to implicit in scalars
!
...
! !USES:
  ...
  use pftvarcon , only : npcropmin
! ! ADDED FOR LITTERBAG EXPERIMENT
...
!
   ...
   !! add local pointers for phosphorus : X.YANG 
   !! implicit in scalars
   real(r8), pointer :: livestemp_to_litter(:) ! livestem N to litter (gN/m2/s)
   real(r8), pointer :: grainp_to_food(:)      ! grain N to food (gN/m2/s)
   real(r8), pointer :: leafp_to_litter(:)     ! leaf N litterfall (gN/m2/s)
   real(r8), pointer :: frootp_to_litter(:)    ! fine root N litterfall (gN/m2/s)
 
   !! implcit in/out scalars
   ...
   real(r8), pointer :: livestemp_to_litr3p(:) ! livestem N litterfall to litter 2 N (gN/m2/s)
...
!
! !OTHER LOCAL VARIABLES:
    integer :: fc,c,pi        ! indices

! !OTHER LOCAL VARIABLES NEEDED FOR LITTERBAG EXPERIMENT:
!  at the first time step, 09/01/11
      ...
      do fc = 1,num_soilc
         ...
         if ( pi <=  npfts(c) ) then
            ...
            if (pwtgcell(p)>0._r8) then
               ...
               leafp_to_litr1p(c) = leafp_to_litr1p(c) + leafp_to_litter(p) * lf_flab(ivt(p)) * wtcol(p)
               leafp_to_litr2p(c) = leafp_to_litr2p(c) + leafp_to_litter(p) * lf_fcel(ivt(p)) * wtcol(p)
               leafp_to_litr3p(c) = leafp_to_litr3p(c) + leafp_to_litter(p) * lf_flig(ivt(p)) * wtcol(p)
               ...
               frootp_to_litr1p(c) = frootp_to_litr1p(c) + frootp_to_litter(p) * fr_flab(ivt(p)) * wtcol(p)
               frootp_to_litr2p(c) = frootp_to_litr2p(c) + frootp_to_litter(p) * fr_fcel(ivt(p)) * wtcol(p)
               frootp_to_litr3p(c) = frootp_to_litr3p(c) + frootp_to_litter(p) * fr_flig(ivt(p)) * wtcol(p)
               ...
               if (ivt(p) >= npcropmin) then ! add livestemc to litter
                  ...
                  livestemp_to_litr1p(c) = livestemp_to_litr1p(c) + livestemp_to_litter(p) * lf_flab(ivt(p)) * wtcol(p)
                  livestemp_to_litr2p(c) = livestemp_to_litr2p(c) + livestemp_to_litter(p) * lf_fcel(ivt(p)) * wtcol(p)
                  livestemp_to_litr3p(c) = livestemp_to_litr3p(c) + livestemp_to_litter(p) * lf_flig(ivt(p)) * wtcol(p)
                  ...
                  grainp_to_litr1p(c) = grainp_to_litr1p(c) + grainp_to_food(p) * lf_flab(ivt(p)) * wtcol(p)
                  grainp_to_litr2p(c) = grainp_to_litr2p(c) + grainp_to_food(p) * lf_fcel(ivt(p)) * wtcol(p)
                  grainp_to_litr3p(c) = grainp_to_litr3p(c) + grainp_to_food(p) * lf_flig(ivt(p)) * wtcol(p)
