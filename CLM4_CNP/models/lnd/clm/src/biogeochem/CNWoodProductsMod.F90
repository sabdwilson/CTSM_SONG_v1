...
!-----------------------------------------------------------------------
   ...
   do fc = 1,num_soilc
      c = filter_soilc(fc)
		...
		cptr%cphof%prod10p_loss(c)    = cptr%cphos%prod10p(c)    * kprod10
		cptr%cphof%prod100p_loss(c)   = cptr%cphos%prod100p(c)   * kprod100
	end do
   ...
   do fc = 1,num_soilc
      ...
      !! add phosphorus
      cptr%cphos%prod10p(c)    = cptr%cphos%prod10p(c)    - cptr%cphof%prod10p_loss(c)*dt
      cptr%cphos%prod100p(c)   = cptr%cphos%prod100p(c)   - cptr%cphof%prod100p_loss(c)*dt
