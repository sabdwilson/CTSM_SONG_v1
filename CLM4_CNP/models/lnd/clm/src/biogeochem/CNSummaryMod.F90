...
! Phosphorus summary calcualtions added
!
! !USES:
...
!
! !INTERFACE:
subroutine PSummary(num_soilc, filter_soilc, num_soilp, filter_soilp)
!
! !DESCRIPTION:
! On the radiation time step, perform pft and column-level nitrogen
! summary calculations
...
!
! !REVISION HISTORY:
! 10/27/2011: Added by Xiaojuan Yang
!
! !LOCAL VARIABLES:
! local pointers to implicit in scalars
   integer , pointer :: ivt(:)            ! pft vegetation type
   real(r8), pointer :: col_fire_ploss(:) ! (gP/m2/s) total column-level fire P loss
   real(r8), pointer :: col_wood_harvestp(:)
   ...
   real(r8), pointer :: col_pft_fire_ploss(:) ! (gP/m2/s) total pft-level fire P loss 
   ...
   real(r8), pointer :: col_totpftp(:)        ! (gP/m2) total pft-level phosphorus
   real(r8), pointer :: col_totvegp(:)            ! (gP/m2) total vegetation phosphorus

   real(r8), pointer :: solutionp(:)              ! (gP/m2) soil labile mineral P
   ...
   real(r8), pointer :: col_ptrunc(:)         ! (gP/m2) column-level sink for P truncation
   real(r8), pointer :: totcolp(:)            ! (gP/m2) total column phosphorus, incl veg
   real(r8), pointer :: totecosysp(:)         ! (gP/m2) total ecosystem phosphorus, incl veg 
   real(r8), pointer :: totlitp(:)            ! (gP/m2) total litter phosphorus
   real(r8), pointer :: totsomp(:)            ! (gP/m2) total soil organic matter phosphorus
   ...
   real(r8), pointer :: pdeploy(:)
   real(r8), pointer :: pft_fire_ploss(:) ! (gP/m2/s) total pft-level fire C loss 
   real(r8), pointer :: retransp_to_ppool(:)          
   real(r8), pointer :: sminp_to_ppool(:)             
   ...
   real(r8), pointer :: retransp(:)           ! (gP/m2) plant pool of retranslocated N
   real(r8), pointer :: ppool(:)              ! (gP/m2) temporary plant N pool
   real(r8), pointer :: pft_ptrunc(:)         ! (gP/m2) pft-level sink for N truncation
   ...
   ! for landcover change
   real(r8), pointer :: wood_harvestp(:)                    ! total N losses to wood product pools (gP/m2/s)
   ...
   real(r8), pointer :: prod10p_loss(:)       ! (gP/m2/s) loss from 10-yr wood product pool
   real(r8), pointer :: prod100p_loss(:)      ! (gP/m2/s) loss from 100-yr wood product pool
   real(r8), pointer :: product_ploss(:)      ! (gP/m2/s) total wood product phosphorus loss
   real(r8), pointer :: prod10p(:)            ! (gP/m2) wood product N pool, 10-year lifespan
   real(r8), pointer :: prod100p(:)           ! (gP/m2) wood product N pool, 100-year lifespan
   real(r8), pointer :: totprodp(:)           ! (gP/m2) total wood product N
!
! local pointers to implicit in/out scalars
!
! local pointers to implicit out scalars
...
!-----------------------------------------------------------------------
    ...
    col_pft_fire_ploss             => clm3%g%l%c%cphof%pphof_a%pft_fire_ploss
    ...
    col_totvegp                    => clm3%g%l%c%cphos%pphos_a%totvegp
    ...
    col_ptrunc                     => clm3%g%l%c%cphos%col_ptrunc
    ...
    totecosysp                     => clm3%g%l%c%cphos%totecosysp
    ...
    pdeploy                        => clm3%g%l%c%p%pphof%pdeploy
    pft_fire_ploss                 => clm3%g%l%c%p%pphof%pft_fire_ploss
    ...
    dispvegp                       => clm3%g%l%c%p%pphos%dispvegp
    ...
    pft_ptrunc                     => clm3%g%l%c%p%pphos%pft_ptrunc
    storvegp                       => clm3%g%l%c%p%pphos%storvegp
    totpftp                        => clm3%g%l%c%p%pphos%totpftp
    totvegp                        => clm3%g%l%c%p%pphos%totvegp
    ! dynamic landcover pointers
    wood_harvestp                  => clm3%g%l%c%p%pphof%wood_harvestp
    col_wood_harvestp              => clm3%g%l%c%cphof%pphof_a%wood_harvestp 
    dwt_ploss                      => clm3%g%l%c%cphof%dwt_ploss
    dwt_conv_pflux                 => clm3%g%l%c%cphof%dwt_conv_pflux
    prod10p_loss                   => clm3%g%l%c%cphof%prod10p_loss
    prod100p_loss                  => clm3%g%l%c%cphof%prod100p_loss
    product_ploss                  => clm3%g%l%c%cphof%product_ploss
    seedp                          => clm3%g%l%c%cphos%seedp
    prod10p                        => clm3%g%l%c%cphos%prod10p
    prod100p                       => clm3%g%l%c%cphos%prod100p
   ...
   do fp = 1,num_soilp
      ...
      ! calculate pft-level summary phosphorus fluxes and states

      ! total N deployment (from sminp and retranslocated N pool) (NDEPLOY)
      pdeploy(p) = &
         sminp_to_ppool(p) + &
         retransp_to_ppool(p)
      ...
      ! pft-level wood harvest
      wood_harvestp(p) = &
         hrv_deadstemp_to_prod10p(p) + &
         hrv_deadstemp_to_prod100p(p)

      ! total pft-level fire N losses
      pft_fire_ploss(p) = &
         m_leafp_to_fire(p)               + &
         m_leafp_storage_to_fire(p)       + &
         m_leafp_xfer_to_fire(p)          + &
         m_frootp_to_fire(p)              + &
         m_frootp_storage_to_fire(p)      + &
         m_frootp_xfer_to_fire(p)         + &
         m_livestemp_to_fire(p)           + &
         m_livestemp_storage_to_fire(p)   + &
         m_livestemp_xfer_to_fire(p)      + &
         m_deadstemp_to_fire(p)           + &
         m_deadstemp_storage_to_fire(p)   + &
         m_deadstemp_xfer_to_fire(p)      + &
         m_livecrootp_to_fire(p)          + &
         m_livecrootp_storage_to_fire(p)  + &
         m_livecrootp_xfer_to_fire(p)     + &
         m_deadcrootp_to_fire(p)          + &
         m_deadcrootp_storage_to_fire(p)  + &
         m_deadcrootp_xfer_to_fire(p)     + &
         m_retransp_to_fire(p)
      ...
      storvegp(p) = &
         ...
         livestemp_storage(p)  + &
         deadstemp_storage(p)  + &
         livecrootp_storage(p) + &
         deadcrootp_storage(p) + &
         leafp_xfer(p)         + &
         frootp_xfer(p)        + &
         livestemp_xfer(p)     + &
         deadstemp_xfer(p)     + &
      ...
      if ( crop_prog .and. ivt(p) >= nc3crop )then
         dispvegp(p) = dispvegp(p) + &
            grainp(p)
          storvegp(p) = storvegp(p) + &
             grainp_storage(p)     + &
             grainp_xfer(p)
      end if

      ! total vegetation phosphorus (TOTVEGN)
      totvegp(p) = dispvegp(p) + storvegp(p)

      ! total pft-level carbon (add pft_ptrunc)
      totpftp(p) = totvegp(p) + pft_ptrunc(p)


   end do  ! end of pfts loop

   ! use p2c routine to get selected column-average pft-level fluxes and states
   call p2c(num_soilc, filter_soilc, pft_fire_ploss, col_pft_fire_ploss)
   call p2c(num_soilc, filter_soilc, wood_harvestp, col_wood_harvestp)
   call p2c(num_soilc, filter_soilc, totvegp, col_totvegp)
   call p2c(num_soilc, filter_soilc, totpftp, col_totpftp)
   ...
   do fc = 1,num_soilc
      ...
      ! total ecosystem phosphorus, including veg (TOTECOSYSN)
      totecosysp(c) = &
         cwdp(c) + &
         totlitp(c) + &
         totsomp(c) + &
      ...
      ! total column phosphorus, including pft (TOTCOLN)
      totcolp(c) = &
         col_totpftp(c) + &
         cwdp(c) + &
         totlitp(c) + &
...
!      print*, " CWDP ", cwdp(c)
!      print*, " TOTAL LIT AND SOIL P", cwdp(c)+totlitp(c)+totsomp(c)
!      print*, " TOTAL SOLUTION AND LABILE P", solutionp(c) + labilep(c)
!      print*, " SECONDARY P",  secondp(c) 
!      print*, " COLUMN PFT P",col_totpftp(c)
!      print*, " COLUMN TRUNCTION P",col_ptrunc(c)
!      print*, " SEED P",seedp(c)
!      print*, " TOTAL COLUMN P", totcolp(c)
   end do ! end of columns loop
