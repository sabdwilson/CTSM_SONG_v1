...
! !OTHER LOCAL VARIABLES:
   ...
   real(r8):: pc,pn,pp    ! truncation terms for pft-level corrections
   ...
   real(r8):: pcrit    ! critical phsophorus state value for truncation
...
!-----------------------------------------------------------------------
   ...
   do fp = 1,num_soilp
      ...
      ! phosphorus is handled the same way       
      ...
      if ( crop_prog .and. ivt(p) >= nc3crop )then
         ...
         if (abs(grainc_storage(p)) < ccrit) then
             ...
             pp = pp + grainp_storage(p)
             grainp_storage(p) = 0._r8
         end if
   ...
   do fc = 1,num_soilc
      ...
      !! Control the values of soil P pools not to be negative, will be modified later(X. YANG)
!      if (soil1p(c) < 0) then
!          cp = cp + soil1p(c)
!          soil1p(c) = 0._r8
!      end if
...
!      end if
      ...
      col_ptrunc(c) = col_ptrunc(c) + cp
      
!      print*,"COLUMN C TRUNCTION ERROR YANGXJ ",col_ctrunc(c),c
!      print*,"COLUMN N TRUNCTION ERROR YANGXJ ",col_ntrunc(c),c
!      print*,"COLUMN P TRUNCTION ERROR YANGXJ ",col_ptrunc(c),c
