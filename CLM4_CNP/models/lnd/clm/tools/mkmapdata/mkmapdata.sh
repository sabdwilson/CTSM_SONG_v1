...
if [ -z "$CSMDATA" ]; then
   CSMDATA=/home/zdr/models/ccsm_inputdata
fi
...
case $hostname in
  ...
  *oic* )
  echo OIC
  ESMFBIN_PATH=/home/zdr/opt/bin
  MPIEXEC="mpirun --hostfile $PBS_NODEFILE -np $REGRID_PROC"
  ;;
...
until ((nfile>${#INGRID[*]})); do
   ...
   cmd="$ESMF_REGRID --ignore_unmapped -s ${INGRID[nfile]} "
   echo $cmd
   cmd="$cmd -d ${GRIDFILE[nfile]} -m conserve -w ${OUTFILE[nfile]}"
   #cmd="$cmd -d ${GRIDFILE[nfile]} -m patch -w ${OUTFILE[nfile]}"
