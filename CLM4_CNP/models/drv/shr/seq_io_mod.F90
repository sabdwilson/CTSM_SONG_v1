!===============================================================================
! SVN $Id: seq_io_mod.F90 34017 2012-01-23 19:42:46Z tcraig $
! SVN $URL: https://svn-ccsm-models.cgd.ucar.edu/drv/seq_mct/trunk_tags/drvseq4_1_01/shr/seq_io_mod.F90 $
!===============================================================================
!BOP ===========================================================================
!
...
!-------------------------------------------------------------------------------
   ...
   integer, parameter :: io_components = num_inst_phys + 1  ! one for each physical component, plus one for the coupler
   integer :: io_compid(io_components)
   character(len=seq_comm_namelen) :: io_compname(io_components)
...
! !INTERFACE: --------------------------------------------------------------------
  ...
  subroutine seq_io_init1(nlfilename, Global_Comm)
    ...
    namelist /pio_inparm/ pio_stride, pio_root, pio_numtasks, pio_typename, &
         pio_async_interface, pio_debug_level, &
    ...
    character(*),parameter :: F02     = "('(seq_io_init) ',a,l3)"

    integer(IN) ::  mpicom, i, inst_index
    include 'mpif.h'
    ...
    if (iamroot) then
       if (debug > 0) then
          ...
          write(logunit,F02) '   pio_async_interface = ',pio_async_interface
       end if
       ...
       call set_io_getiotypefromname(pio_typename, pio_iotype, pio_iotype_netcdf)
    end if

    call seq_io_namelist_set(npes, Global_comm, pio_stride, pio_root, pio_numtasks, pio_iotype)
	...
	call shr_sys_abort(subname//':: async IO requires an MPI2 compliant MPI library')
       ...
       if(iamroot) then
          i = 1  ! running index into the io_components-length vectors
          
          !! Set up information for coupler

          io_compid(i) = CPLID
          io_compname(i) = seq_comm_name(io_compid(i))
          comp_pio_stride(i)   = cpl_pio_stride
          comp_pio_numtasks(i) = cpl_pio_numtasks
          comp_pio_root(i)     = cpl_pio_root
          call set_io_getiotypefromname(cpl_pio_typename, comp_pio_iotype(i), pio_iotype)
          
          i = i + 1

          !! Set up information for atmosphere

          ! set_io_getiotypefromname called outside the loop for efficiency
          call set_io_getiotypefromname(atm_pio_typename, this_pio_iotype, pio_iotype)
          do inst_index = 1, num_inst_atm
             io_compid(i) = ATMID(inst_index)
             io_compname(i) = seq_comm_name(io_compid(i))
             comp_pio_stride(i)   = atm_pio_stride
             comp_pio_numtasks(i) = atm_pio_numtasks
             comp_pio_root(i)     = atm_pio_root
             comp_pio_iotype(i)   = this_pio_iotype

             i = i + 1
          end do

          !! Set up information for land

          call set_io_getiotypefromname(lnd_pio_typename, this_pio_iotype, pio_iotype)
          do inst_index = 1, num_inst_lnd
             io_compid(i) = LNDID(inst_index)
             io_compname(i) = seq_comm_name(io_compid(i))
          ...
          !! Set up information for ocean

          call set_io_getiotypefromname(ocn_pio_typename, this_pio_iotype, pio_iotype)
          do inst_index = 1, num_inst_ocn
             io_compid(i) = OCNID(inst_index)
             io_compname(i) = seq_comm_name(io_compid(i))
          ...
          !! Set up information for ice

          call set_io_getiotypefromname(ice_pio_typename, this_pio_iotype, pio_iotype)
          do inst_index = 1, num_inst_ice
             io_compid(i) = ICEID(inst_index)
             io_compname(i) = seq_comm_name(io_compid(i))
          ...
          !! Set up information for glacier

          call set_io_getiotypefromname(glc_pio_typename, this_pio_iotype, pio_iotype)
          do inst_index = 1, num_inst_glc
             io_compid(i) = GLCID(inst_index)
             io_compname(i) = seq_comm_name(io_compid(i))
          ...
          !! Make sure that we have set all components
          if (i /= (io_components+1)) then
             call shr_sys_abort(subname//':: inconsistency between number of components set'// &
                  ' and io_components')
          end if
          ...
          do i=1,io_components
             ...
             ! ensure consistent case for later string comparisons
             io_compname(i) = shr_string_toUpper(io_compname(i))
          end do
       end if
       do i=1,io_components
          call shr_mpi_bcast(io_compid(i), Global_comm)
          call shr_mpi_bcast(io_compname(i), Global_comm)
    ...
    if(pio_async_interface) then
       ...
       do i=1,io_components
          ...
          if(seq_comm_iamin(io_compid(i))) then
             call seq_io_namelist_set(npes, comms(i), comp_pio_stride(i), comp_pio_root(i), &
...
!===============================================================================
  subroutine seq_io_cpl_init()
    ...
    cpl_io_subsystem=>seq_io_getiosys(cpl_name)
    cpl_pio_iotype = seq_io_getiotype(cpl_name)

  end subroutine seq_io_cpl_init

!===============================================================================
  subroutine set_io_getiotypefromname(typename, iotype, defaulttype)
    use shr_string_mod, only : shr_string_toupper
...
end subroutine set_io_getiotypefromname

!===============================================================================
  subroutine seq_io_namelist_set(npes,mycomm, pio_stride, pio_root, pio_numtasks, pio_iotype)
    integer, intent(in) :: npes, mycomm
    integer, intent(inout) :: pio_stride, pio_root, pio_numtasks
    integer, intent(inout) :: pio_iotype
    character(*),parameter :: subName =   '(seq_io_ seq_io_namelist_set) '
  ...
  end subroutine seq_io_namelist_set
...
!===============================================================================
  function seq_io_getiosys(component) result(iosystem)
    ...
    character(*), parameter :: subName = '(seq_io_getiosys) '

    integer :: i

    i = seq_io_getindex(component)
    if (i == -1) then
       call shr_sys_abort(subname//':: "'//trim(component)//'" not found in io_compname(:)')
    else       
...
!===============================================================================
  function seq_io_getiotype(component) result(io_type)
    ...
    character(*), parameter :: subName = '(seq_io_getiotype) '
...
!===============================================================================
  function seq_io_getioroot(component) result(io_root)
    ...
    character(*), parameter :: subName = '(seq_io_getioroot) '
...
!===============================================================================
  ...
  integer function seq_io_getindex(component)
     use shr_string_mod, only : shr_string_toupper
     ...
     seq_io_getindex = -1  ! flag for not found
     do i=1,io_components
