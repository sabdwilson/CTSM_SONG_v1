module seq_flds_mod
  ...
  !  ---------
  !  1) states: 
  !     a) atm attributes fields that have a state-prefix of Sx_ 
  !        in seq_flds_x2a_states
  ...
  !  
  !     b) atm attribute fields that do not have a state-prefix of Sx_ 
  !         in seq_flds_x2a_states
  ...
  !====================================================================
   ...
   integer, parameter, private :: CSS = 128  ! use longer short character
   integer, parameter, private :: CLL = 512
   integer, parameter, private :: CXX = 1024 ! use local extra-long char
...
! !INPUT/OUTPUT PARAMETERS:
     ...
     character(CXX) :: dom_fields = ' '
     character(CXX) :: a2x_fields = ' '
     character(CXX) :: x2a_fields = ' '
     character(CXX) :: i2x_fields = ' '
     character(CXX) :: x2i_fields = ' '
     character(CXX) :: l2x_fields = ' '
     character(CXX) :: x2l_fields = ' '
     character(CXX) :: o2x_fields = ' '
     character(CXX) :: x2o_fields = ' '
     character(CXX) :: xao_fields = ' '
     character(CXX) :: r2x_fields = ' '
     character(CXX) :: g2x_fields = ' '
     character(CXX) :: x2g_fields = ' '
     character(CXX) :: s2x_fields = ' '
     character(CXX) :: x2s_fields = ' '
     character(CXX) :: stringtmp  = ' '

     !------ namelist -----
     integer,  parameter :: nfldmax = 20
     ...
     namelist /seq_cplflds_inparm/  &
          flds_voc, flds_co2a, flds_co2b, flds_co2c, flds_co2_dmsa, glc_nec, &
          cplflds_custom
...
!-------------------------------------------------------------------------------
     ...
     !---------------------------------------------------------------------------
     ! Read in namelist for use cases - and namelists for new fields
     ...
     call shr_mpi_bcast(flds_voc     , mpicom)
     ...
     do n = 1,nfldmax
        if (cplflds_custom(n) /= ' ') then
           ...
           if (fldname(1:1) == 'S') then
              ...
              call shr_sys_abort("fldname must start with S or F")
           end if
     ...
     call seq_flds_add(dom_other,'ascale')
     longname = ''
     stdname  = 'area scale factor'
     units    = 'unitless'
     attname  = 'ascale'
     call metadata_set(attname, longname, stdname, units)
	...
	!Added by JFM for C13 input, 10/20/11
        call seq_flds_add(a2x_states, "Sa_co2diag")
	call seq_flds_add(a2x_states, "Sa_c13o2diag")
        call seq_flds_add(x2l_states, "Sa_co2diag")
	call seq_flds_add(x2l_states, "Sa_c13o2diag")
	...
	call seq_flds_add(x2o_states, "Sa_c13o2diag")
        longname = 'Diagnostic CO2 at the lowest model level'
        stdname  = ''
        units    = '1e-6 mol/mol'
        attname  = 'Sa_co2diag'
	...
	call seq_flds_add(x2l_states, "Sa_c13o2diag")
     ...
     if (flds_voc) then
        call seq_flds_add(l2x_fluxes,"Fall_flxvoc1")
        call seq_flds_add(x2a_fluxes,"Fall_flxvoc1")
        
        call seq_flds_add(l2x_fluxes,"Fall_flxvoc2")
        call seq_flds_add(x2a_fluxes,"Fall_flxvoc2")
        
        call seq_flds_add(l2x_fluxes,"Fall_flxvoc3")
        call seq_flds_add(x2a_fluxes,"Fall_flxvoc3")
        
        call seq_flds_add(l2x_fluxes,"Fall_flxvoc4")
        call seq_flds_add(x2a_fluxes,"Fall_flxvoc4")
        
        call seq_flds_add(l2x_fluxes,"Fall_flxvoc5")
        call seq_flds_add(x2a_fluxes,"Fall_flxvoc5")
     end if
     ...
     call catFields(seq_flds_r2x_fields,''                  , seq_flds_r2x_fluxes)
