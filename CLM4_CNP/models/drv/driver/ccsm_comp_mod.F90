...
#include <mpif.h>
   ...
   type(mct_gsMap)  :: gsMap_rr(num_inst_lnd)
   ...
   type(AreaCorrectFactor) :: areacor_rr(num_inst_lnd)
   ...
   type(seq_map), SAVE :: mapper_Cr2x(num_inst_lnd)
   type(seq_map), SAVE :: mapper_Cx2r(num_inst_lnd)
...
!  character(CL) :: hist_a2x24hr_flds = 'all'
   ...
   integer :: eai, eli, eoi, eii, egi, exi, efi  ! component instance counters
...
subroutine ccsm_pre_init()
   ...
   if (Global_Comm /= MPI_COMM_WORLD) then
      write(logunit,*) subname,' ERROR in seq_io_init changing Global_Comm'
      call shr_sys_abort(subname//' ERROR in seq_io_init')
   endif
...
subroutine ccsm_init()
 ...
 106  format( A, f23.12)
   ...
   do eli = 1,num_inst_lnd
      ...
      call seq_cdata_init(cdata_rr(eli), LNDID(eli), &
                          dom_rr(eli), gsMap_rr(eli), infodata, &
                          name=('cdata_rr' // trim(lnd_name(eli))))
   ...
   do eoi = 1,num_inst_ocn
      ...
      if (iamin_OCNID(eoi) .and. ocn_present) then
         ...
         call shr_sys_flush(logunit)
         call ocn_init_mct( EClock_o, cdata_oo(eoi), x2o_oo(eoi), o2x_oo(eoi), NLFilename=NLFilename )
         if (drv_threading) call seq_comm_setnthreads(nthreads_GLOID)
      endif
   enddo
   ...
   do eii = 1,num_inst_ice
      ...
      if (iamin_ICEID(eii) .and. ice_present) then
         ...
         call shr_sys_flush(logunit)
         call ice_init_mct( EClock_i, cdata_ii(eii), x2i_ii(eii), i2x_ii(eii), NLFilename=NLFilename )
         if (drv_threading) call seq_comm_setnthreads(nthreads_GLOID)
      endif
   enddo
   ...
   do egi = 1,num_inst_glc
      ...
      if (iamin_GLCID(egi) .and. glc_present) then
         ...
         call shr_sys_flush(logunit)
         call glc_init_mct( EClock_g, cdata_gg(egi), x2g_gg(egi), g2x_gg(egi), NLFilename=NLFilename )
         if (drv_threading) call seq_comm_setnthreads(nthreads_GLOID)
      endif
   enddo
...
!   endif
   ...
   if (atm_present) then
      do eai = 1,num_inst_atm
         if (iamin_ATMID(eai)) then
            ...
            k3 = mct_aVect_indexRa(cdata_aa(eai)%dom%data,"ascale",perrWith='aa ascale')
            cdata_aa(eai)%dom%data%rAttr(k2,:) = cdata_aa(eai)%dom%data%rAttr(k1,:)
            cdata_aa(eai)%dom%data%rAttr(k3,:) = 1.0_r8
            if (drv_threading) call seq_comm_setnthreads(nthreads_GLOID)
         endif
      enddo
   endif
   ...
   if (lnd_present) then
      do eli = 1,num_inst_lnd
         if (iamin_LNDID(eli)) then
            ...
            k3 = mct_aVect_indexRa(cdata_ll(eli)%dom%data,"ascale",perrWith='ll ascale')
            cdata_ll(eli)%dom%data%rAttr(k2,:) = cdata_ll(eli)%dom%data%rAttr(k1,:)
            cdata_ll(eli)%dom%data%rAttr(k3,:) = 1.0_r8
            if (drv_threading) call seq_comm_setnthreads(nthreads_GLOID)
         endif
      enddo
   end if
   ...
   if (rof_present) then
      do eli = 1,num_inst_lnd
         if (iamin_LNDID(eli)) then
            if (drv_threading) call seq_comm_setnthreads(nthreads_LNDID)
            k1 = mct_aVect_indexRa(cdata_rr(eli)%dom%data,"area"  ,perrWith='rr area ')
            k2 = mct_aVect_indexRa(cdata_rr(eli)%dom%data,"aream" ,perrWith='rr aream')
            k3 = mct_aVect_indexRa(cdata_rr(eli)%dom%data,"ascale",perrWith='rr ascale')
            cdata_rr(eli)%dom%data%rAttr(k2,:) = cdata_rr(eli)%dom%data%rAttr(k1,:)
            cdata_rr(eli)%dom%data%rAttr(k3,:) = 1.0_r8
            if (drv_threading) call seq_comm_setnthreads(nthreads_GLOID)
         endif
      enddo
   end if
   ...
   if (sno_present) then
      do eli = 1,num_inst_lnd
         if (iamin_LNDID(eli)) then
            ...
            k3 = mct_aVect_indexRa(cdata_ss(eli)%dom%data,"ascale",perrWith='ss ascale')
            cdata_ss(eli)%dom%data%rAttr(k2,:) = cdata_ss(eli)%dom%data%rAttr(k1,:)
            cdata_ss(eli)%dom%data%rAttr(k3,:) = 1.0_r8
            if (drv_threading) call seq_comm_setnthreads(nthreads_GLOID)
         endif
      enddo
   end if
   ...
   if (ocn_present) then
      do eoi = 1,num_inst_ocn
         if (iamin_OCNID(eoi)) then
            ...
            k3 = mct_aVect_indexRa(cdata_oo(eoi)%dom%data,"ascale",perrWith='oo ascale')
            cdata_oo(eoi)%dom%data%rAttr(k2,:) = cdata_oo(eoi)%dom%data%rAttr(k1,:)
            cdata_oo(eoi)%dom%data%rAttr(k3,:) = 1.0_r8
            if (drv_threading) call seq_comm_setnthreads(nthreads_GLOID)
         endif
      enddo
   endif
   ...
   if (ice_present) then
      do eii = 1,num_inst_ice
         if (iamin_ICEID(eii)) then
            ...
            k3 = mct_aVect_indexRa(cdata_ii(eii)%dom%data,"ascale",perrWith='ii ascale')
            cdata_ii(eii)%dom%data%rAttr(k2,:) = cdata_ii(eii)%dom%data%rAttr(k1,:)
            cdata_ii(eii)%dom%data%rAttr(k3,:) = 1.0_r8
            if (drv_threading) call seq_comm_setnthreads(nthreads_GLOID)
         endif
      enddo
   endif
   ...
   if (glc_present) then
      do egi = 1,num_inst_glc
         if (iamin_GLCID(egi)) then
            ...
            k3 = mct_aVect_indexRa(cdata_gg(egi)%dom%data,"ascale",perrWith='gg ascale')
            cdata_gg(egi)%dom%data%rAttr(k2,:) = cdata_gg(egi)%dom%data%rAttr(k1,:)
            cdata_gg(egi)%dom%data%rAttr(k3,:) = 1.0_r8
            if (drv_threading) call seq_comm_setnthreads(nthreads_GLOID)
         endif
      enddo
   endif
      ...
      do eli = 1,num_inst_lnd
         ...
         if (iamin_CPLLNDID(eli)) then
            if (eli == 1) then
               ...
               call seq_mctext_gsmapInit(gsmap_rr(eli), LNDID(eli), gsmap_rx, CPLID, CPLLNDID(eli))
            endif
            if (iamroot_CPLID) write(logunit,F0I) 'Initializing mapper_Cr2x',eli
            call seq_map_init_rearrsplit(mapper_Cr2x(eli), gsmap_rr(eli), LNDID(eli), gsmap_rx, CPLID, CPLLNDID(eli))
            if (iamroot_CPLID) write(logunit,F0I) 'Initializing mapper_Cx2r',eli
            call seq_map_init_rearrsplit(mapper_Cx2r(eli), gsmap_rx, CPLID, gsmap_rr(eli), LNDID(eli), CPLLNDID(eli))
            call seq_mctext_avInit(r2x_rr(eli), LNDID(eli), r2x_rx(eli), CPLID, gsmap_rx, CPLLNDID(eli))
            if (eli == 1) then
               if (iamroot_CPLID) write(logunit,F0I) 'creating dom_rx'
               call seq_mctext_gGridInit(dom_rr(eli), LNDID(eli), dom_rx, CPLID, gsmap_rx, CPLLNDID(eli))
               call seq_map_map(mapper_Cr2x(eli), dom_rr(eli)%data, dom_rx%data)
            else
               if (iamroot_CPLID) write(logunit,F0I) 'comparing rof domain ensemble number ',eli
               call seq_mctext_avExtend(dom_rx%data,      CPLID,      CPLLNDID(eli))
               call seq_mctext_gGridInit(dom_rr(eli), LNDID(eli), dom_tmp, CPLID, gsmap_rx, CPLLNDID(eli))
               call seq_map_map(mapper_Cr2x(eli), dom_rr(eli)%data, dom_tmp%data)
               if (iamin_CPLID) call seq_domain_compare(dom_rx,dom_tmp,mpicom_CPLID)
               call mct_ggrid_clean(dom_tmp,rc)
            endif
            call mct_avect_init(r2xacc_rx(eli)%data, r2x_rx(eli), mct_aVect_lsize(r2x_rx(eli)))
            call mct_accum_zero(r2xacc_rx(eli))
            r2xacc_rx_cnt = 0
         endif
      enddo
   end if
   ...
   if (ocn_present) then
      do eoi = 1,num_inst_ocn
         ...
         if (iamin_CPLOCNID(eoi)) then
            ...
            call mct_avect_init(x2oacc_ox(eoi)%data, x2o_ox(eoi), mct_aVect_lsize(x2o_ox(eoi)))
            call mct_accum_zero(x2oacc_ox(eoi))
            x2oacc_ox_cnt = 0
         endif
      enddo
   endif
   ...
   if (iamin_CPLID) then
      ...
      do eli = 1,num_inst_lnd
         ...
         call mct_aVect_init(r2x_ox(eli), rList=seq_flds_r2x_fields, lsize=lsize_o)
         call mct_aVect_init(s2x_gx(eli), rList=seq_flds_s2x_fields, lsize=lsize_g)
         call mct_aVect_zero(r2x_ox(eli))
   ...
   if (iamin_CPLID) then
      ...
      if (ocn_present) then
         ...
         if (iamroot_CPLID) write(logunit,*) ' '
         if (iamroot_CPLID) write(logunit,F00) 'Initializing mapper_Sa2o vect'
         call seq_map_initvect(mapper_Sa2o,vect_map,dom_ax,dom_ox, &
              gsmap_s=gsmap_ax,ni=atm_nx,nj=atm_ny,string='mapper_Sa2o initvect')
   ...
   if (atm_present) then
      do eai = 1,num_inst_atm
         if (iamin_CPLATMID(eai)) then
            ...
            if (iamin_ATMID(eai)) then
               call seq_domain_areafactinit(cdata_aa(eai),areacor_aa(eai)%mdl2drv,areacor_aa(eai)%drv2mdl,'areafact_a_'//trim(atm_name(eai)))
   ...
   if (lnd_present) then
      do eli = 1,num_inst_lnd
         if (iamin_CPLLNDID(eli)) then
            ...
            if (iamin_LNDID(eli)) then
               call seq_domain_areafactinit(cdata_ll(eli),areacor_ll(eli)%mdl2drv,areacor_ll(eli)%drv2mdl,'areafact_l_'//trim(lnd_name(eli)))
   ...
   if (rof_present) then
      do eli = 1,num_inst_lnd
         if (iamin_CPLLNDID(eli)) then
            call seq_map_map(mapper_Cx2r(eli), dom_rx%data, dom_rr(eli)%data)
            if (iamin_LNDID(eli)) then
               call seq_domain_areafactinit(cdata_rr(eli),areacor_rr(eli)%mdl2drv,areacor_rr(eli)%drv2mdl,'areafact_r_'//trim(lnd_name(eli)))
               call mct_avect_vecmult(r2x_rr(eli),areacor_rr(eli)%mdl2drv,seq_flds_r2x_fluxes)
            endif
            call seq_map_map(mapper_Cr2x(eli), r2x_rr(eli), r2x_rx(eli))
         endif
      enddo
   end if
   ...
   if (sno_present) then
      do eli = 1,num_inst_lnd
         if (iamin_CPLLNDID(eli)) then
            ...
            if (iamin_LNDID(eli)) then
               call seq_domain_areafactinit(cdata_ss(eli),areacor_ss(eli)%mdl2drv,areacor_ss(eli)%drv2mdl,'areafact_s_'//trim(lnd_name(eli)))
   ...
   if (ocn_present) then
      do eoi = 1,num_inst_ocn
         if (iamin_CPLOCNID(eoi)) then
            ...
            if (iamin_OCNID(eoi)) then
               call seq_domain_areafactinit(cdata_oo(eoi),areacor_oo(eoi)%mdl2drv,areacor_oo(eoi)%drv2mdl,'areafact_o_'//trim(ocn_name(eoi)))
   ...
   if (ice_present) then
      do eii = 1,num_inst_ice
         if (iamin_CPLICEID(eii)) then
            ...
            if (iamin_ICEID(eii)) then
               call seq_domain_areafactinit(cdata_ii(eii),areacor_ii(eii)%mdl2drv,areacor_ii(eii)%drv2mdl,'areafact_i_'//trim(ice_name(eii)))
   ...
   if (glc_present) then
      do egi = 1,num_inst_glc
         if (iamin_CPLGLCID(egi)) then
            ...
            if (iamin_GLCID(egi)) then
               call seq_domain_areafactinit(cdata_gg(egi),areacor_gg(egi)%mdl2drv,areacor_gg(egi)%drv2mdl,'areafact_g_'//trim(glc_name(egi)))
   ...
   if (iamin_CPLID .and. info_debug > 1) then
      ...
      if (lnd_present .or. rof_present .or. sno_present) then
         do eli = 1,num_inst_lnd
            if (lnd_present) then 
               call seq_diag_avect_mct(cdata_lx, l2x_lx(eli),'recv lnd'//trim(lnd_suffix(eli))//' IC')
            end if
            if (rof_present) then
               call seq_diag_avect_mct(cdata_rx, r2x_rx(eli),'recv roff'//trim(lnd_suffix(eli))//' IC')
            end if
            if (sno_present) then
   ...
   if (iamin_CPLID) then
      ...
      do efi = 1,num_inst_frc
         ...
         call seq_frac_init(cdata_ax, cdata_ix, cdata_lx, cdata_ox, cdata_gx, &
                         ice_present, ocn_present, lnd_present, glc_present, dead_comps, &
                         fractions_ax(efi), fractions_ix(efi), fractions_lx(efi), &
                         fractions_ox(efi), fractions_gx(efi), &
                         mapper_Fi2a, mapper_SFo2i, mapper_SFi2o, mapper_Fo2a, &
                         mapper_Fa2o, mapper_Fa2l, mapper_Fl2a)
         if (iamroot_CPLID) write(logunit,*) ' '
         ...
         call seq_frac_set(i2x_ix(eii), &
                        ...
                        fractions_ax(efi), fractions_ix(efi), fractions_lx(efi), fractions_ox(efi), &
                        fractions_gx(efi), mapper_Fi2a, mapper_SFi2o)
      enddo
   ...
   do while ( .not. stop_alarm)
      ...
      if (iamin_CPLID .and. (ice_present.or.ocn_present)) then
         ...
         do eai = 1,num_inst_atm
            ...
            call seq_map_mapvect(mapper_Sa2o,vect_map,a2x_ax(eai),a2x_ox(eai),'Sa_u','Sa_v',norm=.false.)
         enddo
...
!              call mct_accum_average(x2oacc_ox(eoi))
               if (x2oacc_ox_cnt > 0) then
                  x2oacc_ox(eoi)%data%rAttr = x2oacc_ox(eoi)%data%rAttr / (x2oacc_ox_cnt*1.0_r8)
               endif
            enddo
            x2oacc_ox_cnt = 0
            ...
            if (rof_present .and. ocnrof_prognostic) then
               ...
               if (r2xacc_rx_cnt > 0) then
                  ...
                  do eli = 1,num_inst_lnd
                     r2xacc_rx(eli)%data%rAttr = r2xacc_rx(eli)%data%rAttr / &
                                                          (r2xacc_rx_cnt*1.0_r8)
                  enddo
                  r2xacc_rx_cnt = 0
                  ...
                  do eli = 1,num_inst_lnd
                     call seq_map_map(mapper_Fr2o, r2xacc_rx(eli)%data, r2x_ox(eli),norm=.false.)
                  enddo
                  if (do_hist_r2x) then
                     do eli = 1,num_inst_lnd
                        call seq_hist_writeaux(EClock_d,'r2xacc'//trim(lnd_suffix(eli)), &
                                'domr',cdata_rx, r2xacc_rx(eli)%data, rof_nx,rof_ny,1)
                     enddo
                  endif
                  ...
                  do eoi = 1,num_inst_ocn
                     eli = mod((eoi-1),num_inst_lnd) + 1
                     ! The following copies field Forr_roff and Forr_ioff 
                     ! from r2x_ox to x2o_acc_ox
                     call mct_aVect_copy(aVin=r2x_ox(eli), aVout=x2oacc_ox(eoi)%data)
                  enddo
            ...
            if (info_debug > 1) then
               ...
               do eoi = 1,num_inst_ocn
                  call seq_diag_avect_mct(cdata_ox,x2oacc_ox(eoi)%data,'send ocn'//trim(ocn_suffix(eoi)))
               enddo
         ...
         if (ocn_prognostic) then
            do eoi = 1,num_inst_ocn
               if (iamin_CPLOCNID(eoi)) then
                  ...
                  call seq_map_map(mapper_Cx2o(eoi), x2oacc_ox(eoi)%data, x2o_oo(eoi))
         ...
         if (iamin_CPLID) then
            ...
            if (lnd_prognostic) then
               ...
               do eli = 1,num_inst_lnd
                  ...
                  call mrg_x2l_run_mct( cdata_lx, a2x_lx(eai), x2l_lx(eli) )
               enddo
...
!tcx fails     call map_atm2ice_mct( cdata_ax, a2x_ax(eai), cdata_ix, a2x_ix(eai) )
      ...
      if ((lnd_present.or.rof_present.or.sno_present) .and. lndrun_alarm) then
         do eli = 1,num_inst_lnd
            if (iamin_LNDID(eli)) then
               ...
               call lnd_run_mct( EClock_l, cdata_ll(eli), x2l_ll(eli), l2x_ll(eli), &
                                           ...
                                           cdata_ss(eli), x2s_ss(eli), s2x_ss(eli))
               call mct_avect_vecmult(l2x_ll(eli),areacor_ll(eli)%mdl2drv,seq_flds_l2x_fluxes)
               if (rof_present) call mct_avect_vecmult(r2x_rr(eli),areacor_rr(eli)%mdl2drv,seq_flds_r2x_fluxes)
         ...
         enddo   ! eli
      end if   ! (lnd_present.or.rof_present.or.sno_present) .and. lndrun_alarm
      ...
      if (ocn_present .and. iamin_CPLID) then
         ...
         if (ocn_prognostic) then
            ...
            do eoi = 1,num_inst_ocn
!     !        call mct_accum_accumulate(x2o_ox(eoi), x2oacc_ox(eoi))
               if (x2oacc_ox_cnt == 0) then
                  x2oacc_ox(eoi)%data%rAttr = x2o_ox(eoi)%rAttr
               else
                  x2oacc_ox(eoi)%data%rAttr = x2oacc_ox(eoi)%data%rAttr + x2o_ox(eoi)%rAttr
               endif
            enddo
      ...
      if ((lnd_present.or.rof_present.or.sno_present) .and. lndrun_alarm) then
         ...
         do eli = 1,num_inst_lnd
            ...
            if (iamin_CPLLNDID(eli)) then
               ...
               if (rof_present) then
                  call t_drvstartf ('driver_l2c_rofr2rofx',barrier=mpicom_CPLLNDID(eli))
                  call seq_map_map(mapper_Cr2x(eli), r2x_rr(eli), r2x_rx(eli))
            ...
            if (iamin_CPLID) then
               ...
               if (info_debug > 1) then
                  ...
                  if (rof_present) then
                     call seq_diag_avect_mct(cdata_rx, r2x_rx(eli),'recv roff'//trim(lnd_suffix(eli)))
                  endif
               ...
               if (rof_present .and. ocnrof_prognostic) then
                  ...
                  if (r2xacc_rx_cnt == 0) then
                     r2xacc_rx(eli)%data%rAttr = r2x_rx(eli)%rAttr * flux_epbalfact
                  else
                     r2xacc_rx(eli)%data%rAttr = r2xacc_rx(eli)%data%rAttr + r2x_rx(eli)%rAttr * flux_epbalfact
                  endif
      ...
      if (iamin_CPLID .and. do_budgets) then
         ...
         if (rof_present) then
            call seq_diag_rtm_mct(dom_rx, r2x_r=r2x_rx(ens1))
         endif
         if (ocn_present) then
            call seq_diag_ocn_mct(dom_ox, fractions_ox(ens1), o2x_o=o2x_ox(ens1), x2o_o=x2o_ox(ens1), xao_o=xao_ox(ens1), r2x_o=r2x_ox(ens1))
         endif
         if (ice_present) then
...
subroutine ccsm_comp_init(comp, importState, exportState, clock, rc)
   ...
   integer             :: localrc
   type(ESMF_GridComp) :: atmComp, lndComp, iceComp, ocnComp, glcComp, cplComp
   type(ESMF_VM)       :: vm
