  ...
  subroutine mrg_x2a_run_mct( cdata_a, l2x_a, o2x_a, xao_a, i2x_a, fractions_a, x2a_a )
    ...
    if (first_time) then
       ...
       do ka = 1,naflds
          ...
          if (field_atm(1:11) == 'Fall_flxdst') then
             cycle !BUG TO BE REMOVED - only here for backwards compatibility
          end if
...
!--------------------------------------------------------------------------
  ...
  subroutine getfld(n, av, field, suffix)
    ...
    if (field(1:1) /= 'S' .and. field(1:1) /= 'F') then
       write(6,*)'field attribute',trim(field),' must start with S or F' 
       call shr_sys_abort()
    end if
