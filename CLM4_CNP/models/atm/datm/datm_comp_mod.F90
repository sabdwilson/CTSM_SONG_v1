...
!--------------------------------------------------------------------------
  ...
  ! kc13o2d added by JFM and DMR for C13, 08/22/11
  integer(IN) :: krc,krl,ksc,ksl,kswndr,kswndf,kswvdr,kswvdf,kswnet,kco2p,kco2d,kc13o2d
  ...
  integer(IN) :: sprecc,sprecl,sprecn,sco2p,sco2d,sc13o2d,sswup,sprec,starcf
...
! modified by JFM and DMR for C13, 08/22/11 
! integer(IN),parameter :: ktrans = 56
  ...
  character(16),parameter  :: avofld(1:ktrans) = &
     (/"Sa_z            ","Sa_u            ","Sa_v            ","Sa_tbot         ", &
       ...
       "Faxx_fdms       ","Sa_c13o2diag    "                                       /)    !c13 added by JFM and DMR, 08/22/11
  character(16),parameter  :: avifld(1:ktrans) = &
     (/"z               ","u               ","v               ","tbot            ", &
       ...
       "dms             ","c13o2diag       "                                       /)
! added by JFM and DMR for C13, 08/22/11

! modified by JFM and DMR for C13, 08/22/11     
!  integer(IN),parameter :: ktranss = 19
  integer(IN),parameter :: ktranss = 20
  character(16),parameter  :: stofld(1:ktranss) = &
     (/"strm_tbot       ","strm_wind       ","strm_z          ","strm_pbot       ", &
       ...
       "strm_swup       ","strm_prec       ","strm_tarcf      ","strm_c13o2diag  " /)
...
!-------------------------------------------------------------------------------
 ...
 if (phase == 1) then
   ...
   &                          scmlat=scmlat, scmlon=scmLon)
    ...
    call shr_strdata_pioinit(SDATM, seq_io_getiosys(trim(inst_name)), &
                                    seq_io_getiotype(trim(inst_name)))
    ...
    ! added by JFM and DMR for C13, 08/22/11
    kc13o2d = mct_aVect_indexRA(a2x,'Sa_c13o2diag',perrWith='quiet')
  ...
  ! modified by JFM for C13, 10/21/11 
    sc13o2d= mct_aVect_indexRA(avstrm,'strm_c13o2d',perrWith='quiet')
