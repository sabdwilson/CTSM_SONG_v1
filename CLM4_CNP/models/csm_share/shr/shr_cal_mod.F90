...
#ifdef USE_ESMF_LIB 
      call ESMF_TimeSet(ltime1,yy=year,mm=month,dd=1,s=0,calkindflag=ESMF_CALKIND_GREGORIAN,rc=rc)
      if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
      if (month < 12) then
         call ESMF_TimeSet(ltime2,yy=year,mm=month+1,dd=1,s=0,calkindflag=ESMF_CALKIND_GREGORIAN,rc=rc)
         if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
      else
         call ESMF_TimeSet(ltime2,yy=year+1,mm=1,dd=1,s=0,calkindflag=ESMF_CALKIND_GREGORIAN,rc=rc)
         if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
      endif
...
!-------------------------------------------------------------------------------
   ...
   if (trim(lcalendar) == trim(shr_cal_gregorian)) then
#ifdef USE_ESMF_LIB 
      call ESMF_TimeSet(ltime1,yy=year,mm=1,dd=1,s=0,calkindflag=ESMF_CALKIND_GREGORIAN,rc=rc)
      if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
      call ESMF_TimeSet(ltime2,yy=year+1,mm=1,dd=1,s=0,calkindflag=ESMF_CALKIND_GREGORIAN,rc=rc)
      if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
      call ESMF_TimeGet(ltime1,d=eday1,rc=rc)
...
!-------------------------------------------------------------------------------
   ...
   if (trim(lcalendar) == trim(shr_cal_gregorian)) then
#ifdef USE_ESMF_LIB 
      call ESMF_TimeSet(ltime,d=eday,s=0,calkindflag=ESMF_CALKIND_GREGORIAN,rc=rc)
      if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
...
!-------------------------------------------------------------------------------
   ...
   if (trim(lcalendar) == trim(shr_cal_gregorian)) then
#ifdef USE_ESMF_LIB 
      call ESMF_TimeSet(ltime,yy=year,mm=month,dd=day,s=0,calkindflag=ESMF_CALKIND_GREGORIAN,rc=rc)
      if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
