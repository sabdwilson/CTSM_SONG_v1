...
#
#  Daniel M. Riccciuto, Dali Wang, Peter E. Thornton, Wilfred M. Post
...
svnurl="$HeadURL: https://svn-ccsm-models.cgd.ucar.edu/PTCLM/trunk_tags/PTCLM1_111129/PTCLM.py $"
...
options.add_option("--regional", action="store_true", \
                   dest="regional", default=False, \
                   help="Flag for regional run (2x2 or greater)")
options.add_option("--nyears_ad_spinup", default=600, \
                   help="number of years for ad spinup")
options.add_option("--croot", dest="mycasesroot", default="./", \
                    help="Directory where the case would be created")
options.add_option("--rroot", dest="myrunroot", default="./", \
                    help="Directory where the run would be created")
options.add_option("--cleanlogs",dest="cleanlogs", help=\
                   "Removes temporary and log files that are created",\
                   default=False,action="store_true")
options.add_option("--numxpts", dest="mynumxpts", default="", \
                    help="Number of points in Longitude-direction")
options.add_option("--numypts", dest="mynumypts", default="", \
                    help="Number of points in Latitude-direction")
options.add_option("--sitee", dest="mysitee", default="", \
                    help="Site eastern edge")
options.add_option("--sitew", dest="mysitew", default="", \
                    help="Site wastern edge")
options.add_option("--siten", dest="mysiten", default="", \
                    help="Site northern edge")
options.add_option("--sites", dest="mysites", default="", \
                    help="Site southern edge")
options.add_option("--nofire", action="store_true", dest="nofire", default=False, \
                    help="To turn off wildfires")
options.add_option("--npoolmod", action="store_true", dest="npoolmod", default=False, \
                    help="To turn on nitrogen pool modifications")
options.add_option("--q10wbmod", action="store_true", dest="q10wbmod", default=False, \
                    help="To turn on Woodrow-Berry Q10 curve")
options.add_option("--tfmod", action="store_true", dest="tfmod", default=False, \
                    help="To set temperature threshold (0 degC) for plant wilting factor")
options.add_option("--makemetdata", action="store_true", dest="makemet", \
                        default=False, help='Make met data')
options.add_option("--metdir", dest="metdir", default="", \
                        help='Make met data')
...
indatgengroup.add_option("--soilgrid", dest="soilgrid", help = \
                  "Use soil information from global gridded file (rather than site data)",\
                   action="store_true", default=False)
versiongroup  = OptionGroup( parser, "Main Script Version Id: $Id: PTCLM.py 32434 2011-11-30 05:32:20Z erik $ Scripts URL: "+svnurl )
...
def Get_envconf_Value( var ):
     ...
     pwd = os.getcwd()
     os.chdir( mycase)
     os.system("grep "+var+" env_conf.xml > envvar.txt")
     input = open('envvar.txt')
     for s in input:
        #env_val = re.search('value="([^"]*)"', os.system("grep "+var+" env_conf.xml").read() ).group(1)
        env_val = re.search('value="([^"]*)"', s ).group(1)
...
def xmlchange_envbuild_value( var, value ):
     'Function to set the value of a variable in the env_build.xml file'
     cmd = "./xmlchange -file env_build.xml -id "+var+" -val "+value
     system( cmd )
#
...
if mycasename.startswith("/"): mycase = mycasename
   ...
   if ( options.mycasesroot == "./"):
      mycase = abs_base_cesm+"/scripts/"+mycasename
   else:
      mycase = options.mycasesroot+"/"+mycasename
...
if (finidat == " "):
    ...
    if ( not os.path.isfile(finidat) ):
        print( "Warning:  The input finidat file: "+finidat+" does NOT exist" )
...
for row in AFdatareader:
    ...
    if row[0] == mysite:
        ...
        if options.regional == True:
           ...
           lat=lat-resy/2
           lon=lon-resx/2
        #make meteorology data
        if (options.makemet):
             print(" Making meteorological data for site")
             os.system('python '+base_cesm+'/scripts/makemetdata.py' \
                            +' --site '+mysite+' --lat '+row[4]+' --lon '+ \
                            row[3]+' --ccsm_input '+options.ccsm_input+ \
                            ' --metdir '+options.metdir+' --startyear '+row[6]+ \
                            ' --endyear '+row[7])

if ( options.mynumxpts != "" ):
   numxpts=int(options.mynumxpts)
if ( options.mynumypts != "" ):
   numypts=int(options.mynumypts)
...
if ( suprtclm1pt ):
   ...
   clmusrdatname    = str(numxpts)+"x"+str(numypts)+"pt_"+mysite
   ...
   #DMR 4/11/13 - add case ID to PFT-physiology file
   pft_phys_out     = re.search( "(.+)\.nc$",pft_phys_file ).group(1)+"."+ \
                      options.mycaseid+mysite+".nc"
...
#overwrite default length for ad_spinup
if (options.ad_spinup):
     myrun_n = int(options.nyears_ad_spinup)
...
if makeptfiles:
    ...
    for s in input:
        ...
        if line == 3:
           if(options.mynumxpts == ''):
              output.write(" mksrf_lsmlon = "+str(numxpts)+"\n")
           else:
              output.write(" mksrf_lsmlon = "+options.mynumxpts+"\n")
        if line == 4:
           if(options.mynumypts == ''):
              output.write(" mksrf_lsmlat = "+str(numypts)+"\n")
           else:
              output.write(" mksrf_lsmlat = "+options.mynumypts+"\n")
        if line == 5:
           if(options.mysitee == ''):
              output.write(s.replace("SITEE",str(lon+numxpts*resx)))
           else:
              output.write(s.replace("SITEE",options.mysitee))
        if line == 6:
           if(options.mysitew == ''):
              output.write(s.replace("SITEW",str(lon)))
           else:
              output.write(s.replace("SITEW",options.mysitew))
        if line == 7:
           if(options.mysites == ''):
            output.write(s.replace("SITES",str(lat)))
           else:
              output.write(s.replace("SITES",options.mysites))
        if line == 8:
           if(options.mysiten == ''):
            output.write(s.replace("SITEN",str(lat+numypts*resy)))
           else:
              output.write(s.replace("SITEN",options.mysiten))
        if line > 8:
            output.write(s)
        #if line == 1:
        #    output.write(s)
    ...
    system("/bin/mv -f ./fracdata_"+str(numxpts).zfill(4)+"x"+str(numypts).zfill(4)+".nc "+fracfile )
    system("/bin/mv -f ./griddata_"+str(numxpts).zfill(4)+"x"+str(numypts).zfill(4)+".nc "+gridfile )
    if options.cleanlogs:
       system("rm -f mkgriddata.log")
       system("rm -f "+clm_tools+"/mkgriddata/namelist")
    ...
    if ((not options.owritesrfaer) and os.path.exists( surffile) and \
            ...
            for row in AFdatareader:
                ...
                if row[0] == mysite:
                    ...
                    for thispft in range(0,5):
                        ...
                        pft_code[thispft]=int(float(row[2+2*thispft]))
            if ( found == 0 ):
        ...
        if options.cleanlogs:
           system("rm -f tempsitePFT.txt")
           system("rm -f tempsitesoil.txt")
           system("rm -f mksurfdata.log")
           system("rm -f pftdyn_override_hist_simyr*")
           system("rm -f surfdata.pftdyn_"+clmres+".nc")
    ...
    if options.cleanlogs:
       system("rm -f mkdatadomain.log")
       system("rm -f "+clm_tools+"/mkdatadomain/namelist")
       system("rm -f namelist")
       system("rm -f clm.input_data_files")
    ...
    #DMR 5/5/11 - always create this file, give write permission
    #if ( not os.path.exists( pft_phys_out ) ):
    system("/bin/cp "+pft_phys_file+" "+pft_phys_out )
    system("chmod u+w "+pft_phys_out )
...
#assume we want historically varying CO2 for transient run

if ( mycompset == "I20TRCN" ):
   xmlchange_envconf_value("CCSM_BGC", "CO2A" )
   xmlchange_envconf_value("CLM_CO2_TYPE", "diagnostic")

###### SET Spinup and ENV_RUN.XML VALUES ################################################
if (options.nofire):
     clmconfigopts = clmconfigopts+" -nofire"
if (options.npoolmod):
     clmconfigopts = clmconfigopts+" -npoolmod"
if (options.q10wbmod):
     clmconfigopts = clmconfigopts+" -q10wbmod"
if (options.tfmod):
     clmconfigopts = clmconfigopts+" -tfmod"

print "CLMCONFIGOPTS: "+ clmconfigopts
...
if (ad_spinup):
   ...
   nystad  = str(10000+int(options.nyears_ad_spinup)+1)
   adststr = nystad[1:]
   xmlchange_envrun_value( "STOP_DATE",   adststr+'0101'       )
elif (exit_spinup):
   nystad  = str(10000+int(options.nyears_ad_spinup)+1)
   adststr = nystad[1:]
   hist_mfilt  = 12
   ...
   xmlchange_envconf_value( "RUN_REFDATE",         adststr+"-01-01" )
   xmlchange_envconf_value( "GET_REFCASE",         "FALSE" )
elif (final_spinup):
   hist_mfilt  = 12*int(myrun_n)
   xmlchange_envconf_value( "CLM_CONFIG_OPTS",     "'"+clmconfigopts+"'")
...
rest_n = max(1, int(myrun_n)) #max( 1, int(myrun_n) // 5 )
...
if (options.myrunroot != "./"):
   xmlchange_envbuild_value("EXEROOT",options.myrunroot+"/"+mycasename)
   xmlchange_envbuild_value("OBJROOT",options.myrunroot+"/"+mycasename)
   xmlchange_envbuild_value("LIBROOT",options.myrunroot+"/"+mycasename+"/lib")
   xmlchange_envrun_value("DOUT_S_ROOT",options.myrunroot+"/archive/"+mycasename)

if plev>0: print "Scripts created successfully\n"
if plev>0: 
   if ( options.mycasesroot == "./" ):
      print "cd "+mycase+" and then..."
   else:
      print "cd "+options.mycasesroot+"/"+mycasename+" and then..."
