module psens_data
  ...
  character(len=800) basecase_last
  ...
  integer n_param, nchunks, chunksize
  logical docontinue
     ...
     if (trim(sample_file) .ne. 'none' .and. trim(sample_file) .ne. '') then 
        ...
        do 
           ...
           do i=1,np_all
              ...
              if (useparm(i) .eq. 2) then 
                 !apply parameter constraints
                 myrand(i,j) = 1.0 - myrand(i-1,j) - myrand(i-2,j)
              end if
           end do
           j=j+1
        end do
...
5    continue
  ...
  call MPI_BCAST(myrand, np_all*100000, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
  call MPI_BCAST(ngroups, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)

  do p=myid+1,ngroups,numprocs
     print*, myid, ngroups, p
     call setup_psens(p)
  end do
...
subroutine setup_psens(p)
  ...
  basecase = trim(basecase_in) // '_I1850CN_ad_spinup'
  do c=1,ncases  !loop through all four cases if requested
     basecase_last = trim(basecase)
     if (ncases .eq. 4) then 
     ...
     surffile_in = trim(ccsm_input) // '/lnd/clm2/surfdata/' &
          // 'surfdata_1x1pt_' // trim(basecase) // '_simyr1850.nc'
     ...
     if (p .eq. 1) then 
        ...
        rundir = trim(clm_rundir) // '/UQ/' // trim(basecase) // '/g' // pst(2:6)
     else 
        do i=1,np_node
...
subroutine setup_rundir(rundir, pftfile, surffile, c)
  ...
  cmd = 'cp -r ' // trim(ccsm_input) // '/atm/datm7/CLM1PT_data/2HR/1x1pt_' // mysite //  &
        ' ./'
  call system(cmd)
  ...
  do i=1,np_node
     ...
     do j=1,100
        ...
        if (line(1:8) .eq. " fpftcon" .or. line(1:9) .eq. "  fpftcon") then 
           ...
           write(9,*) " nrevsn = './" // trim(basecase_last) // ".clm2_" // lndst(2:5) // ".r." &
	        // nyst(2:5) // "-01-01-00000.nc'"
        else if (c .eq. 3 .and. line(1:8) .eq. ' finidat' .or. line(1:9) .eq. '  finidat') then 
           write(nyst,'(I5)') 10000+nyears_ad + nyears_exit + 1
           write(9,*) " finidat = './" // trim(basecase_last) // ".clm2_" // lndst(2:5) // ".r." &
                // nyst(2:5) // "-01-01-00000.nc'"
...
subroutine modifyparms(pftfile, surffile, p)
  ...
  double precision parm_fac, data(npfts), data1d, datasoil(1,1,10)
  ...
  do i=1,np_all
     if (useparm(i) .ge. 1) then 
        ...
        if (i .le. np) then           !PFT-specific
           ...
           if (trim(parm_name) .eq. 'BCH' .or. trim(parm_name) .eq. &
                'KS' .or. trim(parm_name) .eq. 'THETAS' .or. &
                trim(parm_name) .eq. 'PSIS') then 
              !variables with 10 values
              RCODE = NF90_GET_VAR(NCID, varid, datasoil)
              datasoil(1,1,:) = myrand(i,p)
              RCODE = NF90_PUT_VAR(NCID, varid, datasoil)    
           else
              RCODE = NF90_GET_VAR(NCID, varid, data1d)
              data1d = myrand(i,p)
              RCODE = NF90_PUT_VAR(NCID, varid, data1d)     
